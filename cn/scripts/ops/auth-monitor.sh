...
#
# Suggested cron: */30 * * * * /home/admin/openclawcn/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenClawCN notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openclawcn-alerts)
...
STATE_FILE="$HOME/.openclawcn/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenClawCN if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openclawcn
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenClawCN to $NOTIFY_PHONE..."
            openclawcn send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenClawCN is down. Run: ssh l36 '~/openclawcn/scripts/mobile-reauth.sh'" "urgent"
    exit 1
