...
import Testing
@testable import OpenClawCN

@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: OpenClawCNProtocol.AnyCodable(["ok": OpenClawCNProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OpenClawCNProtocol.AnyCodable(["ok": OpenClawCNProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = OpenClawCNProtocol.AnyCodable([
            "runId": OpenClawCNProtocol.AnyCodable("run-1"),
            "sessionKey": OpenClawCNProtocol.AnyCodable("main"),
            "state": OpenClawCNProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OpenClawCNProtocol.AnyCodable(["a": OpenClawCNProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
