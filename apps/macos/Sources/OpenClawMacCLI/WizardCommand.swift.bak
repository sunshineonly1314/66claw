...
func runWizardCommand(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        openclawcncn-mac wizard

        Usage:
          openclawcncn-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
...
actor GatewayWizardClient {
    ...
    private func sendConnect() async throws {
        ...
        let clientId = "openclawcncn-macos"
        let clientMode = "ui"
        let role = "operator"
        ...
        let client: [String: ProtoAnyCodable] = [
            ...
            "displayName": ProtoAnyCodable(Host.current().localizedName ?? "OpenClawCN macOS Wizard CLI"),
