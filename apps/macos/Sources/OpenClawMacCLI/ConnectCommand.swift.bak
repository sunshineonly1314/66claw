...
struct ConnectOptions {
    ...
    var clientId: String = "openclawcncn-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        openclawcncn-mac connect

        Usage:
          openclawcncn-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: openclawcncn-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "OpenClawCN macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("OpenClawCN macOS Gateway Connect")
