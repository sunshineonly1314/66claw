...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.openclawcn/openclawcn.json. Prefer OPENCLAWCN_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.openclawcn/openclawcn.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.openclawcn/openclawcn.json. Restart the gateway to apply."
        }
