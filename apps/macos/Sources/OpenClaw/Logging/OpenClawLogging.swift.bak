...
enum OpenClawCNLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenClawCNOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenClawCNFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenClawCNLogging.bootstrapIfNeeded()
        let label = OpenClawCNLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenClawCNOSLogHandler: LogHandler {
...
struct OpenClawCNFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenClawCNLogging.parseLabel(self.label)
