...
private let deepLinkLogger = Logger(subsystem: "ai.openclawcn", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "OpenClawCN is paused", message: "Unpause OpenClawCN to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run OpenClawCN agent?", message: body) else { return }
        }
