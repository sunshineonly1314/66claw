...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.openclawcn", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenClawCNSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenClawCNSessionPreviewEntry,
    ...
    private static func cache(payload: OpenClawCNSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenClawCNChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenClawCNKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenClawCNKit.AnyCodable]) -> [OpenClawCNChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenClawCNChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenClawCNChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenClawCNChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenClawCNChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenClawCNChatMessage) -> String? {
