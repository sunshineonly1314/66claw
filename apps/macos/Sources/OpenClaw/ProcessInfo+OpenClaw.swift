...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.openclawcn.mac`) even if the shipped
    ...
    {
        if environment["OPENCLAWCN_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "openclawcn.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "openclawcn.nixMode") { return true }
