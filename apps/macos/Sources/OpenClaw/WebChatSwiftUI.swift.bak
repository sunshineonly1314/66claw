...
private let webChatSwiftLogger = Logger(subsystem: "ai.openclawcn", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OpenClawCNChatTransport, Sendable {
    func requestHistory(sessionKey: String) async throws -> OpenClawCNChatHistoryPayload {
    ...
    func listSessions(limit: Int?) async throws -> OpenClawCNChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OpenClawCNChatSessionsListResponse.self, from: data)
    }
    ...
    func sendMessage(
        ...
        attachments: [OpenClawCNChatAttachmentPayload]) async throws -> OpenClawCNChatSendResponse
    {
    ...
    func events() -> AsyncStream<OpenClawCNChatTransportEvent> {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenClawCNChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpenClawCNGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    OpenClawCNChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    OpenClawCNAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenClawCNChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenClawCNChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenClawCNChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OpenClawCNChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenClawCNChatView>) -> NSViewController
    {
