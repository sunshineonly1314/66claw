...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        if self.isCanvasCommand(command), !Self.canvasEnabled() {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OpenClawCNNodeError(
                    code: .unavailable,
                    message: "CANVAS_DISABLED: enable Canvas in Settings"))
        }
        do {
            switch command {
            case OpenClawCNCanvasCommand.present.rawValue,
                 OpenClawCNCanvasCommand.hide.rawValue,
                 OpenClawCNCanvasCommand.navigate.rawValue,
                 OpenClawCNCanvasCommand.evalJS.rawValue,
                 OpenClawCNCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case OpenClawCNCanvasA2UICommand.reset.rawValue,
                 OpenClawCNCanvasA2UICommand.push.rawValue,
                 OpenClawCNCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleA2UIInvoke(req)
            case OpenClawCNCameraCommand.snap.rawValue,
                 OpenClawCNCameraCommand.clip.rawValue,
                 OpenClawCNCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case OpenClawCNLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MacNodeScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            case OpenClawCNSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case OpenClawCNSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case OpenClawCNSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case OpenClawCNSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case OpenClawCNSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OpenClawCNCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(OpenClawCNCanvasPresentParams.self, from: req.paramsJSON)) ??
                OpenClawCNCanvasPresentParams()
        ...
        case OpenClawCNCanvasCommand.hide.rawValue:
            let sessionKey = self.mainSessionKey
            await MainActor.run {
                CanvasManager.shared.hide(sessionKey: sessionKey)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OpenClawCNCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(OpenClawCNCanvasNavigateParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
            try await MainActor.run {
                _ = try CanvasManager.shared.show(sessionKey: sessionKey, path: params.url)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OpenClawCNCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(OpenClawCNCanvasEvalParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
        ...
        case OpenClawCNCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(OpenClawCNCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case OpenClawCNCanvasA2UICommand.push.rawValue,
             OpenClawCNCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        switch req.command {
        case OpenClawCNCameraCommand.snap.rawValue:
            let params = (try? Self.decodeParams(OpenClawCNCameraSnapParams.self, from: req.paramsJSON)) ??
                OpenClawCNCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OpenClawCNCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(OpenClawCNCameraClipParams.self, from: req.paramsJSON)) ??
                OpenClawCNCameraClipParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OpenClawCNCameraCommand.list.rawValue:
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(OpenClawCNLocationGetParams.self, from: req.paramsJSON)) ??
            OpenClawCNLocationGetParams()
        ...
        do {
            ...
            let payload = OpenClawCNLocationPayload(
    ...
    private func handleA2UIReset(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        (() => {
          const host = globalThis.openclawcnA2UI;
          if (!host) return JSON.stringify({ ok: false, error: "missing openclawcnA2UI" });
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [OpenClawCNKit.AnyCodable]
        if command == OpenClawCNCanvasA2UICommand.pushJSONL.rawValue {
            let params = try Self.decodeParams(OpenClawCNCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
            messages = try OpenClawCNCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
        } else {
            do {
                let params = try Self.decodeParams(OpenClawCNCanvasA2UIPushParams.self, from: req.paramsJSON)
        ...
        let messagesJSON = try OpenClawCNCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
        let js = """
        (() => {
          try {
            const host = globalThis.openclawcnA2UI;
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__openclawcncn__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(OpenClawCNSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(OpenClawCNSystemWhichParams.self, from: req.paramsJSON)
    ...
    private func resolveSystemRunApproval(
        ...
        params: OpenClawCNSystemRunParams,
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(OpenClawCNSystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> OpenClawCNLocationMode {
        let raw = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        return OpenClawCNLocationMode(rawValue: raw) ?? .off
    }
    ...
    private static func errorResponse(
        ...
        code: OpenClawCNNodeErrorCode,
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: OpenClawCNNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: OpenClawCNCanvasSnapshotFormat,
        maxWidth: Int?,
