...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenClawCNProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.openclawcn", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenClawCNKit.AnyCodable]()) {
                $0[$1.key] = OpenClawCNKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenClawCNProtocol.AnyCodable?) -> [String: OpenClawCNProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenClawCNProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenClawCNKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenClawCNProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openclawcn.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openclawcn.control.agent")
}
