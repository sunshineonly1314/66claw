...
final class CanvasA2UIActionMessageHandler: NSObject, WKScriptMessageHandler {
    static let messageName = "openclawcnCanvasA2UIAction"
    ...
    func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        guard let name = OpenClawCNCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId =
        ...
        let contextJSON = OpenClawCNCanvasA2UIAction.compactJSON(userAction["context"])

        // Token-efficient and unambiguous. The agent should treat this as a UI event and (by default) update Canvas.
        let messageContext = OpenClawCNCanvasA2UIAction.AgentMessageContext(
            actionName: name,
        ...
        let text = OpenClawCNCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        Task { [weak webView] in
            ...
            await MainActor.run {
                ...
                let js = OpenClawCNCanvasA2UIAction.jsDispatchA2UIActionStatus(
                    actionId: actionId,
                    ok: result.ok,
                    error: result.error)
    ...
    // Formatting helpers live in OpenClawCNKit (`OpenClawCNCanvasA2UIAction`).
}
