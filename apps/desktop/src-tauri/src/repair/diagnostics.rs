...
fn check_node_binary() -> DiagnosticCheck {
    ...
    if std::env::var("TAURI_DEV").is_ok() || cfg!(debug_assertions) {
        return check("node_binary_exists", "Node.js 运行时", CheckStatus::Warn,
            "开发模式: Node.js 不需要捆绑", None);
    }
    check("node_binary_exists", "Node.js 运行时", CheckStatus::Fail,
        "捆绑的 Node.js 运行时未找到，安装可能不完整", None)
}
...
fn check_master_key(state_dir: &std::path::Path) -> DiagnosticCheck {
    let key_path = state_dir.join(".master-key");
    if key_path.exists() {
        match fs::metadata(&key_path) {
            Ok(meta) if meta.len() == 32 => {
                check("master_key_exists", "加密主密钥", CheckStatus::Pass,
                    ".master-key 存在且大小正确 (32 字节)", None)
            }
            Ok(meta) => {
                check("master_key_exists", "加密主密钥", CheckStatus::Warn,
                    &format!(".master-key 大小异常: {} 字节（应为 32）", meta.len()), None)
            }
            Err(_) => {
                check("master_key_exists", "加密主密钥", CheckStatus::Warn,
                    ".master-key 无法读取", None)
            }
        }
    } else {
        check("master_key_exists", "加密主密钥", CheckStatus::Warn,
            ".master-key 不存在（凭证解密不可用）", None)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_check_master_key_valid() {
        let temp = std::env::temp_dir().join("_repair_diag_masterkey_ok");
        let _ = fs::create_dir_all(&temp);
        fs::write(temp.join(".master-key"), vec![0u8; 32]).unwrap();

        let result = check_master_key(&temp);
        assert!(matches!(result.status, CheckStatus::Pass));
        assert!(result.message.contains("32"));

        let _ = fs::remove_dir_all(&temp);
    }

    #[test]
    fn test_check_master_key_wrong_size() {
        let temp = std::env::temp_dir().join("_repair_diag_masterkey_bad");
        let _ = fs::create_dir_all(&temp);
        fs::write(temp.join(".master-key"), vec![0u8; 16]).unwrap();

        let result = check_master_key(&temp);
        assert!(matches!(result.status, CheckStatus::Warn));
        assert!(result.message.contains("16"));

        let _ = fs::remove_dir_all(&temp);
    }

    #[test]
    fn test_check_master_key_missing() {
        let temp = std::env::temp_dir().join("_repair_diag_masterkey_none");
        let _ = fs::create_dir_all(&temp);
        let _ = fs::remove_file(temp.join(".master-key"));
    ...
    // ── node binary & backend entry (dev mode) ──────────────────────

    #[test]
    fn test_check_node_binary_dev_mode() {
        // In debug builds (test), should return Warn (dev mode)
        let result = check_node_binary();
        assert_eq!(result.id, "node_binary_exists");
        // In test (debug) mode, should be Warn or Pass
        assert!(matches!(result.status, CheckStatus::Pass | CheckStatus::Warn));
    }

    #[test]
    fn test_check_backend_entry_dev_mode() {
