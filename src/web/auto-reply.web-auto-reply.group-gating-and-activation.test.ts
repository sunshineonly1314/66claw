...
describe("web auto-reply", () => {
  ...
  it("allows group messages when whatsapp groups default disables mention gating", async () => {
    ...
    setLoadConfigMock(() => ({
      ...
      messages: { groupChat: { mentionPatterns: ["@openclawcn"] } },
    }));
  ...
  it("detects LID mentions using authDir mapping", async () => {
    ...
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-wa-auth-"));
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    // Text-based mentionPatterns still work (user can type "openclawcn" explicitly).
    await capturedOnMessage?.({
      body: "openclawcn ping",
      from: "123@g.us",
      conversationId: "123@g.us",
      chatId: "123@g.us",
      chatType: "group",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/openclawcn-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/openclawcn-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
