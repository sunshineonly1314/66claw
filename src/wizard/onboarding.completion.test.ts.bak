...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "openclawcn",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/openclawcn.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("openclawcn");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "openclawcn");
