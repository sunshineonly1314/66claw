...
const requireAgentDir = () => {
  const agentDir = process.env.OPENCLAWCN_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENCLAWCN_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.OPENCLAWCN_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENCLAWCN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-oauth-"));
    process.env.OPENCLAWCN_STATE_DIR = tempStateDir;
    process.env.OPENCLAWCN_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENCLAWCN_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENCLAWCN_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-minimax-"));
    process.env.OPENCLAWCN_STATE_DIR = tempStateDir;
    process.env.OPENCLAWCN_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENCLAWCN_AGENT_DIR;
