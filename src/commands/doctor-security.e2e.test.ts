...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.OPENCLAWCN_GATEWAY_TOKEN;
    prevPassword = process.env.OPENCLAWCN_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.OPENCLAWCN_GATEWAY_PASSWORD = prevPassword;
    }
  });
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as OpenClawCNConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.OPENCLAWCN_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as OpenClawCNConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as OpenClawCNConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as OpenClawCNConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
