...
beforeAll(() => {
  previousProfile = process.env.OPENCLAWCN_PROFILE;
  process.env.OPENCLAWCN_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPENCLAWCN_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/openclaw-root.js", () => ({
  resolveOpenClawCNPackageRoot: vi.fn().mockResolvedValue("/tmp/openclawcn"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openclawcn/pnpm-lock.yaml",
      markerPath: "/tmp/openclawcn/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("OpenClawCN status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openclawcn status --all") ||
          l.includes("openclawcn --profile isolated status --all") ||
          l.includes("openclawcn status --all") ||
          l.includes("openclawcn --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    const prevToken = process.env.OPENCLAWCN_GATEWAY_TOKEN;
    process.env.OPENCLAWCN_GATEWAY_TOKEN = "abcd1234";
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.OPENCLAWCN_GATEWAY_TOKEN = prevToken;
      }
    }
  });
