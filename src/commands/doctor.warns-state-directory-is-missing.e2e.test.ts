...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPENCLAWCN_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENCLAWCN_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/openclawcn.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPENCLAWCN_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
