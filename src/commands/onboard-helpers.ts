...
export function guardCancel<T>(value: T | symbol, runtime: RuntimeEnv): T {
  if (isCancel(value)) {
    cancel(stylePromptTitle(t("onboard.setupCancelled")) ?? t("onboard.setupCancelled"));
    runtime.exit(0);
  }
  return value as T;
}
...
export function formatControlUiSshHint(params: {
  ...
  return [
    t("helper.noGuiDetected"),
    t("helper.sshTunnel", { port: String(params.port), target: sshTarget }),
    t("helper.thenOpen"),
    localUrl,
    authedUrl,
    `${t("common.docs")}:`,
...
export async function ensureWorkspaceAndSessions(
  ...
  runtime.log(t("workspace.ok", { path: shortenHomePath(ws.dir) }));
  const sessionsDir = resolveSessionTranscriptsDirForAgent(options?.agentId);
  await fs.mkdir(sessionsDir, { recursive: true });
  runtime.log(t("workspace.sessionsOk", { path: shortenHomePath(sessionsDir) }));
}
...
export async function moveToTrash(pathname: string, runtime: RuntimeEnv): Promise<void> {
  ...
  try {
    ...
    runtime.log(t("helper.movedToTrash", { path: shortenHomePath(pathname) }));
  } catch {
    runtime.log(t("helper.failedToTrash", { path: shortenHomePath(pathname) }));
  }
}
