...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenClawCNAgentDir: vi.fn().mockReturnValue("/tmp/openclawcn-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openclawcn-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openclawcn-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenClawCNAgentDir: mocks.resolveOpenClawCNAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenClawCNAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.auth.storePath).toBe("/tmp/openclawcn-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/openclawcn-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/openclawcn-agent-custom");
