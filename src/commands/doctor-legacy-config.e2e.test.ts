...
describe("normalizeLegacyConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.OPENCLAWCN_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-oauth-"));
    process.env.OPENCLAWCN_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.OPENCLAWCN_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-wa-auth-"));
    try {
