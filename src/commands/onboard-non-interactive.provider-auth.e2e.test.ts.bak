...
function captureEnv(): EnvSnapshot {
  return {
    ...
    stateDir: process.env.OPENCLAWCN_STATE_DIR,
    configPath: process.env.OPENCLAWCN_CONFIG_PATH,
    skipChannels: process.env.OPENCLAWCN_SKIP_CHANNELS,
    skipGmail: process.env.OPENCLAWCN_SKIP_GMAIL_WATCHER,
    skipCron: process.env.OPENCLAWCN_SKIP_CRON,
    skipCanvas: process.env.OPENCLAWCN_SKIP_CANVAS_HOST,
    token: process.env.OPENCLAWCN_GATEWAY_TOKEN,
    password: process.env.OPENCLAWCN_GATEWAY_PASSWORD,
    customApiKey: process.env.CUSTOM_API_KEY,
    disableConfigCache: process.env.OPENCLAWCN_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("OPENCLAWCN_STATE_DIR", prev.stateDir);
  restoreEnvVar("OPENCLAWCN_CONFIG_PATH", prev.configPath);
  restoreEnvVar("OPENCLAWCN_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("OPENCLAWCN_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("OPENCLAWCN_SKIP_CRON", prev.skipCron);
  restoreEnvVar("OPENCLAWCN_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("OPENCLAWCN_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("OPENCLAWCN_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CUSTOM_API_KEY", prev.customApiKey);
  restoreEnvVar("OPENCLAWCN_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.OPENCLAWCN_SKIP_CHANNELS = "1";
  process.env.OPENCLAWCN_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENCLAWCN_SKIP_CRON = "1";
  process.env.OPENCLAWCN_SKIP_CANVAS_HOST = "1";
  process.env.OPENCLAWCN_DISABLE_CONFIG_CACHE = "1";
  delete process.env.OPENCLAWCN_GATEWAY_TOKEN;
  delete process.env.OPENCLAWCN_GATEWAY_PASSWORD;
  delete process.env.CUSTOM_API_KEY;

  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), prefix));
  const configPath = path.join(tempHome, "openclawcn.json");
  process.env.HOME = tempHome;
  process.env.OPENCLAWCN_STATE_DIR = tempHome;
  process.env.OPENCLAWCN_CONFIG_PATH = configPath;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores MiniMax API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("openclawcn-onboard-minimax-", async ({ configPath, runtime }) => {
  ...
  it("supports MiniMax CN API endpoint auth choice", async () => {
    await withOnboardEnv("openclawcn-onboard-minimax-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("openclawcn-onboard-zai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("openclawcn-onboard-zai-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("openclawcn-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("openclawcn-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("openclawcn-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("openclawcn-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("rejects vLLM auth choice in non-interactive mode", async () => {
    await withOnboardEnv("openclawcn-onboard-vllm-non-interactive-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("stores LiteLLM API key and sets default model", async () => {
    await withOnboardEnv("openclawcn-onboard-litellm-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it.each([
    {
      ...
      prefix: "openclawcn-onboard-cf-gateway-",
      options: {
    ...
    },
    {
      ...
      prefix: "openclawcn-onboard-cf-gateway-infer-",
      options: {},
    },
  ])(
    "$name",
  ...
  it("infers Together auth choice from --together-api-key and sets default model", async () => {
    await withOnboardEnv("openclawcn-onboard-together-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers QIANFAN auth choice from --qianfan-api-key and sets default model", async () => {
    await withOnboardEnv("openclawcn-onboard-qianfan-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("openclawcn-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "openclawcn-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "openclawcn-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "openclawcn-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "openclawcn-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("openclawcn-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "openclawcn-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
