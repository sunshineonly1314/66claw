...
beforeEach(() => {
  originalAgentDir = process.env.OPENCLAWCN_AGENT_DIR;
  originalPiAgentDir = process.env.PI_CODING_AGENT_DIR;
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-auth-"));
  process.env.OPENCLAWCN_AGENT_DIR = tempAgentDir;
...
afterEach(() => {
  if (originalAgentDir === undefined) {
    ...
    process.env.OPENCLAWCN_AGENT_DIR = originalAgentDir;
  }
