...
async function maybeRepairTelegramAllowFromUsernames(cfg: OpenClawCNConfig): Promise<{
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetPath = path.join(targetDir, "openclawcn.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: OpenClawCNConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("openclawcn doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("openclawcn doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("openclawcn doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
