...
export const resolveOpenClawCNPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenClawCNPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/openclawcn.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveOpenClawCNPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENCLAWCN_STATE_DIR;
  originalUpdateInProgress = process.env.OPENCLAWCN_UPDATE_IN_PROGRESS;
  process.env.OPENCLAWCN_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENCLAWCN_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENCLAWCN_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENCLAWCN_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
