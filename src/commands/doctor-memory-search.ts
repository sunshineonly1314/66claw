 ...
 * Runs as part of `openclawcn doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(cfg: OpenClawCNConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("openclawcn config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("openclawcn memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`openclawcn auth add --provider ${resolved.provider}`)}`,
        `- To disable: ${formatCliCommand("openclawcn config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("openclawcn memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("openclawcn auth add --provider openai")}`,
