...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.OPENCLAWCN_SKIP_BROWSER_CONTROL_SERVER,
  ...
  beforeAll(async () => {
    ...
    process.env.OPENCLAWCN_SKIP_BROWSER_CONTROL_SERVER = "1";
    delete process.env.OPENCLAWCN_GATEWAY_TOKEN;
    delete process.env.OPENCLAWCN_GATEWAY_PASSWORD;

    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OPENCLAWCN_STATE_DIR = prev.stateDir;
    process.env.OPENCLAWCN_CONFIG_PATH = prev.configPath;
    process.env.OPENCLAWCN_SKIP_CHANNELS = prev.skipChannels;
    process.env.OPENCLAWCN_SKIP_GMAIL_WATCHER = prev.skipGmail;
    process.env.OPENCLAWCN_SKIP_CRON = prev.skipCron;
    process.env.OPENCLAWCN_SKIP_CANVAS_HOST = prev.skipCanvas;
    process.env.OPENCLAWCN_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.OPENCLAWCN_GATEWAY_TOKEN = prev.token;
    process.env.OPENCLAWCN_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "openclawcn");
