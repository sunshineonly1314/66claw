...
describe("resolveStorePath", () => {
  ...
  it("uses OPENCLAWCN_HOME for tilde expansion", () => {
    vi.stubEnv("OPENCLAWCN_HOME", "/srv/openclawcn-home");
    vi.stubEnv("HOME", "/home/other");

    const resolved = resolveStorePath("~/.openclawcn/agents/{agentId}/sessions/sessions.json", {
      agentId: "research",
    });

    expect(resolved).toBe(
      path.resolve("/srv/openclawcn-home/.openclawcn/agents/research/sessions/sessions.json"),
    );
  });
});
...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/openclawcn/agents/main/sessions";
  ...
  it("accepts absolute sessionFile paths that resolve within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/openclawcn/agents/main/sessions/abc-123.jsonl" },
      { sessionsDir },
    );
  ...
  it("accepts absolute sessionFile with topic suffix within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/openclawcn/agents/main/sessions/abc-123-topic-42.jsonl" },
      { sessionsDir },
    );
  ...
  it("rejects absolute sessionFile paths outside known agent sessions dirs", () => {
    ...
    expect(() =>
      resolveSessionFilePath(
        "sess-1",
        { sessionFile: "/tmp/openclawcn/agents/work/not-sessions/abc-123.jsonl" },
        { sessionsDir },
      ),
    ).toThrow(/within sessions directory/);
  });
