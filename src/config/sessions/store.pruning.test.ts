...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-pruning-suite-"));
});
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.OPENCLAWCN_SESSION_CACHE_TTL_MS;
    process.env.OPENCLAWCN_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.OPENCLAWCN_SESSION_CACHE_TTL_MS;
    } else {
      process.env.OPENCLAWCN_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
