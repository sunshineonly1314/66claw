...
import type { OpenClawCNConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
function coerceConfig(value: unknown): OpenClawCNConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenClawCNConfig;
}
...
function stampConfigVersion(cfg: OpenClawCNConfig): OpenClawCNConfig {
...
function warnIfConfigFromFuture(cfg: OpenClawCNConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenClawCNVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenClawCN (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenClawCNConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFileImpl(cfg: OpenClawCNConfig, options: ConfigWriteOptions = {}) {
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENCLAWCN_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const auditRecordBase = {
      ...
      watchSession:
        ...
        deps.env.OPENCLAWCN_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENCLAWCN_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENCLAWCN_WATCH_COMMAND === "string" &&
        deps.env.OPENCLAWCN_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENCLAWCN_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENCLAWCN_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENCLAWCN_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENCLAWCN_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OpenClawCNConfig {
