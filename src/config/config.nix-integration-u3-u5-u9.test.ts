...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when OPENCLAWCN_NIX_MODE is not set", async () => {
      await withEnvOverride({ OPENCLAWCN_NIX_MODE: undefined }, async () => {
        vi.resetModules();
    ...
    it("isNixMode is false when OPENCLAWCN_NIX_MODE is empty", async () => {
      await withEnvOverride({ OPENCLAWCN_NIX_MODE: "" }, async () => {
        vi.resetModules();
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    ...
    it("isNixMode is false when OPENCLAWCN_NIX_MODE is not '1'", async () => {
      await withEnvOverride({ OPENCLAWCN_NIX_MODE: "true" }, async () => {
        vi.resetModules();
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    ...
    it("isNixMode is true when OPENCLAWCN_NIX_MODE=1", async () => {
      await withEnvOverride({ OPENCLAWCN_NIX_MODE: "1" }, async () => {
        vi.resetModules();
        const { isNixMode } = await import("./config.js");
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.openclawcn (or legacy state dir) when env not set", async () => {
      await withEnvOverride({ OPENCLAWCN_STATE_DIR: undefined }, async () => {
        vi.resetModules();
        const { STATE_DIR } = await import("./config.js");
        // Should resolve to .openclawcn or a legacy state dir (.clawdbot, etc.)
        expect(STATE_DIR).toMatch(/\.(openclawcn|clawdbot|moldbot|moltbot)$/);
      });
    });

    it("STATE_DIR respects OPENCLAWCN_STATE_DIR override", async () => {
      await withEnvOverride({ OPENCLAWCN_STATE_DIR: "/custom/state/dir" }, async () => {
        vi.resetModules();
        const { STATE_DIR } = await import("./config.js");
    ...
    it("CONFIG_PATH defaults to ~/.openclawcn/openclawcn.json (or legacy state dir) when env not set", async () => {
      await withEnvOverride(
        { OPENCLAWCN_CONFIG_PATH: undefined, OPENCLAWCN_STATE_DIR: undefined },
        async () => {
          vi.resetModules();
    ...
    it("CONFIG_PATH expands ~ in OPENCLAWCN_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        await withEnvOverride({ OPENCLAWCN_CONFIG_PATH: "~/.openclawcn/custom.json" }, async () => {
          vi.resetModules();
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(home, ".openclawcn", "custom.json"));
        });
      });
    });
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", async () => {
      await withEnvOverride({ OPENCLAWCN_GATEWAY_PORT: undefined }, async () => {
        vi.resetModules();
    ...
    it("prefers OPENCLAWCN_GATEWAY_PORT over config", async () => {
      await withEnvOverride({ OPENCLAWCN_GATEWAY_PORT: "19001" }, async () => {
        vi.resetModules();
    ...
    it("falls back to config when env is invalid", async () => {
      await withEnvOverride({ OPENCLAWCN_GATEWAY_PORT: "nope" }, async () => {
        vi.resetModules();
        const { resolveGatewayPort } = await import("./config.js");
