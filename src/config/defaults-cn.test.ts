...
function setCnRegion(value: boolean) {
  savedRegion = process.env.CLAWDBOT_REGION;
  process.env.CLAWDBOT_REGION = value ? "cn" : "global";
}
...
function restoreRegion() {
  if (savedRegion === undefined) {
    ...
    process.env.CLAWDBOT_REGION = savedRegion;
  }
}
...
describe("applyCnDefaults — 不覆盖已有值", () => {
  ...
  it("不覆盖 tools.exec.security = 'deny'", () => {
    setCnRegion(true);
    const cfg: ClawdbotConfig = { tools: { exec: { security: "deny" } } };
    const result = applyCnDefaults(cfg);
  ...
  it("不覆盖 tools.exec.ask = 'always'", () => {
    setCnRegion(true);
    const cfg: ClawdbotConfig = { tools: { exec: { ask: "always" } } };
    const result = applyCnDefaults(cfg);
  ...
  it("不覆盖 tools.exec.safeBins", () => {
    ...
    const cfg: ClawdbotConfig = { tools: { exec: { safeBins: customBins } } };
    const result = applyCnDefaults(cfg);
