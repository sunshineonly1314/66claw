...
export type ConfigSchema = ReturnType<typeof OpenClawCNSchema.toJSONSchema>;
...
function stripChannelSchema(schema: ConfigSchema): ConfigSchema {
  ...
  const channelsNode = asSchemaObject(root.properties.channels);
  if (channelsNode) {
    // Preserve built-in channel schemas (e.g., feishu, dingtalk, wecom)
    // These are defined in ChannelsSchema and don't come from plugins
    const builtinProps: Record<string, JsonSchemaObject> = {};
    if (channelsNode.properties) {
      for (const [key, value] of Object.entries(channelsNode.properties)) {
        if (BUILTIN_CHANNEL_IDS.has(key)) {
          const schemaObj = asSchemaObject(value);
          if (schemaObj) builtinProps[key] = schemaObj;
        }
      }
    }
    channelsNode.properties = builtinProps;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = OpenClawCNSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "OpenClawCNConfig";
  const hints = mapSensitivePaths(OpenClawCNSchema, "", buildBaseHints());
  const next = {
