...
export type AgentDefaultsConfig = {
  ...
  subagents?: {
    /** Default allow-list for sub-agent spawning. Use ["*"] to allow any agent. */
    allowAgents?: string[];
    /** Max concurrent sub-agent runs (global lane: "subagent"). Default: 1. */
    maxConcurrent?: number;
    /**
     * Max nesting depth for sub-agent spawning.
     * - 1: Only top-level agents can spawn sub-agents (default, backward-compatible)
     * - 2+: Sub-agents can spawn further sub-agents up to this depth
     * - 0: Disable sub-agent spawning entirely
     */
    maxDepth?: number;
...
export type AgentCompactionConfig = {
  ...
  /** Proactive compaction (runs before agent turn when context is near capacity). */
  proactiveCompaction?: {
    /** Enable proactive compaction (default: true). */
    enabled?: boolean;
    /** Context usage ratio threshold (0-1) to trigger proactive compaction. */
