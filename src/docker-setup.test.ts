...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "openclawcn-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENCLAWCN_GATEWAY_TOKEN: "test-token",
    OPENCLAWCN_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENCLAWCN_WORKSPACE_DIR: join(sandbox.rootDir, "openclawcn"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        OPENCLAWCN_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        OPENCLAWCN_EXTRA_MOUNTS: undefined,
        OPENCLAWCN_HOME_VOLUME: "openclawcn-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENCLAWCN_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OPENCLAWCN_EXTRA_MOUNTS=");
    expect(envFile).toContain("OPENCLAWCN_HOME_VOLUME=openclawcn-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("openclawcn-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("openclawcn-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENCLAWCN_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
