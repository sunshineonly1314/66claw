...
describe("【Tester A】正向测试 - 功能正确性验证", () => {
  ...
  describe("A4: 完整性哈希正向测试", () => {
    it("A4.1: 完整性哈希文件应该存在", () => {
      if (!isProductionBuild()) return; // skip in dev build - integrity hashes only generated during production build
  ...
  describe("A5: 构建流程正向测试", () => {
    ...
    it("A5.2: license 模块文件应该完整", () => {
      if (!isProductionBuild()) return; // skip in dev build - license files only present in production build
      const requiredFiles = [
        "device-id.js",
        "heartbeat.js",
        "index.js",
    ...
    it("A5.3: security 模块文件应该完整", () => {
      if (!isProductionBuild()) return; // skip in dev build - security files only present in production build
      ...
      if (fs.existsSync(rsaVerifyPath)) {
        ...
        if (isBytecodeLoader) {
          ...
          const hasKeyFragment =
            ...
            content.includes("uB00UMEJdP/XxmCJ");
