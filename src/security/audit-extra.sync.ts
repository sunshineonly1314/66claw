...
function summarizeGroupPolicy(cfg: OpenClawCNConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: OpenClawCNConfig): boolean {
...
function collectModels(cfg: OpenClawCNConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: OpenClawCNConfig): Set<string> {
  const baseCfg: OpenClawCNConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: OpenClawCNConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OpenClawCNConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OpenClawCNConfig): boolean {
...
function isBrowserEnabled(cfg: OpenClawCNConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OpenClawCNConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: OpenClawCNConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPENCLAWCN_STATE_DIR on a local-only volume and re-run "${formatCliCommand("openclawcn security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: OpenClawCNConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer OPENCLAWCN_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const openclawcnGatewayToken =
    typeof env.OPENCLAWCN_GATEWAY_TOKEN === "string" && env.OPENCLAWCN_GATEWAY_TOKEN.trim()
      ? env.OPENCLAWCN_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-openclawcn-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: OpenClawCNConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectNodeDenyCommandPatternFindings(cfg: OpenClawCNConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: OpenClawCNConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: OpenClawCNConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: OpenClawCNConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
