...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openclawcn-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "openclawcn.plugin.json"), JSON.stringify(manifest), "utf-8");
}
