...
function snapshotStateEnv(): StateEnvSnapshot {
  return { OPENCLAWCN_STATE_DIR: process.env.OPENCLAWCN_STATE_DIR };
}

function restoreStateEnv(snapshot: StateEnvSnapshot) {
  if (snapshot.OPENCLAWCN_STATE_DIR === undefined) {
    delete process.env.OPENCLAWCN_STATE_DIR;
  } else {
    process.env.OPENCLAWCN_STATE_DIR = snapshot.OPENCLAWCN_STATE_DIR;
  }
}
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "openclawcn-typing-heartbeat-"));
  });
