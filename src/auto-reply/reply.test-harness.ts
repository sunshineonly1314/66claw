...
type HomeEnvSnapshot = {
  ...
  OPENCLAWCN_STATE_DIR: string | undefined;
  OPENCLAWCN_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    OPENCLAWCN_AGENT_DIR: process.env.OPENCLAWCN_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.OPENCLAWCN_STATE_DIR = path.join(home, ".openclawcn");
    process.env.OPENCLAWCN_AGENT_DIR = path.join(home, ".openclawcn", "agent");
  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "openclawcn"),
      },
    },
    channels: {
      whatsapp: {
