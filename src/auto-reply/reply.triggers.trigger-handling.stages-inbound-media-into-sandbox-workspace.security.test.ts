...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(async (home) => await fn(home), { prefix: "openclawcn-triggers-bypass-" });
}
...
describe("stageSandboxMedia security", () => {
  it("rejects staging host files from outside the media directory", async () => {
    await withTempHome(async (home) => {
      // Sensitive host file outside .openclawcn
      ...
      await stageSandboxMedia({
        ...
        cfg: {
          agents: {
            defaults: {
              ...
              workspace: join(home, "openclawcn"),
              sandbox: {
                mode: "non-main",
        ...
        workspaceDir: join(home, "openclawcn"),
      });
