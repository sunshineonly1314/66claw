...
type AgentConfig = Partial<NonNullable<NonNullable<OpenClawCNConfig["agents"]>["defaults"]>>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 OpenClawCN ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: OpenClawCNConfig): string {
  const lines = ["ℹ️ Help", ""];
