...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-canvas-fixtures-"));
  });
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("openclawcnCanvasA2UIAction");
    expect(out).toContain("openclawcnSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("openclawcnSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__openclawcn__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("openclawcn-a2ui-host");
      expect(html).toContain("openclawcnCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__openclawcn__/a2ui/a2ui.bundle.js`,
      );
