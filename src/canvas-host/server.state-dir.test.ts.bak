...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPENCLAWCN_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("OpenClawCN Canvas");
    } finally {
      await handler.close();
