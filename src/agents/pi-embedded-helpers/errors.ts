...
// [CN-MERGE:3d4ef56044] Include provider and model name in billing error message
...
// [CN-MERGE] Include provider and model name in auth error message
export function formatAuthErrorMessage(provider?: string, model?: string): string {
  ...
  const providerLabel =
    providerName && modelName ? `${providerName} (${modelName})` : providerName || undefined;
  if (providerLabel) {
    return `⚠️ ${providerLabel}: API Key 无效或已过期，请检查模型配置`;
  }
...
export function formatAssistantErrorText(
  ...
  opts?: { cfg?: OpenClawCNConfig; sessionKey?: string; provider?: string; model?: string },
): string | undefined {
...
const ERROR_PATTERNS = {
  ...
  billing: [
    ...
    // Chinese billing error patterns (中文计费错误)
    "额度不足",
    "额度已用",
    "余额不足",
    "账户欠费",
    ...
    "授权限额已满",
    "请求次数超限",
    // Additional English billing patterns
    /daily[_ ]limit/,
    "billing",
    "quota exceeded",
