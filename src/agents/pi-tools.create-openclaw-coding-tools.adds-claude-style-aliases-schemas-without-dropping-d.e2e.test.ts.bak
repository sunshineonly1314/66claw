...
describe("createOpenClawCNCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from openclawcn read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createOpenClawCNCodingTools({ sandbox });
