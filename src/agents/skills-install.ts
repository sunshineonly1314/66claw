...
/**
 * OpenClawCN: Common Go installation paths on Windows
 */
...
/**
 * OpenClawCN: Find Go executable, checking common Windows install paths
 */
...
/**
 * OpenClawCN: Sanitize command output (Windows encoding issues)
 */
...
/**
 * OpenClawCN: Friendly Chinese error messages for common failures
 */
...
async function collectSkillInstallScanWarnings(entry: SkillEntry): Promise<string[]> {
  ...
  try {
    ...
    if (summary.critical > 0) {
      ...
      warnings.push(
        `Skill "${skillName}" has ${summary.warn} suspicious code pattern(s). Run "openclawcn security audit --deep" for details.`,
      );
    }
  } catch (err) {
    warnings.push(
      `Skill "${skillName}" code safety scan failed (${String(err)}). Installation continues; run "openclawcn security audit --deep" after install.`,
    );
  }
...
/**
 * OpenClawCN: CN mirror configuration (backward compatible)
 */
const CN_MIRRORS = {
...
/**
 * OpenClawCN: Run command with mirror fallback
 */
...
/**
 * Build Node.js package install command.
 * Upstream security: --ignore-scripts prevents post-install script attacks.
 * OpenClawCN: Support CN mirror registries and Windows path resolution.
 */
function buildNodeInstallCommand(
  ...
  switch (prefs.nodeManager) {
    case "pnpm": {
      ...
      return {
        argv: registry
          ? [pnpmPath, "add", "-g", "--ignore-scripts", packageName, "--registry", registry]
          : [pnpmPath, "add", "-g", "--ignore-scripts", packageName],
      };
    }
    case "yarn": {
      ...
      return {
        argv: registry
          ? [yarnPath, "global", "add", "--ignore-scripts", packageName, "--registry", registry]
          : [yarnPath, "global", "add", "--ignore-scripts", packageName],
      };
    }
    case "bun": {
      const bunPath = getManagerPath("bun");
      return {
        argv: [bunPath, "add", "-g", "--ignore-scripts", packageName],
    ...
    default: {
      ...
      return {
        argv: registry
          ? [npmPath, "install", "-g", "--ignore-scripts", packageName, "--registry", registry]
          : [npmPath, "install", "-g", "--ignore-scripts", packageName],
      };
    }
  }
}
...
/**
 * OpenClawCN: Install Node.js package with mirror fallback
 */
async function installNodePackageWithFallback(params: {
  ...
  return runCommandWithMirrorFallback({
    ...
    onMirrorSwitch: (from, to, error) => {
      onProgress?.({
        stage: "installing",
        message: `镜像 ${new URL(from).hostname} 不可用，切换到 ${new URL(to).hostname}...`,
        usingCNMirror: true,
        percent: 55,
      });
    },
...
/**
 * OpenClawCN: Get Go proxy environment for CN acceleration
 */
...
// --------------------------------------------------------------------------
// Upstream security: Archive entry path validation (zip-slip prevention)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// OpenClawCN: Download progress callback
// --------------------------------------------------------------------------
...
/**
 * Download a file with SSRF protection.
 * Uses fetchWithSsrFGuard for upstream SSRF safety when skipSsrfCheck is false.
 * OpenClawCN: Supports progress callbacks and CN proxy headers.
 */
async function downloadFile(
  ...
  // For ClawdSkillsProxy URLs or when explicitly skipping, use direct fetch
  const isClawdProxy = url.startsWith(CLAWDSKILLSPROXY_CONFIG.baseUrl);

  if (!skipSsrfCheck && !isClawdProxy) {
    // Use upstream's SSRF-guarded fetch for non-proxy URLs
    ...
    try {
      ...
      if (onProgress && contentLength > 0) {
        onProgress({ downloaded, total: contentLength, percent: 100, speed: "0 B/s", eta: "" });
      }
...
/**
 * Extract archive with upstream security improvements:
 * - Uses extractArchiveSafe for zip/tar.gz (safe extraction with path validation)
 * - For tar.bz2: preflight listing + symlink detection + path traversal validation
 */
async function extractArchive(params: {
  ...
  try {
    ...
    if (archiveType === "tar.bz2") {
      ...
      // Check for symlinks which could be exploited for path traversal
...
async function installGoDependency(
  ...
  if (platform === "linux") {
    ...
    const goRoot = path.join(process.env.HOME || "", ".openclawcn", "tools", "go");
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  // Upstream security: scan skill directory for dangerous patterns
  ...
  if (command.error) {
    return withWarnings(
      { ok: false, message: command.error, stdout: "", stderr: "", code: null },
      warnings,
    );
  }
  ...
  // OpenClawCN: Check Go availability including common Windows install paths
  ...
  const result = await (async () => {
    ...
    // OpenClawCN: Use multi-mirror fallback for Node.js packages
