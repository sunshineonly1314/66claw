...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENCLAWCN_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENCLAWCN_HOME: "/srv/openclawcn-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/openclawcn-home"), ".openclawcn", "workspace"));
  });
});
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("openclawcn-workspace-");
