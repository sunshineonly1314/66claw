...
metadata: {"openclawcn":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
describe("installSkill code safety scanning", () => {
  ...
  it("adds detailed warnings for critical findings and continues install", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-skills-install-"));
    try {
