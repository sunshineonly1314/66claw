...
function buildSkillStatus(
  ...
  const bundled =
    bundledNames && bundledNames.size > 0
      ...
      : entry.skill.source === "openclawcn-bundled";
...
/**
 * Lightweight batch check: returns only skills that have install instructions.
 * Used by the batch install RPC endpoint to avoid scanning all skills.
 */
export function buildBatchCheckStatus(
  ...
  const skillEntries =
    opts?.entries ??
    loadWorkspaceSkillEntries(managedSkillsDir, {
      config: opts?.config,
      managedSkillsDir,
      bundledSkillsDir: bundledContext.dir,
    });
  const prefs = resolveSkillsInstallPreferences(opts?.config);
  const allSkills = skillEntries.map((entry) =>
    buildSkillStatus(entry, opts?.config, prefs, opts?.eligibility, bundledContext.names),
  );
  const skillsWithInstall = allSkills.filter((s) => s.install.length > 0);
