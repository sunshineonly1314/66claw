...
function resolveExecConfig(params: { cfg?: OpenClawCNConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: OpenClawCNConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createOpenClawCNCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOpenClawCNReadTool(freshReadTool);
