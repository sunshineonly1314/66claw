...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-oauth-"));

    try {
      process.env.OPENCLAWCN_STATE_DIR = tempDir;
      process.env.OPENCLAWCN_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.OPENCLAWCN_AGENT_DIR, {
      ...
      const apiKey = await getApiKeyForModel({
        ...
        store,
        agentDir: process.env.OPENCLAWCN_AGENT_DIR,
      });
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-auth-"));
