...
import { createOpenClawCNCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openclawcn-ws-", async (workspaceDir) => {
      await withTempDir("openclawcn-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenClawCNCodingTools({ workspaceDir });
    ...
    await withTempDir("openclawcn-ws-", async (workspaceDir) => {
      await withTempDir("openclawcn-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello openclawcn");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openclawcn-ws-", async (workspaceDir) => {
      await withTempDir("openclawcn-override-", async (overrideDir) => {
        const tools = createOpenClawCNCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openclawcn-sandbox-", async (sandboxDir) => {
      await withTempDir("openclawcn-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "openclawcn-sbx-test",
        ...
        const tools = createOpenClawCNCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
