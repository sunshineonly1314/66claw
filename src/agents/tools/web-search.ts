...
type WebSearchConfig = NonNullable<OpenClawCNConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenClawCNConfig): WebSearchConfig {
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${params.apiKey}`,
      "HTTP-Referer": "https://openclawcn.ai",
      "X-Title": "OpenClawCN Web Search",
    },
    body: JSON.stringify(body),
    signal: withTimeout(undefined, params.timeoutSeconds * 1000),
  });
