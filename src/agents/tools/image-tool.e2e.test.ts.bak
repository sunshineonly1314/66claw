...
describe("image tool implicit imageModel config", () => {
  ...
  it("stays disabled without auth when no pairing is possible", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-image-"));
  ...
  it("allows workspace images outside default local media roots", async () => {
    const workspaceParent = await fs.mkdtemp(
      path.join(process.cwd(), ".openclawcn-workspace-image-"),
    );
    try {
  ...
  it("allows workspace images via createOpenClawCNCodingTools default workspace root", async () => {
    const workspaceParent = await fs.mkdtemp(
      path.join(process.cwd(), ".openclawcn-workspace-image-"),
    );
    try {
      ...
      const tools = createOpenClawCNCodingTools({ config: cfg, agentDir });
  ...
  it("sandboxes image paths like the read tool", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-image-sandbox-"));
...
describe("image tool MiniMax VLM routing", () => {
  ...
  it("calls /v1/coding_plan/vlm for minimax image models", async () => {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg: OpenClawCNConfig = {
      agents: { defaults: { model: { primary: "minimax/MiniMax-M2.1" } } },
    };
