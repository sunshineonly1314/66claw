...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.OPENCLAWCN_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: openclawcn --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/openclawcn" } }\n}\n';
