...
describe("resolveBundledSkillsDir", () => {
  const originalOverride = process.env.OPENCLAWCN_BUNDLED_SKILLS_DIR;

  afterEach(() => {
    if (originalOverride === undefined) {
      delete process.env.OPENCLAWCN_BUNDLED_SKILLS_DIR;
    } else {
      process.env.OPENCLAWCN_BUNDLED_SKILLS_DIR = originalOverride;
    }
  });
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "openclawcn" }));
