...
interface SkillFileRecord {
  ...
  metadata: OpenClawCNSkillMetadata | undefined;
...
export function loadSkillEntriesFromFileIndex(
  ...
  for (const dir of mergedExtraDirs) {
    dirSources.push({ dir: resolveUserPath(dir), source: "openclawcn-extra" });
  }
  if (bundledSkillsDir) {
    dirSources.push({ dir: bundledSkillsDir, source: "openclawcn-bundled" });
  }
  dirSources.push({ dir: managedSkillsDir, source: "openclawcn-managed" });
  dirSources.push({ dir: workspaceSkillsDir, source: "openclawcn-workspace" });
