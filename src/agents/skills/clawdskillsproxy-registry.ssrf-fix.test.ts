...
describe("ClawdSkillsProxy Registry - SSRF prevention (Phase 2 #7)", () => {
  describe("fetchProxySkillsIndex SSRF protection", () => {
    ...
    it("blocks 127.0.0.1 loopback", async () => {
      ...
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error).toMatch(/SSRF.*blocked|127\.0\.0\.1|loopback|private.*internal.*IP/i);
      }
    });
    ...
    it("blocks link-local addresses", async () => {
      ...
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error).toMatch(/SSRF.*blocked|link-local|169\.254|private.*internal.*IP/i);
      }
    });
