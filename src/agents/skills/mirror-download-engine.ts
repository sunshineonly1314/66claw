 ...
 * ClawdbotCN: mirror-based batch skill installation
 */
...
async function installSingleSkill(
  ...
  const bundledToolsDir = process.env.CLAWDBOT_BUNDLED_TOOLS_DIR;
  if (bundledToolsDir) {
    const requiredBins = targetEntry?.clawdbot?.requires?.bins ?? [];
    ...
    if (requiredBins.length > 0) {
      ...
      if (allBundled) {
        // 将预置二进制复制到 ~/.clawdbot/tools/{toolName}/
        let copyOk = true;
