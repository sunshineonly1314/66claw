  ...
  const lines = [
    ...
    hasGateway && !isMinimal
      ? [
          ...
          "## Configuration Management Rules（配置管理规则，必须遵守）",
          "CRITICAL / 重要：OpenClawCN 配置包含大量 CN 专属字段，编辑错误会导致 gateway 崩溃无法启动。",
          "修改配置的唯一正确方式：",
          "  1. config.get — 读取当前配置，记录 baseHash",
          "  2. config.schema — 不确定字段名或类型时先查 schema",
          "  3. config.patch — 只发送要改的字段（必须带 baseHash），其余字段自动保留",
          "  4. 每次必须设置 note 参数，用一句话说明改了什么",
          "ALWAYS use config.patch for partial/targeted changes — it merges with existing config, preserving all other fields.",
