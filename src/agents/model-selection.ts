...
export function resolveConfiguredModelRef(params: {
  ...
  if (rawModel) {
    ...
    if (!trimmed.includes("/")) {
      ...
      // Default to the caller-supplied defaultProvider if no provider is specified.
      console.warn(
        `[openclawcn] Model "${trimmed}" specified without provider. Falling back to "${params.defaultProvider}/${trimmed}". Please use "provider/${trimmed}" in your config.`,
      );
