...
describe("Agent-specific tool filtering", () => {
  ...
  it("defaults apply_patch to workspace-only (blocks traversal)", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-pi-tools-"));
...
*** End Patch`;
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createOpenClawCNCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createOpenClawCNCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createOpenClawCNCodingTools({
      config: cfg,
    ...
    const defaultTools = createOpenClawCNCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createOpenClawCNCodingTools({
      config: cfg,
    ...
    const bobTools = createOpenClawCNCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createOpenClawCNCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createOpenClawCNCodingTools({
      config: cfg,
