...
describe("tools.fs.workspaceOnly", () => {
  it("defaults to allowing sandbox mounts outside the workspace root", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-sbx-mounts-"));
    ...
    try {
      ...
      const tools = createOpenClawCNCodingTools({ sandbox, workspaceDir: sandboxRoot });
  ...
  it("rejects sandbox mounts outside the workspace root when enabled", async () => {
    ...
    try {
      ...
      const cfg = { tools: { fs: { workspaceOnly: true } } } as unknown as OpenClawCNConfig;
      const tools = createOpenClawCNCodingTools({ sandbox, workspaceDir: sandboxRoot, config: cfg });
      const readTool = tools.find((tool) => tool.name === "read");
      const writeTool = tools.find((tool) => tool.name === "write");
