...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-toolpersist-"));
    process.env.OPENCLAWCN_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  api.on("tool_result_persist", (event) => {
    ...
    const registry = loadOpenClawCNPlugins({
      cache: false,
      workspaceDir: tmp,
      config: {
        plugins: {
