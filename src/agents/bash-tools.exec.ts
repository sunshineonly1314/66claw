...
const DEFAULT_APPROVAL_REQUEST_TIMEOUT_MS = 310_000; // 比审批超时多 10 秒
 ...
 * 从配置读取审批超时时间
 * @param value 配置值（毫秒）
 * @returns 超时时间（毫秒），最小 60 秒，最大 10 分钟
 */
function resolveApprovalTimeoutMs(value?: number): number {
  if (typeof value === "number" && Number.isFinite(value) && value >= 60_000) {
    return Math.min(Math.floor(value), 600_000); // 最大 10 分钟
  }
...
export function createExecTool(
  ...
  const approvalRequestTimeoutMs = approvalTimeoutMs + 10_000; // 比审批超时多 10 秒
