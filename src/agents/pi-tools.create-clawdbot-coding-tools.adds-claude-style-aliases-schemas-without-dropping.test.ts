...
import { __testing, createOpenClawCNCodingTools } from "./pi-tools.js";
...
describe("createOpenClawCNCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenClawCNCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openclawcn-outside.txt");
    await fs.writeFile(outsidePath, "outside", "utf8");
    try {
      const readTool = createSandboxedReadTool({ root: tmpDir, bridge: createHostSandboxFsBridge(tmpDir) });
