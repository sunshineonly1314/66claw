...
export function resolveConfigDir(
  ...
  const override = env.OPENCLAWCN_STATE_DIR?.trim();
  if (override) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENCLAWCN_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENCLAWCN_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENCLAWCN_STATE_DIR.
