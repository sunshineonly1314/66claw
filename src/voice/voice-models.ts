 ...
 *
 * NOT used: gh-proxy.com returns 403 for files >100 MB.
 * NOT used: huggingface.co is blocked in mainland China.
 */
...
/**
 * Qwen3-ASR-0.6B — SOTA Chinese ASR, end-to-end.
 * ~3 GB effective VRAM. Model weights: 1.88 GB.
 */
...
/**
 * Qwen3-TTS-0.6B — high-quality Chinese TTS with dialect support.
 * ~2.8 GB VRAM (bf16). Model weights: ~1.83 GB + ~682 MB speech tokenizer.
 */
...
/**
 * SenseVoice Small INT8 — good Chinese ASR via sherpa-onnx.
 * ~450 MB runtime memory. Model files: ~229 MB.
 */
...
/**
 * Kokoro 82M — lightweight offline TTS for CPU.
 * ~1.1-1.4 GB runtime memory. ONNX model: ~86 MB.
 */
...
// ---------------------------------------------------------------------------
// KWS Model (sherpa-onnx keyword spotting, CPU)
// ---------------------------------------------------------------------------

const KWS_MS = "csukuangfj/sherpa-onnx-kws-zipformer-zh-en-3M-2025-12-20";
const KWS_HF = "k2-fsa/sherpa-onnx-kws-zipformer-zh-en-3M-2025-12-20";

/**
 * KWS Zipformer 3M — wake word detection, Chinese + English.
 * Tiny model: ~5.3 MB int8. Runs on CPU with 1 thread.
 */
const KWS_ZIPFORMER_3M: VoiceModelSpec = {
  id: "kws-zipformer-zh-en-3m",
  displayName: "KWS Zipformer 3M (CPU)",
  backend: "sherpa-onnx",
  estimatedMemoryMB: 30,
  downloadSizeMB: 6,
  modelDirName: "sherpa-onnx-kws-zipformer-zh-en-3M-2025-12-20",
  files: [
    {
      relativePath: "encoder-epoch-13-avg-2-chunk-16-left-64.int8.onnx",
      sizeBytes: 4_400_000,
      sources: sources(KWS_MS, KWS_HF, "encoder-epoch-13-avg-2-chunk-16-left-64.int8.onnx"),
    },
    {
      relativePath: "decoder-epoch-13-avg-2-chunk-16-left-64.onnx",
      sizeBytes: 743_000,
      sources: sources(KWS_MS, KWS_HF, "decoder-epoch-13-avg-2-chunk-16-left-64.onnx"),
    },
    {
      relativePath: "joiner-epoch-13-avg-2-chunk-16-left-64.int8.onnx",
      sizeBytes: 85_000,
      sources: sources(KWS_MS, KWS_HF, "joiner-epoch-13-avg-2-chunk-16-left-64.int8.onnx"),
    },
    {
      relativePath: "tokens.txt",
      sizeBytes: 10_000,
      sources: sources(KWS_MS, KWS_HF, "tokens.txt"),
    },
  ],
};

/**
 * Edge TTS — online Microsoft neural voices, zero local resources.
 */
...
export const ALL_MODELS: Record<string, VoiceModelSpec> = {
  ...
  [KWS_ZIPFORMER_3M.id]: KWS_ZIPFORMER_3M,
};
