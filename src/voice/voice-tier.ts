 ...
 *
 * Decision rules (user requirements):
...
const GPU_FULL_VRAM_MB = 6300; // ASR (~3GB) + TTS (~2.8GB) + CUDA context (~1GB shared)
const GPU_ASR_ONLY_VRAM_MB = 3500; // ASR (~3GB) + CUDA context (~1GB)

// CPU RAM thresholds (after 70% budget applied to free RAM)
const CPU_FULL_RAM_MB = 1900; // SenseVoice (~450MB) + Kokoro (~1.4GB)
const CPU_ASR_ONLY_RAM_MB = 450; // SenseVoice only
...
export function classifyVoiceTier(hw: HardwareSnapshot): VoiceTierDecision {
  ...
  if (hw.gpu?.vendor === "nvidia") {
    ...
    // NVIDIA GPU exists but VRAM too low — fall through to CPU path
  }
...
/**
 * Whether a tier needs GPU sidecar management.
 */
export function tierRequiresGpuSidecar(tier: VoiceTierLevel): boolean {
  return tier === "gpu-full" || tier === "gpu-asr";
}
