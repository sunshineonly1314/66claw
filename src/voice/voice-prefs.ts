 ...
 *
 * Pattern follows src/infra/voicewake.ts.
 */
...
function resolvePath(baseDir?: string): string {
  ...
  return path.join(root, "settings", "voice-prefs.json");
}

// ---------------------------------------------------------------------------
// Read / Write with lock (same pattern as voicewake.ts)
// ---------------------------------------------------------------------------
 ...
 */
export async function loadVoicePrefs(baseDir?: string): Promise<VoicePrefs> {
...
/**
 * Check if a given ASR/TTS provider string is a valid API provider (not "auto").
 */
