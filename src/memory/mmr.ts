/**
 * [CN-MERGE:fa9420069a] Maximal Marginal Relevance (MMR) re-ranking algorithm.
 *
 ...
 *
 * Enhanced with CJK tokenization support for CN locale.
 *
 ...
 * Supports both ASCII and CJK (Chinese/Japanese/Korean) characters.
 * CJK characters are treated as individual tokens for better granularity.
 */
export function tokenize(text: string): Set<string> {
  // Match ASCII words OR CJK unified ideographs (including Extension A and Compatibility)
  const tokens =
    text.toLowerCase().match(/[a-z0-9_]+|[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]+/g) ?? [];
 ...
 * 3. MMR score = lambda * relevance - (1-lambda) * max_similarity_to_already_selected
 *
