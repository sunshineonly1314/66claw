...
describe("QmdMemoryManager", () => {
  ...
  let stateDir: string;
  let cfg: OpenClawCNConfig;
  const agentId = "main";

  async function createManager(params?: { mode?: "full" | "status"; cfg?: OpenClawCNConfig }) {
  ...
  it("blocks non-markdown or symlink reads for qmd paths", async () => {
    ...
    try {
      ...
      // Windows non-admin cannot create symlinks (EPERM) — skip symlink assertion
