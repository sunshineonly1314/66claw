...
export function triggerOpenClawCNRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENCLAWCN_SYSTEMD_UNIT,
        process.env.OPENCLAWCN_PROFILE,
      );
  ...
  const label =
    process.env.OPENCLAWCN_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENCLAWCN_PROFILE);
...
export function scheduleGatewaySigusr1Restart(opts?: {
  ...
  // Authorize immediately so consumeGatewaySigusr1RestartAuthorization() can detect
  // the pending restart before the signal fires.
