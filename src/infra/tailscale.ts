...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = (process.env.OPENCLAWCN_TEST_TAILSCALE_BINARY)?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for OpenClawCN. You can keep running the web gateway without it: \`${formatCliCommand("openclawcn gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
