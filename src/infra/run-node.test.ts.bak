...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-run-node-"));
  try {
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        const exitCode = await runNodeMain({
          ...
          env: {
            ...process.env,
            OPENCLAWCN_FORCE_BUILD: "1",
            OPENCLAWCN_RUNNER_LOG: "0",
          },
          spawn,
        ...
        expect(nodeCalls).toEqual([[process.execPath, "openclawcn.mjs", "--version"]]);
      });
    },
  );
});
