...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENCLAWCN_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openclawcn-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openclawcn";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openclawcn");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openclawcn-gw");
  ...
  records.push(`_openclawcn-gw._tcp IN PTR ${instanceLabel}._openclawcn-gw._tcp`);
  records.push(`${instanceLabel}._openclawcn-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openclawcn-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openclawcn-content-hash: ${contentHash}\n${contentBody}`;
}
