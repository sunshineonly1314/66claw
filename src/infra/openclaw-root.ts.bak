...
const CORE_PACKAGE_NAMES = new Set(["openclawcn", "openclawcn"]);
...
export async function resolveOpenClawCNPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveOpenClawCNPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
