 ...
 * OpenClawCN: batch install RPC endpoints
 */

import fs from "node:fs";
import { buildBatchCheckStatus, type SkillStatusEntry } from "../../agents/skills-status.js";
...
export const skillsBatchHandlers: GatewayRequestHandlers = {
  ...
  "skills.batch.check": ({ params, respond }) => {
    ...
    const missing = notInstalled.map((s: SkillStatusEntry) => {
    ...
    const notInstalledNames = new Set(notInstalled.map((s: SkillStatusEntry) => s.name));
    const installed = skillsWithInstall
      .filter((s: SkillStatusEntry) => !notInstalledNames.has(s.name))
      .map((s: SkillStatusEntry) => ({
        name: s.name,
        icon: s.emoji ?? "",
        tier: getSkillTier(s.name),
      }));
