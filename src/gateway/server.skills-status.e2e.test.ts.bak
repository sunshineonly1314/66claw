...
describe("gateway skills.status", () => {
  it("does not expose raw config values to operator.read clients", async () => {
    const prevBundledSkillsDir = process.env.OPENCLAWCN_BUNDLED_SKILLS_DIR;
    process.env.OPENCLAWCN_BUNDLED_SKILLS_DIR = path.join(process.cwd(), "skills");
    ...
    try {
      ...
      if (prevBundledSkillsDir === undefined) {
        ...
        process.env.OPENCLAWCN_BUNDLED_SKILLS_DIR = prevBundledSkillsDir;
      }
    }
  });
});
