...
export type GatewayServer = {
  ...
  /**
   * Notify connected clients that the gateway is entering drain mode.
   * Called before the drain phase of a graceful restart so clients can
   * stop sending new requests and prepare for reconnection.
   */
  notifyDraining: (opts: {
    reason: string;
    activeTasks: number;
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.OPENCLAWCN_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.OPENCLAWCN_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "OPENCLAWCN_RAW_STREAM",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("openclawcn doctor")}" to migrate.`,
      );
    }
  ...
  await initStateStore(stateStoreConfig);
  ...
  const {
    ...
    broadcastToConnIds: localBroadcastToConnIds,
    agentRunSeq,
    dedupe,
    chatRunState,
    chatRunBuffers,
  ...
  // Wrap broadcaster with distributed pub/sub bridge for cross-instance delivery
  const {
    broadcast,
    broadcastToConnIds,
    teardown: distributedBroadcastTeardown,
  } = await initDistributedBroadcast({
    localBroadcast,
