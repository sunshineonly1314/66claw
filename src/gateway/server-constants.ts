...
export const getHandshakeTimeoutMs = () => {
  if (process.env.VITEST && process.env.OPENCLAWCN_TEST_HANDSHAKE_TIMEOUT_MS) {
    const parsed = Number(process.env.OPENCLAWCN_TEST_HANDSHAKE_TIMEOUT_MS);
...
export const WS_PING_INTERVAL_MS = 45_000; // WebSocket ping interval (offset from tick to avoid burst)
export const WS_PING_MAX_MISSED = 3; // max missed pongs before disconnect (~135s total timeout)
