...
import { markGatewayReady, resetGatewayReady } from "./server-ready.js";

async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.OPENCLAWCN_CONFIG_PATH;
  const prevDisableCache = process.env.OPENCLAWCN_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "openclawcn-plugin-http-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.OPENCLAWCN_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.OPENCLAWCN_DISABLE_CONFIG_CACHE;
    } else {
      process.env.OPENCLAWCN_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
