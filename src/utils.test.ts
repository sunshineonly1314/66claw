...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openclawcn-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openclawcn when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openclawcn-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENCLAWCN_HOME over HOME", () => {
    vi.stubEnv("OPENCLAWCN_HOME", "/srv/openclawcn-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openclawcn-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENCLAWCN_HOME prefix when OPENCLAWCN_HOME is set", () => {
    vi.stubEnv("OPENCLAWCN_HOME", "/srv/openclawcn-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("shortenHomeInString", () => {
  it("uses $OPENCLAWCN_HOME replacement when OPENCLAWCN_HOME is set", () => {
    vi.stubEnv("OPENCLAWCN_HOME", "/srv/openclawcn-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openclawcn")).toBe(path.resolve(os.homedir(), "openclawcn"));
  });
  ...
  it("prefers OPENCLAWCN_HOME for tilde expansion", () => {
    vi.stubEnv("OPENCLAWCN_HOME", "/srv/openclawcn-home");
    vi.stubEnv("HOME", "/home/other");
