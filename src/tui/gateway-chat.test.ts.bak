...
const originalEnvToken = process.env.OPENCLAWCN_GATEWAY_TOKEN;
const originalEnvPassword = process.env.OPENCLAWCN_GATEWAY_PASSWORD;
...
describe("resolveGatewayConnection", () => {
  ...
  afterEach(() => {
    if (originalEnvToken === undefined) {
      ...
      process.env.OPENCLAWCN_GATEWAY_TOKEN = originalEnvToken;
    }

    if (originalEnvPassword === undefined) {
      delete process.env.OPENCLAWCN_GATEWAY_PASSWORD;
    } else {
      process.env.OPENCLAWCN_GATEWAY_PASSWORD = originalEnvPassword;
    }
  });
