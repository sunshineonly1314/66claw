  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          openclawcn: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    process.env.OPENCLAWCN_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OPENCLAWCN_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OPENCLAWCN_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPassword === undefined) {
      ...
      process.env.OPENCLAWCN_GATEWAY_PASSWORD = prevGatewayPassword;
    }
