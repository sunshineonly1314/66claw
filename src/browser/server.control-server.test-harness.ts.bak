...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/openclawcn" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          openclawcn: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenClawCNChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveOpenClawCNUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopOpenClawCNChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.OPENCLAWCN_GATEWAY_PORT;
    process.env.OPENCLAWCN_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.OPENCLAWCN_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.OPENCLAWCN_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.OPENCLAWCN_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.OPENCLAWCN_GATEWAY_TOKEN;
    } else {
      process.env.OPENCLAWCN_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.OPENCLAWCN_GATEWAY_PASSWORD;
    } else {
      process.env.OPENCLAWCN_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
