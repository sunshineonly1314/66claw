...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    ...
    // Start with only openclawcn profile
    ...
    // 2. Simulate adding a new profile to config (like user editing openclawcn.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.openclawcn = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.openclawcn?.cdpPort).toBe(19999);
  });
