...
describe("runGatewayLoop", () => {
  it("restarts after SIGUSR1 even when drain times out, and resets lanes for the new iteration", async () => {
    ...
    type StartServer = () => Promise<{
      ...
      notifyDraining?: (info: Record<string, unknown>) => void;
    }>;

    const closeFirst = vi.fn(async () => {});
    const closeSecond = vi.fn(async () => {});
    const notifyDraining = vi.fn();
    const start = vi
      .fn<StartServer>()
      .mockResolvedValueOnce({ close: closeFirst, notifyDraining })
      .mockResolvedValueOnce({ close: closeSecond, notifyDraining })
