...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openclawcn-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENCLAWCN_NO_RESPAWN: "1",
      OPENCLAWCN_STATE_DIR: stateDir,
      OPENCLAWCN_SKIP_CHANNELS: "1",
      OPENCLAWCN_SKIP_GMAIL_WATCHER: "1",
      OPENCLAWCN_SKIP_CRON: "1",
      OPENCLAWCN_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENCLAWCN_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openclawcn-entry-bootstrap.mjs");
