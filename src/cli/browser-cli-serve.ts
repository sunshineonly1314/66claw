...
export function registerBrowserServeCommands(
  ...
  browser
    ...
    .action(async (opts: { bind?: string; port?: string; token?: string }) => {
      ...
      const envToken = process.env.OPENCLAWCN_BROWSER_CONTROL_TOKEN?.trim();
      const authToken = (opts.token ?? envToken)?.trim();
      if (!isLoopbackBindHost(host) && !authToken) {
        defaultRuntime.error(
          danger(
            `Refusing to bind browser control on ${host} without --token (or OPENCLAWCN_BROWSER_CONTROL_TOKEN).`,
          ),
        );
        defaultRuntime.exit(1);
      }
      ...
      defaultRuntime.log(
        info(
          [
            ...
            "",
            "Paste on the Gateway (openclawcn.json):",
            JSON.stringify(
              {
                browser: {
                  enabled: true,
            ...
            ...(authToken
              ? [
                  ...
                  `export OPENCLAWCN_BROWSER_CONTROL_TOKEN=${JSON.stringify(authToken)}`,
                ]
              : []),
          ].join("\n"),
        ),
