...
export function applyCliProfileEnv(params: {
  ...
  env.OPENCLAWCN_PROFILE = profile;

  const stateDir = env.OPENCLAWCN_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.OPENCLAWCN_STATE_DIR?.trim()) {
    env.OPENCLAWCN_STATE_DIR = stateDir;
  }

  if (!env.OPENCLAWCN_CONFIG_PATH?.trim()) {
    env.OPENCLAWCN_CONFIG_PATH = path.join(stateDir, "openclawcn.json");
  }

  if (profile === "dev" && !env.OPENCLAWCN_GATEWAY_PORT?.trim()) {
    env.OPENCLAWCN_GATEWAY_PORT = "19002";
  }
}
