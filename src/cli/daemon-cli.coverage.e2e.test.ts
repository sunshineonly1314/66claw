...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    OPENCLAWCN_CONFIG_PATH: process.env.OPENCLAWCN_CONFIG_PATH,
    OPENCLAWCN_GATEWAY_PORT: process.env.OPENCLAWCN_GATEWAY_PORT,
    OPENCLAWCN_PROFILE: process.env.OPENCLAWCN_PROFILE,
  };

  beforeEach(() => {
    process.env.OPENCLAWCN_STATE_DIR = "/tmp/openclawcn-cli-state";
    process.env.OPENCLAWCN_CONFIG_PATH = "/tmp/openclawcn-cli-state/openclawcn.json";
    delete process.env.OPENCLAWCN_GATEWAY_PORT;
    delete process.env.OPENCLAWCN_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPENCLAWCN_STATE_DIR !== undefined) {
      process.env.OPENCLAWCN_STATE_DIR = originalEnv.OPENCLAWCN_STATE_DIR;
    } else {
      delete process.env.OPENCLAWCN_STATE_DIR;
    }

    if (originalEnv.OPENCLAWCN_CONFIG_PATH !== undefined) {
      process.env.OPENCLAWCN_CONFIG_PATH = originalEnv.OPENCLAWCN_CONFIG_PATH;
    } else {
      delete process.env.OPENCLAWCN_CONFIG_PATH;
    }

    if (originalEnv.OPENCLAWCN_GATEWAY_PORT !== undefined) {
      process.env.OPENCLAWCN_GATEWAY_PORT = originalEnv.OPENCLAWCN_GATEWAY_PORT;
    } else {
      delete process.env.OPENCLAWCN_GATEWAY_PORT;
    }

    if (originalEnv.OPENCLAWCN_PROFILE !== undefined) {
      process.env.OPENCLAWCN_PROFILE = originalEnv.OPENCLAWCN_PROFILE;
    } else {
      delete process.env.OPENCLAWCN_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENCLAWCN_PROFILE: "dev",
        OPENCLAWCN_STATE_DIR: "/tmp/openclawcn-daemon-state",
        OPENCLAWCN_CONFIG_PATH: "/tmp/openclawcn-daemon-state/openclawcn.json",
        OPENCLAWCN_GATEWAY_PORT: "19001",
      },
