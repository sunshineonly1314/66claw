...
function appendClawHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: browse official skills at https://clawhub.com\n     If unreachable, use \`npx clawhub\` or the built-in skills market in the app.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("openclawcn skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendClawHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("openclawcn skills list")}\` to see available skills.`,
      opts.json,
    );
  }
