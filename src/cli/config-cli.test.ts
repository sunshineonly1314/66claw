...
import type { ConfigFileSnapshot, OpenClawCNConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `openclawcn config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<[], Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<[OpenClawCNConfig], Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: OpenClawCNConfig) => mockWriteConfigFile(cfg),
}));
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: OpenClawCNConfig = {
        agents: {
      ...
      const runtimeMerged: OpenClawCNConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
