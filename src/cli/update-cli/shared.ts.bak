...
const OPENCLAWCN_REPO_URL = "https://github.com/openclawcn/openclawcn.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "openclawcn";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openclawcn@")) {
    return trimmed.slice("openclawcn@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.OPENCLAWCN_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENCLAWCN_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENCLAWCN_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCLAWCN_GIT_DIR to an empty folder or an openclawcn checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENCLAWCN_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "openclawcn.mjs");
