  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("openclawcn completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OPENCLAWCN_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OPENCLAWCN_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("openclawcn gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openclawcn doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("openclawcn gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openclawcn gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating OpenClawCN..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OpenClawCN install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("openclawcn doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("openclawcn gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g openclawcn@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g openclawcn@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
