...
function listAgents(cfg: OpenClawCNConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OpenClawCNConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpenClawCNConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OpenClawCNConfig, EvaluatedBindingsCache>();
