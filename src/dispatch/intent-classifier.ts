...
const log = createSubsystemLogger("dispatch/intent-classifier");
...
export async function classifyIntent(params: ClassifyIntentParams): Promise<ClassifyIntentResult> {
  ...
  if (ruleResults.length > 0 && ruleResults[0].confidence >= threshold) {
    if (compiled.settings.debug) {
      log.debug(
        `Rule match: intent=${ruleResults[0].intentId} ` +
  ...
  if (llmResult) {
    if (compiled.settings.debug) {
      log.debug(
        `LLM match: intent=${llmResult.intentId} ` +
  ...
  if (ruleResults.length > 0) {
    if (compiled.settings.debug) {
      log.debug(
        `Fallback to best rule: intent=${ruleResults[0].intentId} ` +
          `confidence=${ruleResults[0].confidence.toFixed(2)}`,
      );
    }
    return {
