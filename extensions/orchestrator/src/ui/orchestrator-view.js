...
function renderOrchestrator(state, handlers, t) {
  return html`
    <div class="orch-container">
      ...
      <div class="orch-thread">
        ...
        ${state.phase === "gathering" && state.gatheringQuestions.length > 0 ? renderGatheringQuestions(state, handlers, t) : nothing}
        ${state.phase === "proposed" && state.proposal ? renderTeamProposal(state.proposal, handlers, t) : nothing}
        ${state.phase === "previewing" && state.previewTemplate ? renderTemplatePreview(state.previewTemplate, handlers, t) : nothing}
        ...
        ${state.phase === "proposing" || state.phase === "refining" ? state.proposingSteps.length > 0 ? renderProposingSteps(state.proposingSteps, t) : renderThinking(t) : nothing}
      </div>
      ${renderCompose(state, handlers, t)}
    </div>
  `;
}
function renderHeader(handlers, t) {
  return html`
...
function renderWelcome(state, handlers, t) {
  const examples = [
    "\u5E2E\u6211\u505A\u4E00\u4E2A\u6BCF\u5929\u603B\u7ED3\u884C\u4E1A\u65B0\u95FB\u7684\u56E2\u961F",
    "\u6211\u5F00\u6DD8\u5B9D\u5E97\uFF0C\u9700\u8981AI\u5E2E\u6211\u5904\u7406\u552E\u524D\u54A8\u8BE2",
    "\u6211\u60F3\u8981\u4E00\u4E2A\u5E2E\u6211\u7BA1\u7406\u5FAE\u4FE1\u793E\u7FA4\u7684\u52A9\u624B"
...
function renderCommunitySection(state, handlers, t) {
...
function renderCommunityCard(tpl, index, handlers, t) {
  return html`
...
function renderMessages(state, handlers, t) {
  return html`
...
function renderProposalWidget(data, handlers, t) {
  const proposal = data;
...
function renderQuestionsWidget(data, handlers) {
  const qData = data;
...
function renderSoulPreviewWidget(data, handlers, t) {
  const soulData = data;
  if (!soulData?.agents?.length) return html``;
  return html`
    ${soulData.agents.map((agent, i) => html`
      <div class="orch-soul">
        <div class="orch-soul-head" @click=${(e) => {
...
function renderGatheringQuestions(state, handlers, t) {
...
function renderTeamProposal(proposal, handlers, t) {
  return html`
        ...
        ${proposal.agents.map((agent, i) => html`
          <div class="orch-preview-agent orch-ability-card">
            ...
            <div class="orch-preview-agent-tags">
              <span class="orch-preview-tag orch-preview-tag--tier">${agent.modelName ?? (agent.modelTier === "sota" ? t("orch.tierSota") : agent.modelTier === "mid" ? t("orch.tierMid") : t("orch.tierCheap"))}</span>
...
function renderTemplatePreview(tpl, handlers, t) {
  return html`
    <div class="orch-preview">
      <div class="orch-preview-header">
      ...
      <div class="orch-preview-agents">
        ${tpl.agents.map((agent, i) => html`
          <div class="orch-preview-agent">
            ...
            <div class="orch-preview-agent-tags">
              <span class="orch-preview-tag orch-preview-tag--tier">${agent.modelTier === "sota" ? t("orch.tierSota") : agent.modelTier === "mid" ? t("orch.tierMid") : t("orch.tierCheap")}</span>
...
function formatProfileName(profile, t) {
  const PROFILE_NAMES = {
    minimal: "minimal",
    coding: "coding",
    messaging: "messaging",
...
function renderDeployProgress(state, handlers, t) {
  ...
  return html`
    ...
    const cls = agent.status === "ready" ? "done" : agent.status === "failed" ? "failed" : agent.status === "pending" ? "pending" : "active";
...
function humanizeDeployError(error, t) {
...
function renderSuccess(state, handlers, t) {
  ...
  return html`
    ...
    if (e.key === "Enter") {
      ...
      if (val) handlers.onActionClick("try-it-send", val);
          ...
          />
          <button class="btn btn--sm primary" @click=${(e) => {
...
function renderDeployReportSummary(summary, t) {
  return html`
...
function renderAgentStepBadges(agentReport, t) {
  ...
  return html`
    <div class="orch-deployed-card-steps">
      ${agentReport.steps.filter((s) => s.status !== "ok").map((s) => html`
          <span class="orch-step-badge orch-step-badge--${s.status}" title=${s.detail}>
            ${s.step === "soul" ? t("orch.stepSoul") : s.step === "tool-policy" ? t("orch.stepToolPolicy") : s.step === "keywords" ? t("orch.stepKeywords") : s.step}
          </span>
        `)}
    </div>
  `;
}
function renderError(state, handlers, t) {
...
function renderProposingSteps(steps, t) {
  return html`
        ...
        ${steps.map((step, i) => html`
          <div class="orch-proposing-step orch-proposing-step--${step.status}">
            <div class="orch-proposing-step-icon">
              ${step.status === "done" ? checkIcon : step.status === "active" ? html`<div class="orch-deploy-card-spinner"></div>` : html`<span class="orch-proposing-step-num">${i + 1}</span>`}
            </div>
...
function renderCompose(state, handlers, t) {
...
function renderOrchestratorEntry(onOpen, t) {
  return html`
