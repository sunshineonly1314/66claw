...
import { inferAgentCapabilities, estimateRoleComplexity, isSupervisorRole } from "../guided/capability-inference.js";
...
function generateTestTeam(requirement: string, userContext: UserContext): AgentBlueprint[] {
  ...
  type RoleCandidate = {
    name: string; id: string; pattern: RegExp; priority: number;
    baseRole: string; modelTier: "cheap" | "mid" | "sota";
    tools: { allow: string[]; profile?: string };
  };

  const ROLE_CANDIDATES: RoleCandidate[] = [
    { name: "文案写手", id: "copywriter", pattern: /写|文案|内容|创作|copy|writing|content/i, priority: 20, baseRole: "撰写和优化各类文案内容", modelTier: "mid", tools: { allow: ["group:web", "group:memory"] } },
    { name: "编程助手", id: "code-assistant", pattern: /代码|开发|编程|code|dev|program/i, priority: 15, baseRole: "编写、审查和调试代码", modelTier: "mid", tools: { allow: ["group:web", "group:fs", "group:memory"], profile: "coding" } },
    { name: "数据分析师", id: "data-analyst", pattern: /数据|分析|报表|data|analy|统计/i, priority: 25, baseRole: "分析数据、生成报表和可视化", modelTier: "mid", tools: { allow: ["group:web", "group:memory", "group:fs"] } },
    { name: "客服专员", id: "support-agent", pattern: /客服|support|答疑|咨询|接待|helpdesk/i, priority: 20, baseRole: "自动回答客户常见问题，处理咨询", modelTier: "cheap", tools: { allow: ["group:web", "group:memory"] } },
    { name: "研究员", id: "researcher", pattern: /研究|调研|research|报告|论文|paper/i, priority: 20, baseRole: "搜索资料、整理信息、撰写研究报告", modelTier: "mid", tools: { allow: ["group:web", "group:memory"] } },
    { name: "翻译专员", id: "translator", pattern: /翻译|translate|双语|多语|本地化|locali[sz]/i, priority: 25, baseRole: "翻译和本地化各类文档内容", modelTier: "mid", tools: { allow: ["group:web", "group:memory"] } },
    { name: "配图助手", id: "image-helper", pattern: /配图|图片|封面|插图|image|illustrat|画图|设计图/i, priority: 30, baseRole: "生成配图、封面和视觉素材", modelTier: "mid", tools: { allow: ["group:web", "group:memory"] } },
    { name: "新闻采编", id: "news-editor", pattern: /新闻|资讯|热点|简报|news|briefing|早报/i, priority: 25, baseRole: "采集新闻资讯、编辑简报", modelTier: "mid", tools: { allow: ["group:web", "group:memory"] } },
    { name: "日程管家", id: "scheduler", pattern: /日程|日历|提醒|预约|schedule|calendar|remind|会议/i, priority: 30, baseRole: "管理日程安排和定时提醒", modelTier: "cheap", tools: { allow: ["group:memory"] } },
  ];
  ...
  if (matched.length === 0) {
    team.push({
      name: "主力助手", id: "primary-assistant",
      role: "核心任务处理，负责响应用户需求和执行主要工作",
      soul: "", modelTier: userContext.budget === "cheap" ? "cheap" : "mid",
      tools: { allow: ["group:web", "group:memory"], profile: "minimal" },
    });
  } else {
    for (const m of matched) {
      team.push({
        name: m.name, id: m.id, role: m.contextRole, soul: "",
...
describe("场景1: 跨境电商客服+数据分析", () => {
  ...
  it("客服agent应该推荐客服相关skills", () => {
    ...
    expect(
      skills.some(s => /troubleshoot|summarize|faq|wechat-cs/i.test(s)),
    ).toBe(true);
  });
  ...
  it("数据分析agent应该推荐分析相关skills", () => {
    ...
    expect(
      skills.some(s => /csv|data|nano-pdf|sql/i.test(s)),
    ).toBe(true);
  });
...
describe("场景3: 复杂科研需求", () => {
  const requirement = "我要搭建一个AI科研团队，能帮我调研论文、写代码做实验、分析实验数据，最后写研究报告";
  const ctx = defaultCtx({
    scenario: "research",
  ...
  it("编程助手应该有coding相关的tools和skills", () => {
    ...
    expect(caps.tools.alsoAllow?.some(t => /fs|runtime/i.test(t)) ||
           coder!.tools.allow?.includes("group:fs")).toBe(true);
  });
  ...
  it("研究员应该有web搜索和pdf相关能力", () => {
    ...
    expect(
      caps.skills.some(s => /researcher|web-research|nano-pdf|summarize/i.test(s)),
    ).toBe(true);
  });
...
describe("场景6: 能力推断精准度审计", () => {
  it("客服agent不应该推荐coding-agent skill", () => {
    const bp: AgentBlueprint = {
      name: "客服专员", id: "cs", role: "自动回答客户常见问题，处理咨询",
      soul: "", modelTier: "cheap",
  ...
  it("编程助手不应该推荐xiaohongshu skill", () => {
    const bp: AgentBlueprint = {
      name: "编程助手", id: "coder", role: "编写、审查和调试代码",
      soul: "", modelTier: "mid",
  ...
  it("简单转发角色应该用cheap模型", () => {
    const bp: AgentBlueprint = {
      name: "消息转发", id: "fwd", role: "转发消息通知到指定渠道",
      soul: "", modelTier: "mid",
      tools: { allow: [] },
    };
    ...
    const cheapModels = ["deepseek/deepseek-chat", "qwen/qwen-turbo", "qwen/qwen-plus", "zhipu/glm-4-plus"];
    expect(cheapModels.some(m => model.includes(m.split("/")[1]))).toBe(true);
  });

  it("研究角色应该有heartbeat=undefined（非定时任务）", () => {
    const bp: AgentBlueprint = {
      name: "研究员", id: "researcher", role: "搜索资料、整理信息、撰写研究报告",
      soul: "", modelTier: "mid",
      tools: { allow: [] },
    };
  ...
  it("定时提醒角色应该有heartbeat", () => {
    const bp: AgentBlueprint = {
      name: "提醒助手", id: "reminder", role: "定时提醒用户完成待办事项",
      soul: "", modelTier: "cheap",
      tools: { allow: [] },
    };
  ...
  it("知识库角色应该启用memorySearch", () => {
    const bp: AgentBlueprint = {
      name: "知识管家", id: "kb", role: "管理知识库，检索历史记录",
      soul: "", modelTier: "mid",
      tools: { allow: [] },
    };
    const caps = inferAgentCapabilities(bp, defaultCtx());
...
describe("场景7: Pipeline多轮优化质量", () => {
  it("应该修复complexity-model mismatch（复杂角色用cheap模型）", () => {
    const blueprints: AgentBlueprint[] = [
      {
        name: "代码架构师", id: "architect",
        role: "负责系统架构设计、代码审查和技术选型",
        soul: "", modelTier: "cheap", // 故意错误：complex角色用cheap
        tools: { allow: ["group:fs"] },
      },
    ];
    const result = runPipeline("我需要一个架构师帮我做代码审查", blueprints, defaultCtx({ scenario: "coding" }));
  ...
  it("应该检测重复agent ID", () => {
    const blueprints: AgentBlueprint[] = [
      { name: "助手A", id: "helper", role: "做事A", soul: "", modelTier: "mid", tools: { allow: [] } },
      { name: "助手B", id: "helper", role: "做事B", soul: "", modelTier: "mid", tools: { allow: [] } },
    ];
  ...
  it("应该检测团队规模过大（>8个成员）", () => {
    ...
    for (let i = 0; i < 9; i++) {
      blueprints.push({
        name: `Agent ${i}`, id: `agent-${i}`,
        role: `处理任务 ${i}`, soul: "", modelTier: "cheap",
        tools: { allow: [] },
      });
    }
...
describe("场景10: MCP Server推荐精准度", () => {
  it("数据库相关角色应该推荐sqlite MCP", () => {
    const bp: AgentBlueprint = {
      name: "数据管理", id: "db", role: "管理数据库和执行SQL查询",
      soul: "", modelTier: "mid", tools: { allow: [] },
    };
  ...
  it("github相关角色应该推荐github MCP", () => {
    const bp: AgentBlueprint = {
      name: "代码管理", id: "git", role: "管理github仓库和PR",
      soul: "", modelTier: "mid", tools: { allow: [] },
    };
  ...
  it("notion相关角色应该推荐notion MCP", () => {
    const bp: AgentBlueprint = {
      name: "笔记管理", id: "note", role: "管理notion文档和知识库",
      soul: "", modelTier: "mid", tools: { allow: [] },
    };
  ...
  it("纯文案角色不应推荐数据库MCP", () => {
    const bp: AgentBlueprint = {
      name: "文案写手", id: "writer", role: "撰写小红书文案和公众号文章",
      soul: "", modelTier: "mid", tools: { allow: [] },
    };
...
describe("场景13: Skills推荐上限控制", () => {
  ...
  it("每个agent的MCP hints不应超过7个", () => {
    ...
    const caps = inferAgentCapabilities(bp, defaultCtx({ resources: ["database", "github", "notion", "pdf"] }));
...
describe("场景14: Budget对模型选择的影响", () => {
  ...
  it("premium预算应该把cheap tier升级到mid", () => {
    const bp: AgentBlueprint = {
      name: "助手", id: "h", role: "通用辅助",
      soul: "", modelTier: "cheap", tools: { allow: [] },
    };
