...
async function discoverInstalledSkills(workspaceDir?: string): Promise<DiscoveredSkill[]> {
  ...
  try {
    ...
    return entries.map((entry) => ({
      ...
      source: ((entry.metadata as Record<string, unknown> | undefined)?.source as DiscoveredSkill["source"]) ?? "bundled",
...
async function discoverMCPServers(): Promise<DiscoveredMCPServer[]> {
  try {
    ...
    return (status.servers ?? []).map((server: { config: { id: string; enabled: boolean }; status: string; tools?: Array<{ name: string; description?: string }> }) => ({
      id: server.config.id,
...
function tokenizeForMatching(text: string): string[] {
  ...
  const stopWords = new Set([
    ...
    "can", "will", "but", "all", "each", "into", "over", "also", "use", "how",
    "的", "和", "在", "是", "了", "有", "不", "人", "我", "他", "她", "们",
    "这", "那", "个", "上", "下", "把", "被", "让", "给", "对", "到",
    "agent", "助手", "负责", "进行", "工作",
  ]);
...
export function matchCapabilitiesToRole(
  ...
  const confidence = allScores.length > 0
