...
function resolveClawdbotRoot(): string {
  ...
  const override = process.env.OPENCLAWCN_ROOT?.trim() || process.env.CLAWDBOT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    const found = findPackageRoot(start, "openclawcn");
    if (found) {
      coreRootCache = found;
      return found;
    }
  ...
  throw new Error(
    "Unable to resolve OpenClawCN root. Set OPENCLAWCN_ROOT to the package root.",
  );
}
