...
export function buildMemberPerformanceProfile(
  ...
  for (const member of nonSupervisor) {
    ...
    if (avgMs > 10000) notes.push("响应较慢");
 ...
 *
 * If the SOUL already contains a `<learning-hints>` block, it's replaced.
 * If not, the hints are appended before the last section.
 *
 * @param existingSoul - The current SOUL.md content
 * @param hints - New learning hints content (output of generateLearningHints)
 * @returns Updated SOUL string with learning hints
 */
export function appendLearningHintsToSoul(existingSoul: string, hints: string): string {
  ...
  // Check if existing SOUL already has learning hints
  ...
  // Insert after Quality Gates (Phase 4 section) but before Operating Rules.
  // This keeps learning hints adjacent to the performance/routing sections.
  const qualityGatesIdx = existingSoul.indexOf("## Quality Gates");
  if (qualityGatesIdx > 0) {
    // Find the next "##" heading after Quality Gates
    const afterQG = existingSoul.indexOf("\n## ", qualityGatesIdx + 1);
    if (afterQG > 0) {
      return (
        existingSoul.slice(0, afterQG) +
        "\n\n" + hints +
        existingSoul.slice(afterQG)
      );
    }
  }
 ...
 *
 * This is separate from SOUL — it's injected dynamically each time
 * the supervisor starts processing a message.
 *
 * @returns Context string, or empty string if no useful data
 */
export function buildSupervisorLearningContext(
  ...
  const profileSection = buildMemberPerformanceProfile(
    project, statsMap, healthMap, analysis?.specializations,
  );
  if (profileSection) {
