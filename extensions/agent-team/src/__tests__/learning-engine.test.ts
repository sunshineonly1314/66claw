...
function makeHealth(agentId: string, state: "healthy" | "degraded" | "down" = "healthy"): MemberHealth {
  return {
    agentId,
    state,
...
function makeHealthMap(project: Project, overrides?: Record<string, "healthy" | "degraded" | "down">): Map<string, MemberHealth> {
...
function makeStatsMap(project: Project, countOverrides?: Record<string, number>): Map<string, MemberStats> {
...
describe("learning-engine", () => {
  ...
  describe("Pattern 3: utilization_imbalance", () => {
    it("detects heavily overloaded agent (> 60% share)", () => {
      ...
      const imbalanceInsights = result.insights.filter(i => i.category === "utilization_imbalance");
  ...
  describe("generateLearningHints", () => {
    ...
    it("includes routing patterns in hints when specializations also present", () => {
      ...
      const analysis: LearningAnalysis = {
        ...
        specializations: [
          { agentId: "agent-a", strengths: ["销售"], avgDurationMs: 1500, successRate: 0.95, totalCalls: 20 },
        ],
        summary: "分析 50 个事件",
      };
      const hints = generateLearningHints(analysis);
