...
function makeAnalysis(overrides?: Partial<LearningAnalysis>): LearningAnalysis {
  return {
    ...
    insights: [],
    routingPatterns: [
      { trigger: "报价", agentId: "agent-a", confidence: 0.92, sampleSize: 15 },
    ],
    specializations: [
      { agentId: "agent-a", strengths: ["报价", "客户服务"], avgDurationMs: 1200, successRate: 0.95, totalCalls: 15 },
  ...
  describe("buildMemberPerformanceProfile", () => {
    ...
    it("respects MAX_PROFILE_CHARS limit (800 chars)", () => {
      ...
      const statsMap = new Map(
        manyMembers.map(m => [m.id, makeStats(m.id, 10, 10_000)]),
      );
      const healthMap = new Map(
        manyMembers.map(m => [m.id, makeHealth(m.id)]),
      );
    ...
    it("uses specialization strengths when provided", () => {
      ...
      const specializations: AgentSpecialization[] = [
        { agentId: "agent-a", strengths: ["报价", "折扣"], avgDurationMs: 1000, successRate: 0.95, totalCalls: 10 },
      ];
