...
const OVERLOADED_THRESHOLD = 0.60;

/** High failure rate threshold */
const HIGH_FAILURE_RATE = 0.30;
 ...
 * @param events - Activity buffer events
 * @param healthMap - Current member health states
 * @param statsMap - Current member call stats
 * @param project - Project definition (for member info)
 * @returns Complete learning analysis
 */
...
function analyzeRoutingFailures(
  events: ActivityEventLike[],
  project: Project,
): LearningInsight[] {
  const insights: LearningInsight[] = [];
  ...
  for (const [agentId, data] of agentFailures) {
    ...
    if (failureRate >= HIGH_FAILURE_RATE) {
      ...
      // Check if another agent could handle these tasks better
...
function analyzeTimeoutPatterns(
  ...
  for (const member of project.members) {
    if (member.id === project.supervisorId) continue; // Skip supervisor
    ...
    if (avgDuration > SLOW_AGENT_THRESHOLD_MS) {
      // Check if timeouts are increasing
      const recentEvents = events
        .filter(e => e.agentId === member.id)
        .slice(-10);
...
function analyzeUtilizationBalance(
  events: ActivityEventLike[],
  _statsMap: Map<string, MemberStats>,
  project: Project,
): LearningInsight[] {
  const insights: LearningInsight[] = [];

  // Count events per agent (excluding supervisor)
...
function analyzeSuccessPatterns(
  ...
  // Find agents with consistently high success rates
...
function buildRoutingPatterns(
  ...
  // For each pattern, find the best agent
...
function findBetterAlternative(
  ...
  // Find another agent in the same team with better success rate AND overlapping task types
  const nonSupervisorMembers = project.members.filter(
    m => m.id !== project.supervisorId && m.id !== failingAgentId,
  );

  // Collect task types the failing agent has been handling
  ...
  for (const member of nonSupervisorMembers) {
    ...
    // Check role compatibility: candidate must have handled at least one overlapping task type
    // or have keyword overlap in their role (if taskType data is unavailable)
 ...
 * Returns a list of applied changes for the audit log.
 *
...
export function applyAutoOptimizations(
  ...
  for (const pattern of analysis.routingPatterns) {
    // Require reasonable confidence (≥75%) and sufficient observations (≥3)
    ...
    if (!keywords.some(kw => kw.toLowerCase() === trigger)) {
      member.keywords = [...keywords, trigger]; // Normalize to lowercase
      changes.push(`为「${member.name}」添加路由关键词「${trigger}」（历史成功率 ${Math.round(pattern.confidence * 100)}%, n=${pattern.sampleSize}）`);
    }
  }

  // Auto-opt 2: Note successful specializations (informational, for SOUL hints)
  // This is tracked in the analysis but not auto-applied to project config
 ...
 * Compact format, budget-controlled.
 */
export function generateLearningHints(analysis: LearningAnalysis): string {
  ...
  // Specialization profiles (most useful for routing)
  ...
  // Key insights (high severity only)
...
export function formatLearningReport(analysis: LearningAnalysis): string {
  ...
  if (analysis.specializations.length > 0) {
    ...
    for (const spec of analysis.specializations) {
      ...
      lines.push(
        `  ${spec.agentId}: 成功率 ${Math.round(spec.successRate * 100)}%, ` +
        `平均 ${Math.round(spec.avgDurationMs / 1000)}s, ` +
        `擅长 ${strengths} (${spec.totalCalls} 次调用)`,
      );
    }
    lines.push("");
  }
  ...
  if (analysis.routingPatterns.length > 0) {
    ...
    for (const pattern of analysis.routingPatterns) {
      lines.push(
        ...
        `(置信度 ${Math.round(pattern.confidence * 100)}%, 样本 ${pattern.sampleSize})`,
      );
    }
    lines.push("");
  }
