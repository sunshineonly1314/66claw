 ...
 *
 * If ActivityEvent fields are renamed/removed, callers that pass
 * ActivityEvent[] here will get a compile error due to structural
 * typing — no explicit `extends` needed.
 */
...
export function formatActivitySummary(
  ...
  for (const evt of recent) {
    ...
    const duration = evt.durationMs != null
...
function formatOutcome(evt: ActivityEventLike): string {
  ...
  if (evt.outcome) {
    switch (evt.outcome) {
      ...
      case "failure": return evt.error ? `failed (${sanitizeForPrompt(truncate(evt.error, 40))})` : "failed";
      case "timeout": return "timed out";
      case "partial": return "partial result";
      default: return sanitizeForPrompt(evt.outcome);
    }
  }
