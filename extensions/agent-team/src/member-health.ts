 ...
 *
 * Inspired by src/dispatch/provider-health.ts but with agent-level thresholds.
 */
...
export function recordMemberSuccess(health: MemberHealth): MemberHealth {
  ...
  // Transition: degraded → healthy (3 consecutive successes)
    ...
    updated.consecutiveSuccesses >= RECOVERY_THRESHOLD
  ) {
 ...
 */
export function recordMemberFailure(
  health: MemberHealth,
  error?: string,
): MemberHealth {
  const now = new Date().toISOString();
  ...
  if (
    ...
    updated.consecutiveFailures >= DEGRADED_THRESHOLD
  ) {
    updated.state = "degraded";
  }
  // Transition: degraded → down
    ...
    health.state === "degraded" &&
    updated.consecutiveFailures >= DOWN_THRESHOLD
  ) {
    updated.state = "down";
  }
