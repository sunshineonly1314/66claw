...
const SHARED_PROFILE_MAX_ENTRIES = 50;
const SHARED_MEMORY_MAX_PROMPT_CHARS = 1500;
const SHARED_MAX_KEY_LENGTH = 50;
const SHARED_MAX_VALUE_LENGTH = 200;
...
function resolveSharedProfileDir(projectId) {
  return path.join(resolveProjectDir(projectId), SHARED_MEMORY_DIR);
}
function resolveSharedProfilePath(projectId) {
  return path.join(resolveSharedProfileDir(projectId), SHARED_PROFILE_FILENAME);
}
function computeSharedEntryScore(entry, now = Date.now()) {
  ...
  const ageDays = Math.max(
    0,
    (now - entry.updatedAt) / (1e3 * 60 * 60 * 24)
  );
...
function readSharedProfile(projectId) {
...
function writeSharedProfile(projectId, profile) {
  const filePath = resolveSharedProfilePath(projectId);
...
const _locks = /* @__PURE__ */ new Map();
const LOCK_TIMEOUT_MS = 3e4;
async function withSharedProfileLock(projectId, fn) {
...
function upsertSharedEntry(profile, entry) {
  const now = Date.now();
...
function formatSharedProfileForPrompt(profile, maxChars = SHARED_MEMORY_MAX_PROMPT_CHARS, excludeAgentId) {
  const eligible = excludeAgentId ? profile.entries.filter((e) => e.sourceAgentId !== excludeAgentId) : profile.entries;
  if (eligible.length === 0) return "";
  const sorted = [...eligible].sort(
    (a, b) => computeSharedEntryScore(b) - computeSharedEntryScore(a)
  );
  const categoryOrder = ["identity", "fact", "preference"];
  const categoryLabels = {
    identity: "Identity",
    fact: "Facts",
    preference: "Preferences"
  };
  const lines = ["## Team Shared Knowledge"];
...
function sanitizeCrossAgentValue(value) {
  let s = value.replace(/[\r\n]+/g, " ").replace(
    ...
    "[FILTERED]"
  ).replace(/(?:忽略|无视|覆盖).*(?:指令|规则|提示|上下文)/gi, "[FILTERED]").replace(/(?:你现在是|扮演|假装)/gi, "[FILTERED]").replace(/^#{1,6}\s+/gm, "").trim();
  if (s.length > SHARED_MAX_VALUE_LENGTH) {
    s = s.slice(0, SHARED_MAX_VALUE_LENGTH);
  }
  return s;
}
function sanitizeSharedKey(key) {
  return key.replace(/[\r\n\t]+/g, "_").replace(/[#<>[\]{}`]/g, "").trim().slice(0, SHARED_MAX_KEY_LENGTH);
}
function resetSharedProfileCache() {
  _cache.clear();
}
function resetSharedProfileLocks() {
  _locks.clear();
}
export {
