...
import { FeishuDocJsonSchema, type FeishuDocParams } from "./doc-schema.js";
...
export function registerFeishuDocTools(api: ClawdbotPluginApi) {
    ...
    api.registerTool(
      {
        ...
        label: "飞书文档",
        description:
          "飞书文档操作。支持: read(读取), write(覆盖写入), append(追加), create(创建), list_blocks(列出块), get_block(获取块), update_block(更新块), delete_block(删除块)",
    ...
    api.registerTool(
      {
        ...
        label: "飞书应用权限",
        description:
          "列出当前应用的权限 (scopes)。用于调试权限问题或检查可用能力。",
