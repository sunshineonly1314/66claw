...
function createConfig(overrides?: Record<string, unknown>): OpenClawCNConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclawcn-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("sendBlueBubblesMedia local-path hardening", () => {
  ...
  it("rejects relative mediaLocalRoots entries", async () => {
    const allowedRoot = await makeTempDir();
    const allowedFile = path.join(allowedRoot, "allowed.txt");
    // Use a genuinely relative path (path.relative may return an absolute path
    // on Windows when cwd and target are on different drives).
