...
### 目标
为 Clawdbot 建立完整的 Linux 打包、分发和安装体系，面向小白用户，实现：
- **一键安装**：用户只需一行命令即可完成安装
...
| 3 | **RPM spec 缺少 %dir 标记** | 添加 `%dir /opt/clawdbot` 确保目录归属 | 🔴 待修复 |

#### P1 (建议改进)

| # | 问题 | 改进建议 | 状态 |
|---|------|----------|------|
| 4 | **缺少日志轮转** | 添加 logrotate 配置 (`/etc/logrotate.d/clawdbot`) | 🟡 待实现 |
| 5 | **DEB 缺少依赖声明** | 添加 `Depends: libc6 (>= 2.17)` 等基础依赖 | 🟡 待实现 |
| 6 | **缺少 man page** | 生成 `clawdbot(1)` man page | 🟡 可选 |
...
| # | 问题 | 改进建议 | 状态 |
|---|------|----------|------|
| 11 | **缺少 APT/YUM 仓库** | 建立自有包仓库，用户可 `apt install clawdbot` | 🔵 未来 |
| 12 | **缺少 Snap/Flatpak** | 增加 Snap Store / Flathub 分发 | 🔵 未来 |
| 13 | **缺少自动更新** | 添加 `clawdbot update` 自更新机制 | 🔵 未来 |
...
```
build/linux-release/
├── clawdbot-linux-x64-standalone.tar.gz   ← 推荐给用户
├── clawdbot-linux-arm64-standalone.tar.gz
├── clawdbot-linux-portable.tar.gz
├── clawdbot_2026.2.0_amd64.deb
├── clawdbot_2026.2.0_arm64.deb
├── clawdbot-2026.2.0-1.x86_64.rpm
├── clawdbot-2026.2.0-1.aarch64.rpm
...
```bash
# 1. 下载 (选一个适合你系统的)
wget https://github.com/clawdbot/clawdbot/releases/latest/download/clawdbot-linux-x64-standalone.tar.gz
...
```bash
# 独立版
cd ~/clawdbot && ./uninstall.sh

# DEB 包
sudo dpkg -r clawdbot
