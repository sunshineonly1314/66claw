...
CLI 系统基于 Commander.js，提供 Clawdbot 的命令行界面，包括网关管理、技能操作、配置向导、维护诊断等功能。
...
| 命令 | 说明 |
|------|------|
| `clawdbot gateway run` | 启动网关服务 |
| `clawdbot gateway status` | 查看网关状态 |
| `clawdbot gateway health` | 健康检查 |
| `clawdbot gateway probe` | 探测连接 |
| `clawdbot gateway discover` | Bonjour 发现 |
| `clawdbot gateway usage` | 使用统计 |
...
| 命令 | 说明 |
|------|------|
| `clawdbot skills list` | 列出已安装技能 |
| `clawdbot skills info <name>` | 技能详情 |
| `clawdbot skills install <name>` | 安装技能 |
| `clawdbot skills remote` | 远程技能列表 |
| `clawdbot skills sync` | 同步技能索引 |
...
| 命令 | 说明 |
|------|------|
| `clawdbot doctor` | 诊断检查 |
| `clawdbot dashboard` | 仪表盘 |
| `clawdbot reset` | 重置配置 |
