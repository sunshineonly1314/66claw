 ...
 * 3. clawdhub-skills-mirror/skills/ (991 个 proxy 镜像)
 *
 * 用法:
...
const CONFIG = {
  sources: [
    ...
    { name: "mirror", path: "./clawdhub-skills-mirror/skills", priority: 2 },
    { name: "awesome", path: "./skills-awesome", priority: 3 },
  ],
  mergedDir: "./skills-merged",           // 合并后的技能目录
  outputBase: "./skillsqingxi/output-unified",
  batchSize: 300,
...
async function washBatchConcurrent(batchNum) {
  ...
  const promises = chunks.map(async (chunk) => {
    ...
    // 运行 skillsqingxi/run.ts - 每个子进程独立输出目录
    ...
    try {
      const { stdout, stderr } = await execAsync(`npx tsx skillsqingxi/run.ts --dir "${tempDir}"`, { env, timeout: 900000, maxBuffer: 50 * 1024 * 1024, cwd: process.cwd() });
...
║  来源:                                                   ║
║    skills/                        (55 本地)             ║
║    clawdhub-skills-mirror/skills/ (991 镜像)            ║
