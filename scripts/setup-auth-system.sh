#!/bin/bash
# Setup OpenClawCN Auth Management System
...
echo "=== OpenClawCN Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openclawcn-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenClawCN message: Send warning via OpenClawCN itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openclawcn-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openclawcn-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openclawcn-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openclawcn-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openclawcn-auth-monitor.timer"
echo ""
echo "Quick commands:"
