...
IMAGE_NAME="openclawcn-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/openclawcn-bin:$PATH"
  mkdir -p /tmp/openclawcn-bin

  cat > /tmp/openclawcn-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/openclawcn-bin/systemctl

  cat > /tmp/openclawcn-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/openclawcn-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/openclawcn"
  npm_entry="/tmp/npm-prefix/lib/node_modules/openclawcn/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/openclawcn-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/openclawcn-gateway.service"
    if [ ! -f "$unit_path" ]; then
