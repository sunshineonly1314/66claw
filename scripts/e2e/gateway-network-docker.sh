...
IMAGE_NAME="openclawcncn-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openclawcncn-net-e2e-$$"
GW_NAME="openclawcncn-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPENCLAWCN_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPENCLAWCN_SKIP_CHANNELS=1" \
	  -e "OPENCLAWCN_SKIP_GMAIL_WATCHER=1" \
	  -e "OPENCLAWCN_SKIP_CRON=1" \
	  -e "OPENCLAWCN_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
