...
IMAGE_NAME="openclawcncn-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    OPENCLAWCN_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    OPENCLAWCN_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export OPENCLAWCN_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/openclawcncn-bin:$PATH"
  mkdir -p /tmp/openclawcncn-bin
  cat > /tmp/openclawcncn-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/openclawcncn-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$OPENCLAWCN_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/openclawcncn-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/openclawcncn-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$OPENCLAWCN_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/openclawcncn-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    workspace_dir="$HOME/.openclawcn/workspace"
    config_path="$HOME/.openclawcn/openclawcn.json"
    sessions_dir="$HOME/.openclawcn/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    export HOME="$home_dir"
	    ...
	    node "$OPENCLAWCN_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.openclawcn/openclawcn.json" <<'"'"'JSON'"'"'
{
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$OPENCLAWCN_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$OPENCLAWCN_ENTRY\" configure --section skills" send_skills_flow
