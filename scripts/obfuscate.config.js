 ...
 * - 90% control flow flattening (nearly all code paths obscured)
 * - 40% dead code injection (heavy noise)
 * - 5 string array wrapper layers (deep indirection)
 * - Numbers → expressions, unicode escape sequences
 * - Finer string splitting (3-char chunks)
 *
 * Still safe:
...
export const aggressive = {
  ...
  // === Control Flow (MAXED) ===
  controlFlowFlattening: true,
  controlFlowFlatteningThreshold: 0.9,     // 0.5 → 0.9
  ...
  stringArrayRotate: true,
  stringArrayShuffle: true,
  stringArrayWrappersCount: 5,              // 2 → 5 (deeper indirection)
  stringArrayWrappersChainedCalls: true,
  stringArrayWrappersType: 'function',
  ...
  // === Split Strings (FINER) ===
  splitStrings: true,
  splitStringsChunkLength: 3,               // 8 → 3 (more fragmented)
