...
```bash
# 1. 清洗第一批
npx tsx skillsqingxi/batch-wash.ts --batch 1

# 2. Review 结果
npx tsx skillsqingxi/batch-wash.ts --review 1

# 3. 标记误判案例（手动检查前 50 个 rejected）
# 如果发现误判：
npx tsx skillsqingxi/batch-wash.ts --restore <skill-name>

# 4. 分析误判模式
npx tsx skillsqingxi/optimize-prompts.ts --analyze

# 5. 生成优化建议
npx tsx skillsqingxi/optimize-prompts.ts --suggest

# 6. 手动优化提示词
# - 编辑 skillsqingxi/layer1-rules.ts
# - 编辑 skillsqingxi/layer2-security.ts
# - 编辑 skillsqingxi/layer3-quality.ts
```
...
```bash
# 1. 清洗第二批（使用优化后的提示词）
npx tsx skillsqingxi/batch-wash.ts --batch 2

# 2. Review 结果
npx tsx skillsqingxi/batch-wash.ts --review 2
...
# 方式 1: 逐批处理
npx tsx skillsqingxi/batch-wash.ts --batch 3
npx tsx skillsqingxi/batch-wash.ts --batch 4
# ...
...
npx tsx skillsupdate/run.ts

# 4. 查看最终统计
npx tsx skillsupdate/run.ts --stats
```
