...
const CACHE_DIR = path.join(ROOT_DIR, ".release-cache");
...
async function main() {
  ...
  if (skipDelta) {
    ...
    for (const oldVersion of oldVersions) {
      ...
      const tarPath = path.join(DEPLOY_DIR, version, tarName);
      fs.mkdirSync(path.join(DEPLOY_DIR, version), { recursive: true });
  ...
  const fullTarPath = path.join(DEPLOY_DIR, version, "full.tar.gz");
  fs.mkdirSync(path.join(DEPLOY_DIR, version), { recursive: true });
  ...
  const versionDir = path.join(DEPLOY_DIR, version);
  ...
  // ─── Step 6: 生成 latest.json ───

  step(7, totalSteps, "生成 latest.json");

  // 根据上传方式确定下载 URL 前缀
  let urlBase: string;
  if (useOss) {
    if (ossDomain) {
      // 自定义域名: https://dl.openclawcn.com/releases/2026.2.20/full.tar.gz
      urlBase = `https://${ossDomain}/${OSS_CONFIG.keyPrefix}`;
    } else {
      // OSS 默认域名: https://bucket.oss-cn-hangzhou.aliyuncs.com/releases/2026.2.20/full.tar.gz
    ...
    if (!serverHost) {
      warn("未指定 --domain 且无 --server，latest.json 中的 URL 将使用 PLACEHOLDER_HOST");
  ...
  if (installersDir && fileExists(installersDir)) {
    ...
    for (const entry of entries) {
      ...
      if (ext === ".exe") platform = "windows-nsis";
      else if (ext === ".dmg") platform = "macos";
      else if (ext === ".msi") platform = "windows-msi";
      else if (ext === ".deb") platform = "linux-deb";
      else if (ext === ".appimage") platform = "linux-appimage";
  ...
  interface LatestJson {
    ...
    deltas: { from: string; url: string; size: number }[];
    fullSize: number;
    fullSha256: string;
  ...
  // Add installers to latest.json if any were found
  if (Object.keys(installers).length > 0) {
  ...
  if (outputOnly || skipUpload) {
    ...
    await uploadToOss(version, ossDomain);
  } else {
    uploadToServer(server!, port, version);
  }
...
function getCachedVersions(): string[] {
  if (!fileExists(CACHE_DIR)) return [];
...
/** 上传到阿里云 OSS */
async function uploadToOss(version: string, ossDomain?: string) {
  ...
  // 上传版本目录中的所有文件（递归，包含 installers/ 子目录）
  const allFiles = findFilesRecursive(versionDir);
  info(`上传 ${allFiles.length} 个文件到 OSS: ${OSS_CONFIG.bucket}/${prefix}/${version}/`);

  for (const relPath of allFiles) {
    const localPath = path.join(versionDir, relPath);
  ...
  info(`  full.tar.gz: ${urlHost}/${prefix}/${version}/full.tar.gz`);
}

/** scp 上传到服务器 */
function uploadToServer(server: string, port: string, version: string) {
  const versionDir = path.join(DEPLOY_DIR, version);
  const latestJsonPath = path.join(DEPLOY_DIR, "latest.json");
  ...
  // 在服务器上创建版本目录
  const hasInstallerFiles = fs.existsSync(path.join(versionDir, "installers"));
  const mkdirTarget = hasInstallerFiles
    ? `${SERVER_RELEASES_FS_PATH}/${version}/installers`
    : `${SERVER_RELEASES_FS_PATH}/${version}`;
  exec(`ssh -p ${port} ${server} "mkdir -p ${mkdirTarget}"`, { silent: true });
...
/** 缓存当前版本的 dist/ */
function cacheCurrentDist(version: string) {
