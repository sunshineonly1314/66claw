...
# OpenClawCN Auth Widget for Termux
...
SERVER="${OPENCLAWCN_SERVER:-l36}"

# Check auth status
termux-toast "Checking OpenClawCN auth..."

STATUS=$(ssh "$SERVER" '$HOME/openclawcn/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/openclawcn/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPENCLAWCN_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/openclawcn/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENCLAWCN_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpenClawCN won't work until re-auth"
                ;;
        esac
        ;;
