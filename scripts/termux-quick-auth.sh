...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openclawcn/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenClawCN Re-Auth" -c "After getting key, run: ssh $SERVER '~/openclawcn/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
