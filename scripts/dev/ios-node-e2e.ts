...
const urlRaw = getArg("--url") ?? process.env.OPENCLAWCN_GATEWAY_URL;
const token = getArg("--token") ?? process.env.OPENCLAWCN_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.OPENCLAWCN_RUN_DANGEROUS === "1";
...
if (!urlRaw || !token) {
  ...
  console.error(
    "Usage: bun scripts/dev/ios-node-e2e.ts --url <wss://host[:port]> --token <gateway.auth.token> [--node <id|name-substring>] [--dangerous] [--json]\n" +
      "Or set env: OPENCLAWCN_GATEWAY_URL / OPENCLAWCN_GATEWAY_TOKEN",
  );
  process.exit(1);
}
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "openclawcn ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "openclawcn-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "OpenClawCN E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
