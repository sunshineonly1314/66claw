...
ZIP=${1:?"Usage: $0 OpenClawCN-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/openclawcn/openclawcn/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OpenClawCN-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/openclawcn/openclawcn/releases/download/v${VERSION}/"}
