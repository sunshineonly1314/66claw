...
const SKILL_FRIENDLY_NAMES: Record<string, string> = {
  ...
  oracle: "多模型对比", gog: "搜索引擎", himalaya: "邮件管理",
  "1password": "密码管理", camsnap: "摄像头截图", "nano-pdf": "PDF 阅读",
  "openai-whisper": "语音转文字", "openai-image-gen": "AI 绘图",
  openhue: "智能灯光", goplaces: "附近搜索", gemini: "Gemini AI",
  obsidian: "笔记管理", mcporter: "MCP 工具", "apple-notes": "Apple 备忘录",
  "apple-reminders": "Apple 提醒事项", slack: "Slack 消息",
...
export function renderSkillsBatchConfirmEnhanced(props: {
  ...
  const handleConfirm = (e: Event) => {
    ...
    if (!modal) return;
    const boxes = modal.querySelectorAll<HTMLInputElement>("input[name='batch-skill']:checked:not([disabled])");
    ...
    <div data-batch-confirm class="modal-overlay">
      <div class="modal">
        ...
        <div class="categories-section">
          ...
          ${Array.from(categoryGroups.entries()).slice(0, 5).map(([cat, skills]) => html`
            <div class="category-row">
              <span class="cat-emoji">${CATEGORY_ICONS[cat] || "📦"}</span>
              <span class="cat-name">${cat === "productivity" ? "生产力工具" :
                                        cat === "development" ? "开发工具" :
                                        cat === "platform" ? "平台专属" :
                                        cat === "media" ? "音频 & 媒体" :
                                        cat === "iot" ? "智能硬件 & 服务" : cat}</span>
        ...
        <details class="skills-details">
          ...
          <div class="skills-list">
            ${Array.from(tierGroups.entries()).map(([tier, skills]) => html`
