                ...
                <div class="list" style="margin-top: 16px;">
                  ${entries.map((entry) => {
                    ...
                    const status = summary.total
                      ? t("agents.connectedCount", { connected: summary.connected, total: summary.total })
                      : t("agents.noAccounts");
                    const config = summary.configured
                      ? t("agents.configuredCount", { count: summary.configured })
                      : t("agents.notConfigured");
                    const enabled = summary.total ? t("agents.enabledCount", { count: summary.enabled }) : t("agents.channelDisabled");
