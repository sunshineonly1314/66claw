...
function renderContextPanel(
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t("usage.context.ofInput", { pct: String(Math.min((totalContextTokens / inputTokens) * 100, 100).toFixed(0)) });
    }
  }
    ...
    <div class="session-logs-compact">
      ...
      <div class="session-logs-list">
        ${filteredEntries.map((entry) => {
          ...
          const roleLabel =
            log.role === "user" ? t("usage.logs.roleYou") : log.role === "assistant" ? t("usage.logs.roleAssistant") : t("usage.logs.roleTool");
          return html`
