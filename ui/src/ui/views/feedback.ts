...
const feedbackIcons = {
  lightbulb: html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg>`,
  ...
  arrowRight: html`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>`,
...
function renderFeedbackForm(props: FeedbackViewProps) {
  const { state, onTypeChange, onContentChange, onContactChange, onAddAttachment, onSubmit } = props;
  ...
  return html`
    <div class="fb-sheet-form">
      ...
      ${state.error
        ? html`<div class="fb-error">${state.error}</div>`
        : nothing}
...
export function buildFeedbackPayload(
  ...
  return {
    ...
    attachments: state.attachments.length > 0
      ? state.attachments.map((a) => a.dataUrl)
      : undefined,
    context: {
      ...context,
