...
export function renderNetworkCenter(props: NetworkCenterProps): TemplateResult {
  return html`
    <div class="network-center" style="
...
function renderDevicesTab(props: NetworkCenterProps): TemplateResult {
  ...
  return html`
    ...
    ${!hasDevices && !props.presenceLoading
...
function renderNodeEntry(node: Record<string, unknown>): TemplateResult {
  ...
  const displayName = String(node.displayName ?? node.name ?? nodeId);
  const ip = String(node.remoteIp ?? node.ip ?? "");
  ...
  return html`
    <section class="card" style="margin-top: 16px; text-align: center; padding: 32px 24px;">
      ...
      <div style="text-align: left; max-width: 400px; margin: 0 auto; line-height: 2;">
        ...
        <div>2. ${t("network.devices.emptyState.step2")}</div>
      ...
      <details>
        ...
        <div style="padding: 12px 0; line-height: 2; font-size: 0.9em;">
          ...
          <div style="padding-left: 16px;">
            <div>1. ${t("network.devices.tutorial.mobileStep1")}</div>
          ...
          <div style="padding-left: 16px;">
            <div>1. ${t("network.devices.tutorial.desktopStep1")}</div>
...
function renderNetworkModeSelector(props: NetworkCenterProps): TemplateResult {
  ...
  const modes: Array<{ id: "loopback" | "lan" | "tailnet"; labelKey: string; descKey: string }> = [
    { id: "loopback", labelKey: "network.connection.mode.loopback", descKey: "network.connection.mode.loopbackDesc" },
    { id: "lan", labelKey: "network.connection.mode.lan", descKey: "network.connection.mode.lanDesc" },
    { id: "tailnet", labelKey: "network.connection.mode.tailnet", descKey: "network.connection.mode.tailnetDesc" },
  ];
              ...
              @click=${() => {
                if (m.id !== currentMode) {
                  if (m.id === "loopback") {
                    ...
                    props.onConfigure({ bind: m.id, generateAuthToken: !props.status?.hasAuthToken });
                  }
                }
              }}
            >
    ...
    <section class="card" style="margin-bottom: 16px;">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">${t("network.connection.discovery.desc")}</div>
        </div>
...
function renderDiscoveredGateway(gw: NetworkDiscoveredGateway, props: NetworkCenterProps): TemplateResult {
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title" style="display: flex; align-items: center; gap: 8px;">
      ...
      ? html`
          <div class="callout ${props.probeResult.reachable ? "" : "danger"}" style="margin: 4px 0 8px 28px; font-size: 0.88em;">
            ${props.probeResult.reachable
              ? t("network.connection.discovery.reachable").replace("{ms}", String(props.probeResult.latencyMs))
              : t("network.connection.discovery.unreachable").replace("{error}", props.probeResult.error ?? "")}
          </div>
        `
      : nothing}
  `;
...
function renderInterfacesSection(props: NetworkCenterProps): TemplateResult {
  ...
  if (ipv4.length === 0) return html`${nothing}`;
