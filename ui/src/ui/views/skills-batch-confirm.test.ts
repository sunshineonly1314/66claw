...
function createCheckResult(overrides: Partial<BatchCheckResult> = {}): BatchCheckResult {
  return {
    missing: [
      { name: "weather", icon: "🌤", category: "lifestyle", size_bytes: 20_000_000, method: "npm", tier: "core", description: "天气查询" },
      { name: "summarize", icon: "📝", category: "productivity", size_bytes: 20_000_000, method: "npm", tier: "core", description: "网页摘要" },
      { name: "gemini", icon: "✨", category: "creative", size_bytes: 20_000_000, method: "npm", tier: "recommended", description: "Gemini AI" },
      { name: "songsee", icon: "🎵", category: "lifestyle", size_bytes: 25_000_000, method: "download", tier: "recommended", description: "音乐识别" },
      { name: "gifgrep", icon: "🎬", category: "creative", size_bytes: 15_000_000, method: "npm", tier: "optional", description: "" },
    ],
...
describe("skills-batch-confirm", () => {
  it("renders three tier groups: core, recommended, optional", () => {
    ...
    render(renderSkillsBatchConfirm({
  ...
  it("confirm button sends only checked skills", () => {
    ...
    const confirmBtn = Array.from(container.querySelectorAll("button")).find(
      btn => btn.textContent?.includes("开始安装"),
    );
