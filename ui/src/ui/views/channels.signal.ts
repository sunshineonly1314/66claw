...
export function renderSignalCard(params: {
  ...
  const cardClasses = [
    ...
    signal?.lastError ? (isUnconfiguredError(signal.lastError) ? "channel-card--unconfigured" : "channel-card--error") : "",
  ].filter(Boolean).join(" ");

  // 自动展开：运行中 或 有真实错误
  const shouldOpen = signal?.running || (signal?.lastError != null && !isUnconfiguredError(signal.lastError));
