...
function tierToBadge(tier: ImageGenTierLevel): string {
	switch (tier) {
		case "gpu-hq": return "gold";
		case "gpu-fast": return "silver";
		case "cpu": return "bronze";
		case "api-only": return "disabled";
...
function tierLabel(tier: ImageGenTierLevel): string {
	switch (tier) {
		...
		case "gpu-fast": return "GPU 快速";
		case "cpu": return "CPU 模式";
		case "api-only": return "仅云端 API";
...
function badgeLabel(badge: string): string {
	switch (badge) {
		case "gold": return "最佳";
		case "silver": return "良好";
		case "bronze": return "可用";
		default: return "";
	}
}
		...
		<div style="border: 1px solid var(--border); border-radius: var(--radius-lg, 8px); margin-bottom: 16px; overflow: hidden;">
				...
				<div style="padding: 0 16px 12px; display: flex; gap: 16px; font-size: 13px;">
					...
					${status.sidecar ? html`
						<div style="display: flex; align-items: center; gap: 6px;">
							...
							<span>引擎:</span>
							<span style="color: var(--fg-muted);">
								${status.sidecar.status === "running" ? "运行中"
									: status.sidecar.status === "starting" ? "启动中..."
									: status.sidecar.status === "error" ? "错误"
									: "已停止"}
							</span>
						</div>
					` : nothing}
			...
			${hasLocalTier ? html`
				<div style="padding: 8px 16px 12px; display: flex; gap: 8px; flex-wrap: wrap;">
					<!-- Install button -->
					${status.installedModels.length === 0 ? html`
						<button
							style="
								padding: 6px 16px;
								border-radius: 6px;
					...
					<!-- Download size hint -->
					${status.installedModels.length === 0 && tier.model ? html`
					...
					${status.installedModels.length > 0 && status.sidecar ? html`
						${status.sidecar.status === "running" ? html`
							<button
