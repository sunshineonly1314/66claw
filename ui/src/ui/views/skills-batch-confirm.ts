...
function getTierMeta(): Record<string, { label: string; badge: string; badgeBg: string; badgeColor: string }> {
  return {
    core:        { label: t("batch.confirm.tierCore"), badge: t("batch.confirm.badgeCore"), badgeBg: "var(--ok-subtle)", badgeColor: "var(--ok)" },
    recommended: { label: t("batch.confirm.tierRecommended"), badge: t("batch.confirm.badgeRecommended"), badgeBg: "var(--accent-subtle)", badgeColor: "var(--accent)" },
    optional:    { label: t("batch.confirm.tierOptional"), badge: t("batch.confirm.badgeOptional"), badgeBg: "var(--secondary)", badgeColor: "var(--muted)" },
  };
}
...
const SKILL_FRIENDLY_NAMES: Record<string, string> = {
  ...
  taskleef: "任务看板", treeline: "Treeline 财务",
  trein: "荷兰铁路查询", vibetunnel: "VibeTunnel 终端",
  ynab: "YNAB 预算", confluence: "Confluence 文档",
  "linear-app": "Linear 项目管理", todoist: "Todoist 待办",
  airtable: "Airtable 数据表", asana: "Asana 项目管理",
  jira: "Jira 工单", "google-calendar": "Google 日历",
  "google-drive": "Google 云盘", "google-maps": "Google 地图",
  telegram: "Telegram 消息", "x-twitter": "X/Twitter 社交",
  reddit: "Reddit 论坛", hackernews: "Hacker News",
  "stock-market": "股票行情", coinmarketcap: "加密货币行情",
  "aws-cli": "AWS 命令行", "azure-cli": "Azure 命令行",
  "gcloud-cli": "GCloud 命令行", vercel: "Vercel 部署",
  netlify: "Netlify 部署", supabase: "Supabase 数据库",
  firebase: "Firebase 服务", stripe: "Stripe 支付",
  twilio: "Twilio 通信", sendgrid: "SendGrid 邮件",
  "youtube-dl": "YouTube 下载", "bilibili-dl": "B站下载",
  "screen-recorder": "录屏工具", "qr-code": "二维码工具",
  translator: "翻译工具", "text-to-image": "文字转图片",
  "image-to-text": "图片转文字", ocr: "OCR 识别",
  calculator: "计算器", "unit-converter": "单位换算",
  timer: "计时器", pomodoro: "番茄钟",
  "password-gen": "密码生成器", "lorem-ipsum": "占位文本生成",
};
...
function getDisplayName(skill: MissingSkill | InstalledSkill): string {
  return SKILL_FRIENDLY_NAMES[skill.name] || ("description" in skill ? skill.description : "") || skill.name;
}
...
function renderTierSection(tier: string, skills: MissingSkill[], installedInTier: InstalledSkill[]): TemplateResult {
    ...
    <div data-batch-confirm style="position:fixed;inset:0;z-index:9000;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.7);backdrop-filter:blur(6px);animation:batchOverlayIn 0.25s ease;">
      <div style="width:92%;max-width:640px;max-height:85vh;background:var(--card);border:1px solid var(--border);border-radius:18px;overflow:hidden;display:flex;flex-direction:column;animation:batchModalIn 0.35s cubic-bezier(0.34,1.3,0.64,1);">
        ...
        <div style="flex:1;overflow-y:auto;padding:10px 32px 18px;min-height:0;">
          ${allTiers.map((tier) => renderTierSection(
            tier,
            tierGroups.get(tier) ?? [],
            installedGroups.get(tier) ?? [],
          ))}
        </div>
