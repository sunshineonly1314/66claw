...
function renderFilterChips(
  ...
  const daysLabel = selectedDays.length === 1 ? selectedDays[0] : t("usage.chip.daysCount", { count: String(selectedDays.length) });
  const hoursLabel =
    selectedHours.length === 1 ? `${selectedHours[0]}:00` : t("usage.chip.hoursCount", { count: String(selectedHours.length) });
