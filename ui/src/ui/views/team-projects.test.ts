...
function createDetail(overrides: Partial<TeamProjectDetail["project"]> = {}): TeamProjectDetail {
  return {
    project: {
      ...
      memberIds: ["sup-1", "worker-a", "worker-b"],
      members: [
        { id: "sup-1", name: "Team Supervisor", role: "Team coordinator and message router", emoji: "🎯" },
        { id: "worker-a", name: "文案写手", role: "内容创作", emoji: "✍️", keywords: ["文案", "写作"] },
        { id: "worker-b", name: "配图助手", role: "图片配图", emoji: "🖼️", keywords: ["配图", "图片"] },
      ],
      version: 1,
      autoSupervisor: true,
...
describe("team-projects members panel", () => {
  ...
  it("worker cards have remove buttons", () => {
    ...
    const workerCards = Array.from(container.querySelectorAll(".project-member-card:not(.project-member-card--supervisor)"));
  ...
  it("renders correctly when health data is provided", () => {
    const health: TeamProjectHealthResult = {
      ...
      status: "active",
      members: [
        { agentId: "sup-1", state: "healthy", totalSuccesses: 10, totalFailures: 0, lastError: null, lastSuccessAt: null, lastFailureAt: null },
        { agentId: "worker-a", state: "degraded", totalSuccesses: 5, totalFailures: 2, lastError: "timeout", lastSuccessAt: null, lastFailureAt: null },
        { agentId: "worker-b", state: "healthy", totalSuccesses: 8, totalFailures: 1, lastError: null, lastSuccessAt: null, lastFailureAt: null },
      ],
    };
