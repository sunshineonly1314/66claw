...
function tierToBadge(tier: VoiceTierLevel): string {
	switch (tier) {
		case "gpu-full": return "gold";
		case "gpu-asr": return "silver";
		case "cpu-full": return "silver";
		case "cpu-asr": return "bronze";
...
function tierLabel(tier: VoiceTierLevel): string {
	switch (tier) {
		case "gpu-full": return t("voiceTier.tier.gpuFull");
		case "gpu-asr": return t("voiceTier.tier.gpuAsr");
		case "cpu-full": return t("voiceTier.tier.cpuFull");
		case "cpu-asr": return t("voiceTier.tier.cpuAsr");
		case "disabled": return t("voiceTier.tier.disabled");
	}
}

function badgeLabel(badge: string): string {
	switch (badge) {
		case "gold": return t("voiceTier.badge.gold");
		case "silver": return t("voiceTier.badge.silver");
		case "bronze": return t("voiceTier.badge.bronze");
		default: return "";
	}
}
...
export function renderVoiceTierCard(props: VoiceTierCardProps): TemplateResult {
	const { state, onInstall, onRedetect, onSidecarStart, onSidecarStop, onAsrProviderChange, onTtsProviderChange } = props;
	...
	const isInstalling = state.installProgress !== null &&
	...
	return html`
		<div style="border: 1px solid var(--border); border-radius: var(--radius-lg, 8px); margin-bottom: 16px; overflow: hidden;">
			...
			<!-- Install Progress -->
			${isInstalling && state.installProgress ? html`
				<div style="padding: 0 16px 12px;">
					...
					${state.installProgress.mirrorUsed ? html`
			...
			<!-- Install Error -->
			${state.installProgress?.stage === "failed" ? html`
			...
			${tier.tier !== "disabled" ? html`
				<div style="padding: 8px 16px 12px; display: flex; gap: 8px; flex-wrap: wrap;">
					<!-- Install button -->
					${status.installState !== "complete" ? html`
						<button
							style="
								padding: 6px 16px;
								border-radius: 6px;
					...
					<!-- Download size hint -->
					${status.installState !== "complete" && tier.asrModel ? html`
					...
					${(tier.tier === "gpu-full" || tier.tier === "gpu-asr") && status.installState === "complete" ? html`
						${status.gpuSidecar?.status === "running" ? html`
							<button
