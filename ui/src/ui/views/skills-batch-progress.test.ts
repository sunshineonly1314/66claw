...
describe("skills-batch-progress", () => {
  it("renders minimize button", () => {
    ...
    render(renderSkillsBatchProgress({
  ...
  it("calls onMinimize when minimize button is clicked", () => {
    ...
    const minimizeBtn = Array.from(container.querySelectorAll("button")).find(
      btn => btn.textContent?.includes("最小化"),
    );
  ...
  it("calls onCancel when cancel button is clicked", () => {
    ...
    const cancelBtn = Array.from(container.querySelectorAll("button")).find(
      btn => btn.textContent?.includes("取消安装"),
    );
  ...
  it("displays overall progress percentage", () => {
    ...
    render(renderSkillsBatchProgress({
      ...
      progress: { completed: 1, total: 2, bytesDownloaded: 1024, bytesTotal: 2048, speedBps: 512 },
      onCancel: vi.fn(),
      onMinimize: vi.fn(),
    }), container);
