        ...
        ? html`
            <div class="skill-install-deps-group">
              ...
              <div class="skill-install-deps-items">
                ${missing.bins.map(
                  (bin) => html`<span class="skill-install-dep-item skill-install-dep-bin">${bin}</span>`,
                )}
              </div>
            </div>
          `
      ...
      ${hasEnv
        ? html`
            <div class="skill-install-deps-group">
              ...
              <div class="skill-install-deps-items">
                ${missing.env.map(
                  (env) => html`<span class="skill-install-dep-item skill-install-dep-env">${env}</span>`,
                )}
              </div>
            </div>
          `
      ...
      ${hasConfig
        ? html`
            <div class="skill-install-deps-group">
              ...
              <div class="skill-install-deps-items">
                ${missing.config.map(
                  (cfg) => html`<span class="skill-install-dep-item skill-install-dep-config">${cfg}</span>`,
                )}
              </div>
            </div>
          `
...
export function renderSkillInstallApproval(state: AppViewState) {
  ...
  const remaining = remainingMs > 0 ? `${t("skillInstall.expiresIn")} ${formatRemaining(remainingMs)}` : t("skillInstall.expired");
