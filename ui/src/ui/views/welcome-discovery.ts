...
function renderScanning(progress: number) {
  ...
  const currentPhase = scanPhases.find((p) => progress <= p.threshold) ?? scanPhases[scanPhases.length - 1];
...
function renderDone(props: WelcomeDiscoveryProps) {
  const { capabilities, workspace, suggestions, detectTimeMs, onSuggestionClick, onSkip, onConfigClick } = props;
      ...
      ${hasNeedsConfigCapabilities
        ? html`
            <div class="welcome-discovery__section welcome-discovery__section--needs-config">
              ...
              <div class="welcome-discovery__capabilities">
                ${needsConfigCapabilities.map((cap) =>
                  renderCapabilityCard(cap, onConfigClick),
                )}
              </div>
            </div>
          `
