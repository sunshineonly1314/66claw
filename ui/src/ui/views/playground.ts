...
const CN_DEPRIORITIZED_SKILLS = new Set([
  // ── 依赖 Google API（被墙） ──
  "gemini", "nano-banana-pro", "gog", "goplaces", "local-places",
  "google-search", "google-calendar", "google-drive", "google-docs",
  "nest-devices", "research",
  // ── 依赖 OpenAI API（被墙） ──
  "oracle", "openai-image-gen", "openai-whisper-api", "summarize", "coding-agent",
  "openai-chat", "openai-tts", "gpt", "sora-video-gen",
  // ── 依赖海外社交平台 ──
  "slack", "discord", "wacli", "bluebubbles",
  "trello", "linear", "notion",
  "messenger", "pinterest", "signal-cli",
  // ── 依赖海外媒体/服务 ──
  "sag", "spotify-player", "spotify-applescript", "voice-call", "gifgrep",
  "food-order", "ordercli", "bird", "eightctl",
  "youtube-search", "youtube-summarizer",
  // ── 依赖海外金融/支付 ──
  "monarch-money", "plaid", "stripe", "paypal", "moonpay", "transak",
  // ── 其他海外专属 ──
  "comfy", "homekit", "substack-formatter", "typefully",
  "gmail-client", "gmail-manager",
]);
...
const CN_NATIVE_SKILLS = new Set([
  // ── 国内搜索 ──
  "zhipu-web-search", "baidu-search", "baidu-scholar-search", "aliyun-search",
  // ── 国内社交 ──
  "wechat", "wecom", "zhihu", "xiaohongshu", "weread",
  // ── 国内智能家居 ──
  "mijia", "midea-ac",
  // ── 国内 AI ──
  "qwen-image", "qwen-tts", "minimax-tts", "seedream-image-gen",
  // ── 飞书/钉钉 ──
  "feishu-memory-recall", "dingtalk",
  // ── 本地离线工具（中国可用） ──
  "edge-tts", "sherpa-onnx-tts", "openai-whisper", "ollama-local",
  // ── 天工系列 ──
  "tiangong-notebooklm", "tiangong-ppt", "tiangong-word",
]);
...
const SKILL_CATEGORY_RULES: Array<{ keywords: string[]; category: string }> = [
  ...
  {
    keywords: [
      "stock", "finance", "budget", "accounting", "investment", "crypto",
      "banking", "hyperliquid", "trading", "sec-filing", "monarch",
      "a-stock", "coin", "wallet", "ledger", "portfolio",
      // 加密货币 (§7.11)
      "solana", "onchain", "okx", "moonpay", "transak", "ethereum",
      "blockchain", "defi",
      // 预测市场 (§7.11)
      "polymarket", "pm-odds",
      // 传统金融 (§7.11)
  ...
  {
    keywords: [
      "spotify", "songsee", "music", "game", "canvas", "gifgrep",
      "video-frames", "podcast", "media-player", "sonoscli",
      "entertainment", "fun", "play", "trivia", "quiz",
      // 音乐控制 (§7.9)
      "blucli", "roon-controller",
      // 游戏 (§7.18)
      "steam", "pokemon", "sudoku", "riddle", "magic-8-ball",
      "xkcd", "strands", "mtg-edh", "spacemolt", "moltbot-arena",
      // 多媒体 (§7.9)
      "media-converter", "sound-fx", "subtitles", "video-subtitles",
      "svg-draw", "vhs-recorder", "remotion",
    ],
  ...
  {
    keywords: [
      "1password", "obsidian", "apple-notes", "apple-reminders",
      "things-mac", "bear-notes", "himalaya", "calendar", "todo",
      "4todo", "task", "notion", "trello", "linear",
      "imap-email", "daily-recap", "session-logs",
      "meeting", "schedule", "agenda", "note",
      // 任务管理 (§7.8)
      "todoist", "omnifocus", "ticktick", "microsoft-todo",
      "taskmaster", "no-nonsense-tasks",
      // 笔记与知识库 (§7.8)
      "logseq", "workflowy", "obsidian-daily",
      // 邮件 (§7.7)
      "fastmail", "imap-smtp", "send-email", "email-send",
      "apple-mail", "mailgun",
      // 日历 (§7.8)
      "brainz-calendar", "email-to-calendar", "ms365",
      // 文档处理 (§7.8)
      "nano-pdf", "pdf", "md-2-pdf", "xlsx", "markdown-converter",
      // 微信读书 (CN)
      "weread",
    ],
    category: "work",
  ...
  {
    keywords: [
      "weather", "local-places", "food-order", "goplaces", "openhue",
      "ordercli", "plan2meal", "recipe", "fitness", "health",
      "smart-home", "homekit", "bambu", "sports-ticker", "flight",
      "pollen",
      // 智能家居 (§7.10)
      "homeassistant", "switchbot", "nanoleaf", "wled", "nest-devices",
      "samsung-smartthings", "tado", "sensibo", "midea-ac", "mijia",
      "mqtt-client", "unifi", "pihole",
      // 健康 (§7.15)
      "whoop", "oura", "strava", "withings", "workout",
      "weight-loss", "muscle-gain", "morning-routine", "night-routine",
      "mindfulness", "meditation", "sleep", "stress",
      "quit-smoking", "quit-vaping", "quit-alcohol",
      // 出行 (§7.15)
      "travel-concierge", "travel-manager", "surfline",
      "swiss-transport", "uk-trains", "ns-trains",
    ],
    category: "lifestyle",
  },
  ...
  {
    keywords: [
      "openai-image", "nano-banana", "comfyui", "image-gen",
      "summarize", "gemini", "oracle", "edge-tts",
      "sherpa-onnx", "openai-whisper", "pptx", "marketing",
      "elevenlabs", "tts", "translate", "draw", "design",
      // AI 图像 (§7.1)
      "nvidia-image-gen", "seedream", "qwen-image", "fal-ai", "falai",
      "pollinations", "ai-picture-book",
      // AI 语音 (§7.1)
      "piper-tts", "mac-tts", "sapi-tts", "minimax-tts", "qwen-tts",
      "mlx-stt", "mlx-whisper", "parakeet", "voice-transcribe",
      "transcribe", "transcribee",
      // 内容创作 (§7.16)
      "blogwatcher", "newsletter", "social-content", "social-post",
      "seo-audit", "seo-optimizer", "marketing-ideas", "marketing-mode",
      "podcast-generation", "tiangong",
      // 教育 (§7.17)
      "academic-deep-research", "overleaf", "study-habits", "readwise",
      "agentarxiv", "tldr", "tldw",
    ],
    category: "creative",
  },
  // 💬 通讯社交 — 微信、钉钉、飞书、Telegram、Discord (~50+ skills)
  {
    keywords: [
      ...
      "voice-call", "telegram", "wechat", "dingtalk", "feishu",
      "qqbot", "message", "chat", "sms",
      // 国内平台 (§4 CN 特色)
      "wecom", "zhihu", "xiaohongshu",
      // 海外社交 (§7.7)
      "bluesky", "reddit", "hackernews", "twitter",
      "mastodon", "linkedin", "pinchedin", "tiktok",
      // 邮件 (只放 Gmail 等社交性邮件)
      "gmail",
      // 聊天平台 (§7.7)
      "beeper", "signal", "messenger",
    ],
  ...
  // 💻 开发工具 — GitHub、代码、调试、搜索引擎、Agent、数据库 (~200+ skills)
  {
    keywords: [
      ...
      "model-usage", "relay",
      // Git (§7.2)
      "gitlab", "gitea", "gitflow", "git-essentials", "git-summary",
      // API 开发 (§7.2)
      "api-dev", "test-patterns", "test-runner", "openapi", "openspec",
      // 代码质量 (§7.2)
      "pr-reviewer", "perf-profiler", "log-analyzer", "sentry",
      "ripgrep",
      // 搜索引擎 (§7.4)
      "tavily", "exa-search", "serpapi", "serper", "searxng",
      "duckduckgo", "ddg-search", "kagi",
      "zhipu-web-search", "baidu-search", "baidu-scholar", "aliyun-search",
      // 浏览器自动化 (§7.3)
      "clawbrowser", "fast-browser", "playwright", "puppeteer",
      "stagehand", "browser",
      // Agent 框架 (§7.5)
      "agent-builder", "agent-orchestrat", "claw-swarm",
      "agent-protocol", "agent-memory", "agentmemory",
      // 记忆系统 (§7.6)
      "memory", "vector-memory", "chromadb", "lancedb",
      "sequential-thinking",
      // 数据库 (§7.13)
      "postgres", "database", "sqlite", "redis", "mongodb",
      "nocodb", "duckdb", "supabase", "snowflake",
      // DevOps (§7.14)
      "kubernetes", "n8n", "portainer", "proxmox",
      "pm2", "vercel", "netlify", "railway",
      "nginx", "uptime-kuma", "digital-ocean",
      // 编程语言专用 (§7.2)
      "python", "nextjs", "trpc", "swift", "php",
      "noir-developer",
      // MCP/技能 (§7.2)
      "mcp-builder", "skill-scaffold", "skill-evaluator",
      // 深度研究 (§7.1)
  ...
  {
    keywords: [
      "peekaboo", "camsnap", "screenshot", "filesystem", "backup",
      "simple-backup", "packaging", "eightctl",
      "clawdbot-skill-update", "clawdlink",
      "anyone-proxy", "verify-on-browser",
      // 安全 (§7.12)
      "security", "clawsec", "clawguard", "clawscan",
      "praesidia", "virustotal", "nmap",
      "nordvpn", "tailscale", "openssl", "proton-pass",
      // 桌面控制 (§7.14)
      "desktop-commander", "desktop-control",
      "windows-control", "everything-search",
      // 文件管理
      "clawdbot-filesystem", "file-search", "dropbox",
      // 系统工具
      "mactop", "unraid",
    ],
    category: "system",
...
function getSkillScore(skill: SkillStatusEntry): number {
  ...
  const hasMissingDeps = skill.missing.bins.length > 0 || skill.missing.env.length > 0 || skill.missing.config.length > 0;
...
function organizeSkillsByCategory(sortedSkills: SkillStatusEntry[]): Map<string, SkillStatusEntry[]> {
...
const INSTALL_MESSAGE_RULES: Array<{ pattern: RegExp; transform: (msg: string) => string }> = [
  // 0. 已是中文/CN 专属 — 直接透传
  { pattern: /ClawdbotCN|🇨🇳|安装成功|安装失败|正在安装/, transform: (m) => m },
  // 1. 正在安装
  { pattern: /(?:正在.*安装|Installing).*go/i, transform: () => "🚀 正在为您安装 Go 语言运行时..." },
  { pattern: /(?:正在.*安装|Installing).*node/i, transform: () => "🚀 正在为您安装 Node.js 运行时..." },
  { pattern: /(?:正在.*安装|Installing).*uv/i, transform: () => "🚀 正在为您安装 Python 包管理器 (uv)..." },
  // 2. 未安装错误（必须在「安装成功」之前匹配，否则 "not installed" 包含 "installed"）
  { pattern: /go not installed/i, transform: () => `❌ ${t("playground.goNotInstalled")}（请访问 https://go.dev/dl/ 下载安装）` },
  { pattern: /node(?:js)? not installed|npm not installed/i, transform: () => `❌ ${t("playground.nodeNotInstalled")}（请访问 https://nodejs.org/ 下载安装）` },
  { pattern: /(?:home)?brew not installed/i, transform: () => `❌ ${t("playground.brewNotInstalled")}（请访问 https://brew.sh/ 安装）` },
  { pattern: /uv not installed/i, transform: () => `❌ ${t("playground.uvNotInstalled")}（请访问 https://docs.astral.sh/uv/ 了解安装方法）` },
  // 3. 安装失败
  { pattern: /install.*fail|failed to install/i, transform: (m) => {
...
function getProcessedSkills(report: SkillStatusReport | null) {
  if (report === _cachedReportRef) {
    return { sorted: _cachedSorted, byCategory: _cachedByCategory, availableCount: _cachedAvailableCount };
  }
...
export function renderPlayground(props: PlaygroundProps) {
  const { sorted: sortedSkills, byCategory: skillsByCategory, availableCount } = getProcessedSkills(props.report);
