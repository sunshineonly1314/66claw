  ...
  return html`
    ...
    >
      <button
        ...
        @click=${() => {
          startRename(_contextMenuKey!, title);
          requestUpdate();
        }}
      >
        ${icons.edit} ${t("sidebar.rename")}
      </button>
      <button
        class="conv-sidebar__context-menu-item"
        @click=${() => {
      ...
      <button
        class="conv-sidebar__context-menu-item"
        @click=${() => {
          props.onPinSession(_contextMenuKey!, !isPinned);
          hideContextMenu();
          requestUpdate();
        }}
      >
        ${icons.pin} ${isPinned ? t("sidebar.unpin") : t("sidebar.pin")}
      </button>
      ...
      <button
        ...
        @click=${() => {
          if (confirm(t("sidebar.deleteConfirm"))) {
            props.onDeleteSession(_contextMenuKey!);
          }
          hideContextMenu();
          requestUpdate();
        }}
      ...
      <div class="conv-sidebar__context-menu-sep"></div>
      <button
        class="conv-sidebar__context-menu-item conv-sidebar__context-menu-item--subtle"
        @click=${() => {
          _contextMenuShowMore = !_contextMenuShowMore;
          requestUpdate();
        }}
      >
        ${icons.moreHorizontal} ${t("sidebar.more")}
      </button>
      ${_contextMenuShowMore ? html`
        <button
          class="conv-sidebar__context-menu-item conv-sidebar__context-menu-item--sub"
          @click=${() => {
            props.onArchiveSession(_contextMenuKey!);
            hideContextMenu();
            requestUpdate();
          }}
        >
          ${icons.archive} ${t("sidebar.archive")}
        </button>
        <button
          class="conv-sidebar__context-menu-item conv-sidebar__context-menu-item--sub"
          @click=${() => {
...
function renderAssetsTab(
  ...
  const filtered = _assetFilter === "all"
    ? assets
    : assets.filter((a) => a.type === _assetFilter);
    ...
    <div class="conv-sidebar ${!props.open ? "conv-sidebar--collapsed" : ""}">
      ...
      <div class="conv-sidebar__tabs">
        <button
          ...
          @click=${() => { _activeTab = "conversations"; requestUpdate(); }}
        >
          ${icons.messageSquare}
          <span>对话</span>
        </button>
        <button
          ...
          @click=${() => { _activeTab = "assets"; props.onAssetsTabActivated?.(); requestUpdate(); }}
        >
          ${icons.image}
          <span>资源</span>
        </button>
