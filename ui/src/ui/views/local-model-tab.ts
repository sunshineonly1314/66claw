	...
	return html`
		<div class="le-device-bar">
			...
			<div class="le-device-bar__hw">
				...
				${hw.platform === "win32" ? html`<span class="le-device-bar__chip">Windows</span>` : nothing}
				${hw.platform === "darwin" ? html`<span class="le-device-bar__chip">macOS</span>` : nothing}
				${hw.platform === "linux" ? html`<span class="le-device-bar__chip">Linux</span>` : nothing}
			</div>
			<div class="le-device-bar__stats">
				...
				${hasRecommended ? html`<button class="le-device-bar__btn le-device-bar__btn--rec" @click=${(e: Event) => { e.stopPropagation(); actions.onInstallRecommended(); }}>一键安装推荐</button>` : nothing}
...
export function renderLocalModelTab(
	...
	if (!status) {
		return html`<div class="le-tab-empty">加载本地模型信息中...</div>`;
	}

	const models = status.models[capGroupId];
	if (!models || models.length === 0) {
		return html`<div class="le-tab-empty">该能力暂无可用的本地模型</div>`;
	}
			...
			${hasMultiSubs
				? subCaps.map(sc => html`
					<div class="le-subcap-group">
						...
						${models.filter(m => m.capability === sc).map(m =>
							renderModelRow(m, progress[m.id], actions)
						)}
					</div>
				`)
		...
		<div class="le-model-row le-model-row--${statusClass(effectiveStatus)} ${model.recommended ? 'le-model-row--recommended' : ''}">
			<div class="le-model-row__main">
				<div class="le-model-row__header">
					...
					${model.recommended ? html`<span class="le-badge-rec">推荐</span>` : nothing}
				</div>
...
function renderModelAction(
	...
	switch (status) {
		...
		case "installable":
			return html`<button class="le-btn le-btn--install" @click=${(e: Event) => { e.stopPropagation(); actions.onInstall(model.id); }}>安装</button>`;
		...
		case "installed": {
			const domain = model.capability === "audio" || model.capability === "tts" ? "voice" as const : "imagegen" as const;
			const canStart = model.runMode === "gpu";
			return html`
				<div class="le-installed-actions">
					${canStart ? html`<button class="le-btn le-btn--start" @click=${(e: Event) => { e.stopPropagation(); actions.onStartSidecar(domain); }}>启动</button>` : nothing}
					<button class="le-btn le-btn--uninstall" @click=${(e: Event) => { e.stopPropagation(); actions.onUninstall(model.id); }}>卸载</button>
				</div>
			`;
		}
		...
		case "running": {
			...
			return html`
				<div class="le-running-actions">
					...
					<button class="le-btn le-btn--stop" @click=${(e: Event) => { e.stopPropagation(); actions.onStopSidecar(domain); }}>停止</button>
				</div>
			`;
		}
...
function renderRecommendedBanner(
	...
	return html`
		<div class="le-rec-banner">
			...
			<button class="le-btn le-btn--rec" @click=${(e: Event) => { e.stopPropagation(); actions.onInstallRecommended(); }}>
				一键安装推荐
			</button>
		</div>
	`;
...
export function renderLocalManageModal(
	...
	return html`
		<div class="modal-overlay" @click=${actions.onClose} @keydown=${(e: KeyboardEvent) => { if (e.key === "Escape") actions.onClose(); }}>
			<div class="modal le-manage-modal" @click=${(e: Event) => e.stopPropagation()}>
				...
				<div class="modal-body le-manage-body">
					...
					<!-- All models by group -->
					${Object.entries(status.models).map(([groupId, models]) => html`
...
function tierLabel(tier: string): string {
	switch (tier) {
		case "gpu-full": return "GPU 全功能";
		case "gpu-asr": return "GPU 语音";
		case "cpu-full": return "CPU 全功能";
		case "cpu-asr": return "CPU 语音";
		case "gpu-hq": return "GPU 高质量";
		case "gpu-fast": return "GPU 极速";
		case "cpu": return "CPU";
		case "api-only": return "仅 API";
