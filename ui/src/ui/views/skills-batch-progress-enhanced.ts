...
const SKILL_FRIENDLY_NAMES: Record<string, string> = {
  ...
  oracle: "多模型对比", himalaya: "邮件管理", openhue: "智能灯光",
  "openai-whisper": "语音转文字", mcporter: "MCP 工具",
  // 更多...
};
...
export function renderSkillsBatchProgressEnhanced(props: {
  ...
  const getStatusClass = (status: SkillBatchItem["status"]) => {
    switch (status) {
      ...
      case "downloading": return "task-active";
      case "retrying": return "task-retrying";
      case "verifying": return "task-verifying";
      case "failed": return "task-failed";
      default: return "task-queued";
    }
  };
  ...
  const getTaskDetail = (skill: SkillBatchItem) => {
    ...
    return skill.status === "done" ? "已完成" :
           skill.status === "queued" ? "等待中" :
           skill.status === "verifying" ? "验证中" : "";
  };
