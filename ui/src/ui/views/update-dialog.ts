...
function renderConfirmState(props: UpdateDialogProps): TemplateResult {
  ...
  const typeKey =
    info.updateType === "delta" ? "update.dialog.typeDelta"
    : info.updateType === "full" ? "update.dialog.typeFull"
  ...
  return html`
    <div style="padding: 0 24px 16px;">
      ...
      <div style="display: flex; gap: 6px; margin-bottom: 12px;">
        ...
        ">${t(typeKey)}</span>
        ${info.mandatory ? html`<span style="
          font-size: 11px; padding: 2px 8px; border-radius: 4px;
          background: #ef4444; color: #fff;
        ">${t("update.dialog.mandatory")}</span>` : nothing}
      </div>
      ${changelog ? html`
        <div style="
      ...
      <div style="display: flex; justify-content: flex-end; gap: 8px;">
        ...
        ` : nothing}
        ${info.updateType === "installer" ? html`
...
export function renderUpdateDialog(props: UpdateDialogProps | null): TemplateResult | typeof nothing {
