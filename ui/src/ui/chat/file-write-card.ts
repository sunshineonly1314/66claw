...
export function extractFileWriteDetails(message: unknown): FileWriteDetails | null {
  ...
  if (details?.path && (details.status === "success" || details.status === "created" || details.status === "updated")) {
  ...
  if (Array.isArray(content)) {
    ...
    if (isFileWrite) {
      ...
      if (textBlock?.text) {
        ...
        if (text.includes("successfully") || text.includes("written") || text.includes("saved") || text.includes("created")) {
...
export function renderFileWriteResult(details: FileWriteDetails): TemplateResult {
  ...
  const preview = details.content
    ? truncatePreview(details.content, 5)
    : undefined;
  ...
  return html`
    <div class="media-gen-progress">
      ...
      <div class="media-gen-progress__body">
        ...
        ${filename
          ? html`<div class="media-gen-progress__prompt">${filename}</div>`
          : nothing}
      </div>
    </div>
  `;
...
function buildFileDetails(data: Record<string, unknown>): FileWriteDetails {
  ...
  return {
    ...
    size: typeof data.size === "number" ? data.size : (typeof data.content === "string" ? new Blob([data.content]).size : undefined),
    language: detectLanguage(filePath),
...
const LANG_MAP: Record<string, string> = {
  ...
  py: "python", md: "markdown", json: "json", csv: "csv",
