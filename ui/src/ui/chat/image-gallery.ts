...
export function renderImageGallery(props: ImageGalleryProps): TemplateResult {
  ...
  return html`
    <div class="image-gallery-overlay" role="dialog" aria-modal="true" aria-label="Image gallery" @click=${(e: Event) => {
        ...
        ${images.length === 0
          ? html`
              <div class="image-gallery__empty">
                <svg viewBox="0 0 24 24" width="40" height="40" stroke="currentColor" fill="none" stroke-width="1.5">
...
export function closeImageGallery(): void {
  if (_galleryElement) {
    ...
    const handler = (_galleryElement as unknown as { _escHandler?: (e: KeyboardEvent) => void })._escHandler;
