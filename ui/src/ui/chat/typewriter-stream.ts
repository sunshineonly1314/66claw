...
const MAX_BACKLOG = 800;

/** 积压处理的目标时间（秒）- 积压的文本应在此时间内追上 */
const TARGET_CATCHUP_TIME = 2.5;
...
class TypewriterStreamDirective extends AsyncDirective {
  ...
  override update(part: import("lit/directive").Part, [fullText, streamKey]: [string, string]) {
    ...
    // 检测新增文本量
    const newChars = this.fullText.length - this.lastTextLength;
    ...
    const run = () => {
      ...
      // 平滑过渡到目标速度
      this.currentSpeed = this.currentSpeed + (targetSpeed - this.currentSpeed) * SPEED_SMOOTHING;
    ...
    if (!this.rafId && this.revealedLength < this.fullText.length) {
  ...
  private calculateTargetSpeed(): number {
    ...
    const ratio = Math.min(excess / (MAX_BACKLOG - CATCHUP_THRESHOLD / 2), 1);
    
    // 使用 easeOutQuad 曲线使加速更自然
    const eased = 1 - (1 - ratio) * (1 - ratio);
    
    // 在基础速度和最大速度之间插值
    const speed = BASE_CHARS_PER_SECOND + (MAX_CHARS_PER_SECOND - BASE_CHARS_PER_SECOND) * eased;
