  ...
  const openMenu = (triggerBtn: HTMLElement) => {
    ...
    for (const tool of TOOL_SHORTCUTS) {
      ...
      item.innerHTML = `<span class="cc-tools-menu__icon">${
        tool.icon === "penLine" ? '<svg viewBox="0 0 24 24"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>'
        : tool.icon === "grid" ? '<svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>'
        : tool.icon === "layers" ? '<svg viewBox="0 0 24 24"><path d="m12.83 2.18a2 2 0 0 0-1.66 0L2.6 6.08a1 1 0 0 0 0 1.83l8.58 3.91a2 2 0 0 0 1.66 0l8.58-3.9a1 1 0 0 0 0-1.83Z"/><path d="m22 12.5-8.58 3.9a2 2 0 0 1-1.66 0L2.6 12.5"/><path d="m22 17-8.58 3.91a2 2 0 0 1-1.66 0L2.6 17"/></svg>'
        : tool.icon === "image" ? '<svg viewBox="0 0 24 24"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>'
            ...
            : props.voiceRecording || props.voiceProcessing
              ? html`
                  <button
                    ...
                    @click=${() => props.onVoiceToggle?.({ autoSend: true })}
