  ...
  return {
    ...
    request: {
      ...
      installSteps: Array.isArray(request.installSteps)
        ...
        : null,
      estimatedTime:
        typeof request.estimatedTime === "string" ? request.estimatedTime : null,
      originalMessage:
        typeof request.originalMessage === "string" ? request.originalMessage : null,
      sessionKey:
  ...
  return {
    ...
    decision:
      typeof payload.decision === "string"
        ? (payload.decision as SkillInstallDecision)
        : null,
...
export function parseSkillInstallProgress(
  ...
  if (payloadDownloadInfo && isRecord(payloadDownloadInfo)) {
    downloadInfo = {
      ...
      downloaded: typeof payloadDownloadInfo.downloaded === "string" ? payloadDownloadInfo.downloaded : undefined,
      total: typeof payloadDownloadInfo.total === "string" ? payloadDownloadInfo.total : undefined,
    };
  }

  // ClawdbotCN 专属：解析国内镜像标识
  const usingCNMirror = typeof (payload as Record<string, unknown>).usingCNMirror === "boolean"
    ? (payload as Record<string, boolean>).usingCNMirror
    : existingProgress?.usingCNMirror;

  // ClawdbotCN 专属：解析当前依赖名称
  const currentDependency = typeof (payload as Record<string, unknown>).currentDependency === "string"
 ...
 * 清理过期的安装请求
 */
export function pruneSkillInstallQueue(
...
/**
 * 创建简单进度状态（用于直接安装场景）
 * ClawdbotCN 专属：用于 playground 等直接安装场景
 */
