...
/** 创建连接的 host */
function connectedHost(overrides?: Partial<ModelConfigState>, clientResponses?: Record<string, unknown>) {
  return {
    ...createInitialModelConfigState(),
...
describe("model-config Controller", () => {
  ...
  describe("toggleProviderGroup", () => {
    it("应该切换指定分组的展开状态", () => {
      const host = connectedHost({
        providerGroups: [
          { id: "cn", name: "国内", description: "", icon: "🇨🇳", defaultExpanded: true, order: 1, expanded: true },
          { id: "intl", name: "国际", description: "", icon: "🌍", defaultExpanded: false, order: 2, expanded: false },
        ],
      });
  ...
  describe("closeModelSelector", () => {
    it("应该重置所有模型选择器状态", () => {
      const host = connectedHost({
        ...
        modelSelectorModels: [{ providerId: "openai", providerName: "OpenAI", providerIcon: "", modelId: "gpt-4o", modelName: "GPT-4o", pricing: { type: "paid" as const }, configured: true, active: true }],
