 ...
 */
export async function redetectHardware(
	client: GatewayBrowserClient,
): Promise<unknown> {
...
export async function installModel(
	...
	return await client.request<{ started: boolean; modelId: string }>(
...
export async function installRecommended(
	...
	return await client.request<{ started: boolean }>(
		"local_engine.install_recommended",
	);
}
...
export async function uninstallModel(
	...
	return await client.request<{ modelId: string }>(
		"local_engine.uninstall",
		{ modelId },
	);
}
...
export function subCapLabel(capability: string): string {
	switch (capability) {
		case "audio": return "语音识别 (ASR)";
		case "tts": return "语音合成 (TTS)";
