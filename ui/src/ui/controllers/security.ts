...
export async function loadSecurityModes(state: SecurityState): Promise<void> {
  console.log("[security] loadSecurityModes called:", { hasClient: !!state.client, connected: state.connected });
...
export async function setSecurityMode(
  ...
  console.log("[security] setSecurityMode called:", { mode, confirmed, hasClient: !!state.client, connected: state.connected });
  
  if (!state.client || !state.connected) {
    console.warn("[security] Cannot set security mode: not connected", { hasClient: !!state.client, connected: state.connected });
    return { ok: false };
  }
 ...
 * 获取安全模式显示名称
 */
export function getSecurityModeName(
  modes: SecurityModeInfo[],
  modeId: SecurityMode,
): string {
 ...
 * 获取安全模式图标
 */
export function getSecurityModeIcon(
  modes: SecurityModeInfo[],
  modeId: SecurityMode,
): string {
  const mode = modes.find((m) => m.id === modeId);
 ...
 * 判断模式是否危险（需要确认）
 */
export function isSecurityModeDangerous(
  modes: SecurityModeInfo[],
  modeId: SecurityMode,
): boolean {
  const mode = modes.find((m) => m.id === modeId);
