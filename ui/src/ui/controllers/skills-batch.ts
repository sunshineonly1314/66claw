...
export type SkillsBatchPhase = "idle" | "banner" | "confirm" | "downloading" | "result" | "complete";

export type SkillBatchItem = {
  name: string; icon: string;
  status: "queued" | "downloading" | "retrying" | "verifying" | "done" | "failed";
  progress?: number; bytesDownloaded?: number; bytesTotal?: number;
  detail?: string; mirror?: string; retryMirror?: string; error?: string;
};

export type BatchProgress = {
  completed: number; total: number; bytesDownloaded: number; bytesTotal: number;
  speedBps: number; activeMirror?: string; activeMirrorLatency?: number;
};

export type FailedSkillItem = {
  name: string; icon: string; error: string;
  mirrorsTried: { name: string; error: string }[];
};

export type BatchCheckResult = {
  missing: {
    name: string; icon: string; category: string; size_bytes: number; method: string;
    tier: "core" | "recommended" | "optional"; description: string;
  }[];
  installed?: { name: string; icon: string; tier: string }[];
  total_size_bytes: number; estimated_seconds: number;
  disk_available_bytes: number; disk_ok: boolean;
};

export type SkillsBatchState = {
  batchPhase: SkillsBatchPhase; batchId: string | null;
  batchSkills: SkillBatchItem[]; batchProgress: BatchProgress;
  batchResult: { succeeded: string[]; failed: FailedSkillItem[]; durationMs: number } | null;
  batchCheckResult: BatchCheckResult | null; reportSent: boolean;
  batchMinimized: boolean;
};

export function createDefaultBatchState(): SkillsBatchState {
  return {
    batchPhase: "idle", batchId: null, batchSkills: [],
    batchProgress: { completed: 0, total: 0, bytesDownloaded: 0, bytesTotal: 0, speedBps: 0 },
    batchResult: null, batchCheckResult: null, reportSent: false, batchMinimized: false,
  };
}
...
export async function checkBatchSkills(
  ...
  try {
    const res = (await state.client.request("skills.batch.check", {})) as BatchCheckResult | undefined;
    if (res && res.missing && res.missing.length > 0) {
      state.batchCheckResult = res; state.batchPhase = "banner";
    } else {
      state.batchCheckResult = null; state.batchPhase = "idle";
    }
  } catch (err) {
...
export async function startBatchInstall(
  ...
  state.batchPhase = "downloading"; state.reportSent = false; state.batchResult = null;
  ...
  state.batchProgress = {
    completed: 0, total: skillNames.length, bytesDownloaded: 0,
    bytesTotal: state.batchCheckResult?.total_size_bytes ?? 0, speedBps: 0,
  };
  try {
    // Method name: "skills.batch.install" (matches backend handler)
    const res = (await state.client.request("skills.batch.install", { skills: skillNames })) as { batch_id?: string } | undefined;
    if (res?.batch_id) state.batchId = res.batch_id;
  } catch (err) {
    console.error("[skills-batch] start failed:", getErrorMessage(err));
    state.batchResult = {
      succeeded: [],
      failed: skillNames.map((n) => ({ name: n, icon: "", error: getErrorMessage(err), mirrorsTried: [] })),
      durationMs: 0,
    };
...
export async function cancelBatchInstall(
  ...
  state.batchPhase = "idle"; state.batchId = null;
}
...
export function handleBatchEvent(
  ...
  if (wsEvent === "progress") {
    ...
    if (type === "skill.progress") {
      ...
      state.batchProgress = {
        ...
        activeMirrorLatency: (event.active_mirror_latency_ms as number) ?? state.batchProgress.activeMirrorLatency,
      };
    }
  ...
  if (wsEvent === "complete") {
    ...
    const rawFailed = (event.failed as Array<{ skill: string; error: string; mirrors_tried: string[] }>) ?? [];
