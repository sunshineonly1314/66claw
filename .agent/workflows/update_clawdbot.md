---
description: Update OpenClawCN from upstream when branch has diverged (ahead/behind)
---

# OpenClawCN Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "OpenClawCN" || true

# Move old version
mv /Applications/OpenClawCN.app /tmp/OpenClawCN-backup.app

# Install new build
cp -R dist/OpenClawCN.app /Applications/
...
pnpm openclawcn health

# Verify no zombie processes
ps aux | grep -E "(openclawcn|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm openclawcn agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update openclawcn.json with fallback chains
...
pnpm openclawcn agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
