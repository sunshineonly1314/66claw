...
description_zh: A股实时行情与分时量能分析。获取沪深股票实时价格、涨跌、成交量，分析分时量能分布（早盘/尾盘放量）、主力动向（抢筹/出货信号）、涨停封单。支持持仓管理和盈亏分析。Use when: (1) 查询A股实时行情, (2) 分析主力资金动向, (3) 查看分时成交量分布, (4) 管理股票持仓, (5) 分析持仓盈亏。
---
# A股实时行情与分时量能分析

使用东方财富网和新浪财经接口获取A股数据，支持实时行情、分时K线、量能分析。
...
直接使用6位数字代码，系统自动识别所属市场：

- **沪市**：以6开头（例如 600789）
- **深市**：以0或3开头（例如 002446、300001）
- **北交所**：以8或4开头
...
| 时段 | 说明 |
|------|------|
| 早盘30分（9:30–10:00） | 主力早盘动作，占比 >30% 为抢筹信号 |
| 上午中段（10:00–11:30） | 正常交易时段 |
| 下午中段（13:00–14:30） | 正常交易时段 |
| 尾盘30分（14:30–15:00） | 占比 >25% 为尾盘异动信号 |
...
- **早盘放量 >40%**：主力强势介入  
- **尾盘放量 >15%**：尾盘有一定放量  
- **尾盘放量 >25%**：可能存在主力抢筹或出货行为  
- **封板状态**：涨停/跌停，重点关注封单量  

### 放量时段 TOP 10

显示当日成交量最大的前10个分钟级时段，用于识别：
- 主力建仓/出货时机  
- 重大消息发布时点  
- 涨停封板/开板时刻  
...
| 数据 | 接口 | 延迟 |
|------|------|------|
| 实时行情 | `hq.sinajs.cn` | 约3秒 |
| 分时K线 | `CN_MarketDataService.getKLineData` | 约1分钟 |
...
持仓数据存储于：`~/.clawdbot/skills/a-stock-analysis/portfolio.json`
