...
name_zh: 自进化代理 v1.0.2
description: "记录学习所得、错误及修正措施，以实现持续改进。在以下情况中使用：(1) 命令或操作意外失败；(2) 用户纠正 Claude（例如‘不对，这是错的……’‘实际上……’）；(3) 用户提出当前尚不支持的功能需求；(4) 外部 API 或工具调用失败；(5) Claude 意识到自身知识已过时或不准确；(6) 为重复性任务发现了更优方法。此外，在执行重大任务前，应复核已有学习记录。"
description_zh: 记录学习所得、错误及修正措施，以实现持续改进。在以下情况中使用：(1) 命令或操作意外失败；(2) 用户纠正 Claude（例如‘不对，这是错的……’‘实际上……’）；(3) 用户提出当前尚不支持的功能需求；(4) 外部 API 或工具调用失败；(5) Claude 意识到自身知识已过时或不准确；(6) 为重复性任务发现了更优方法。此外，在执行重大任务前，应复核已有学习记录。
---
# 自我改进技能

将学习所得与错误日志记录至 Markdown 文件，以支持持续改进。编码智能体（coding agents）后续可据此生成修复方案；重要学习项则会被提升至项目级记忆（project memory）。
...
| 与现有条目相似 | 使用 `**See Also**` 建立链接，并考虑提升优先级 |
| 具广泛适用性的学习 | 提升至 `CLAUDE.md`、`AGENTS.md` 和/或 `.github/copilot-instructions.md` |
...
当某项学习具有广泛适用性（而非一次性修复）时，应将其提升至永久性项目级记忆。
...
1. **先行搜索**：`grep -r "keyword" .learnings/`  
...
通过 agent 钩子启用自动提醒功能。此功能为**可选启用**——你必须显式配置钩子。
...
| 脚本 | 钩子类型 | 用途 |
|--------|-----------|---------|
| `scripts/activator.sh` | UserPromptSubmit | 在任务完成后提醒评估学习项 |
| `scripts/error-detector.sh` | PostToolUse (Bash) | 在命令执行出错时触发 |

详见 `references/hooks-setup.md` 获取详细配置与故障排查指南。

## 自动化技能抽取

当某项学习足够重要、可复用于多种场景时，可借助提供的辅助工具将其抽取为可重用技能。
...
4. **更新原学习条目**：状态设为 `promoted_to_skill`，并添加 `Skill-Path` 字段  
5. **验证**：在全新会话中阅读该技能，确保其自包含、无需依赖原始上下文  
...
2. 参考 `assets/SKILL-TEMPLATE.md` 中的模板  
3. 遵循 [Agent Skills 规范](https://agentskills.io/specification)：  
   - YAML 前置元数据须包含 `name` 和 `description`  
   - 文件夹名须与 `name` 完全一致  
...
- [ ] 代码示例完全自包含  
- [ ] 不含任何项目专属硬编码值  
- [ ] 符合技能命名规范（全小写，使用短横线分隔）  

## 多 Agent 支持

本技能兼容各类 AI 编程智能体，支持 agent-特定激活机制。

### Claude Code

**激活方式**：钩子（UserPromptSubmit、PostToolUse）  
**配置方式**：在 `.claude/settings.json` 中配置钩子  
**检测方式**：通过钩子脚本自动触发  

### Codex CLI

**激活方式**：钩子（模式同 Claude Code）  
**配置方式**：在 `.codex/settings.json` 中配置钩子  
**检测方式**：通过钩子脚本自动触发  
...
1. **发现非显而易见的内容** —— 解决方案并非一蹴而就  
2. **自我纠正** —— 初始方案被证明有误  
3. **习得项目约定** —— 发现未文档化的实践模式  
4. **遭遇意外错误** —— 尤其诊断过程困难时  
...
| 工具陷阱（gotchas） | `TOOLS.md` | “MCP 服务器 X 需要刷新认证” |
| 项目事实 | `CLAUDE.md` | 项目标准约定 |
...
同时使用两者时：  
1. 使用 `.learnings/` 存储项目专属学习项  
2. 使用 clawdbot 工作区文件存储跨项目通用模式  
3. 将高价值学习项同步至两个系统  
