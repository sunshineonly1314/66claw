...
| `JUPITER_API_KEY` | Jupiter API 密钥（用于身份验证请求） | 否 |
...
- 高频交易或 MEV 相关操作  
- 需要增强型交易数据（如 Helius 提供的服务）  
- 对可用性要求严苛的生产环境（需保障 99.9% 正常运行时间）  
...
本技能在代币兑换操作中收取小额平台费用（0.2%），用于支持项目持续开发。该费用将在每次兑换前向用户明确披露。
...
**⚠️ 关键提示：执行任何兑换前，必须向用户完整展示兑换详情，并等待其明确确认。**
...
**重要提示**：须向用户提问“是否确认执行本次兑换？”，并严格等待明确答复（如“是”、“继续”、“确认”）后方可继续。
...
1. **始终**在执行前向用户完整展示交易详情，并等待其明确确认  
2. **绝不**在未经用户明确授权的情况下自动执行兑换或转账  
3. **始终**在发起交易前检查账户余额  
4. 若价格影响超过 1%，**必须**向用户发出警告  
5. 若滑点设置高于 1%（即 100 个基点），**必须**向用户发出警告  
6. **绝不**记录、显示或传输私钥内容  
7. 交易执行后，**始终**向用户展示交易签名及区块链浏览器链接  
...
| 错误 | 原因 | 解决方案 |
|-------|-------|----------|
| “余额不足” | 代币余额不足 | 检查余额，减少交易数量 |
| “超出滑点容忍范围” | 兑换过程中市场价格变动 | 获取最新报价，适当提高滑点设置 |
| “交易已过期” | 区块哈希（blockhash）已失效 | 获取最新报价并重试 |
| “账户未找到” | 缺少对应代币账户 | 系统将自动创建该账户 |
| “未找到兑换路径” | 当前无足够流动性 | 尝试减小交易数量，或更换交易对 |
