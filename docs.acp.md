# OpenClawCN ACP Bridge

This document describes how the OpenClawCN ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openclawcn acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenClawCN Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a OpenClawCN Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `openclawcn acp` over stdio.

Example config:

```bash
openclawcn config set gateway.remote.url wss://gateway-host:18789
openclawcn config set gateway.remote.token <token>
```

Example run:

```bash
openclawcn acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
openclawcn acp --session agent:main:main
openclawcn acp --session agent:design:main
openclawcn acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “OpenClawCN ACP” to start a thread.

## Execution Model

- ACP client spawns `openclawcn acp` and speaks ACP messages over stdio.
...
```bash
openclawcn acp --session agent:main:main
openclawcn acp --session-label "support inbox"
openclawcn acp --reset-session
```
...
`openclawcn acp` resolves the Gateway URL and auth from CLI flags or config:
