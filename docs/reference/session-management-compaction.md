...
This document explains how OpenClawCN manages sessions end-to-end:
...
OpenClawCN is designed around a single **Gateway process** that owns session state.
...
OpenClawCN persists sessions in two layers:
...
OpenClawCN resolves these via `src/config/sessions.ts`.
...
OpenClawCN intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OpenClawCN consumes the events, but Pi decides when to compact).
...
OpenClawCN also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OpenClawCN bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OpenClawCN leaves it alone.
...
- `openclawcn status` (CLI)
- `openclawcn sessions` / `sessions --json`
...
OpenClawCN supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OpenClawCN strips/suppresses this in the delivery layer.

As of `2026.1.10`, OpenClawCN also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OpenClawCN uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OpenClawCN’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `openclawcn status`.
