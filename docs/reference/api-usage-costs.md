...
OpenClawCN features that can generate provider usage or paid API calls.
...
- `openclawcn status --usage` and `openclawcn channels list` show provider **usage windows**
...
OpenClawCN can pick up credentials from:
...
### 8) Model scan / probe
`openclawcn models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
