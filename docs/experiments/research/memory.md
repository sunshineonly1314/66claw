...
read_when:
  ...
  - Deciding: standalone CLI vs deep OpenClawCN integration
...
Recommendation: **deep integration in OpenClawCN**, but keep a separable core library.

### Why integrate into OpenClawCN?
- OpenClawCN already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openclawcn memory recall "…" --k 25 --since 30d`
  - `openclawcn memory reflect --since 7d`
