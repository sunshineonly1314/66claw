...
Delivery:
- Publish as `@openclawcn/plugin-sdk` (or export from core under `openclawcn/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `OpenClawCNPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenClawCNConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenClawCNConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenClawCNConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: OpenClawCNConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenClawCNConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenClawCNConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@openclawcn/plugin-sdk`.
- Add `api.runtime` to `OpenClawCNPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openclawcnRuntime: ">=2026.2.0"`).
