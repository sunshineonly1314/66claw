...
```bash
curl -fsSL https://openclawcn.com/install.sh | bash -s -- --verbose
```

For beta installs:

```bash
curl -fsSL https://openclawcn.com/install.sh | bash -s -- --beta --verbose
```

You can also set `OPENCLAWCN_VERBOSE=1` instead of the flag.
...
### `docs.openclawcn.com` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.openclawcn.com` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.openclawcn.com` to the allowlist, then retry.
...
- Check the allowlist: `openclawcn config get agents.defaults.models`
...
If you can, include the relevant log tail from `openclawcn logs --follow`.
