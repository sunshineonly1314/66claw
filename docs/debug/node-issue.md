...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenClawCN
---

# Node + tsx "__name is not a function" crash

## Summary
Running OpenClawCN via Node with `tsx` fails at startup with:

```
[openclawcn] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openclawcn status` and `gateway:watch` worked.
