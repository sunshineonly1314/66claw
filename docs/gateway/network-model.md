...
Most operations flow through the Gateway (`openclawcn gateway`), a single long-running
...
- Loopback first: the Gateway WS defaults to `ws://127.0.0.1:18789`. The wizard generates a gateway token by default, even for loopback. For tailnet access, run `openclawcn gateway --bind tailnet --token ...` because tokens are required for non-loopback binds.
...
- Canvas host is served by the Gateway HTTP server on the **same port** as the Gateway (default `18789`):
  - `/__openclawcncn__/canvas/`
  - `/__openclawcncn__/a2ui/`
