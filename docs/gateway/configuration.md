---
summary: "All configuration options for ~/.openclawcn/openclawcn.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

OpenClawCN reads an optional **JSON5** config from `~/.openclawcn/openclawcn.json` (comments + trailing commas allowed).

If the file is missing, OpenClawCN uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
OpenClawCN only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `openclawcn doctor`, `openclawcn logs`, `openclawcn health`, `openclawcn status`, `openclawcn service`, `openclawcn help`).
- Run `openclawcn doctor` to see the exact issues.
- Run `openclawcn doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `openclawcn config set`. Keep a backup of `~/.openclawcn/openclawcn.json`.
...
```bash
openclawcn gateway call config.get --params '{}' # capture payload.hash
openclawcn gateway call config.apply --params '{
...
```bash
openclawcn gateway call config.get --params '{}' # capture payload.hash
openclawcn gateway call config.patch --params '{
...
{ "$include": "/etc/openclawcn/base.json5" } // absolute
...
OpenClawCN reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, OpenClawCN runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `OPENCLAWCN_LOAD_SHELL_ENV=1`
- `OPENCLAWCN_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${OPENCLAWCN_GATEWAY_TOKEN}"
    }
  }
}
```
...
OpenClawCN stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.openclawcn/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.openclawcn/credentials/oauth.json` (or `$OPENCLAWCN_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.openclawcn/agent/*` (migrated by `openclawcn doctor` into `~/.openclawcn/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `OPENCLAWCN_OAUTH_DIR`
- Agent dir (default agent root override): `OPENCLAWCN_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, OpenClawCN imports `oauth.json` entries into `auth-profiles.json`.
...
If set, OpenClawCN derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/openclawcn/openclawcn-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/openclawcn/openclawcn.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/openclawcn/openclawcn.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
...
- The legacy single-account Baileys auth dir is migrated by `openclawcn doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "openclawcn", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.openclawcn/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `openclawcn.json`).
...
OpenClawCN starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
OpenClawCN starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `openclawcn plugins install @openclawcn/mattermost` (or `./extensions/mattermost` from a git checkout).
...
OpenClawCN starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
OpenClawCN spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, OpenClawCN
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/openclawcn" } }
}
```
...
When a file exceeds this limit, OpenClawCN logs a warning and injects a truncated
...
message envelopes). If unset, OpenClawCN uses the host timezone at runtime.
...
`[openclawcn]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[openclawcn]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[openclawcn]"`, OpenClawCN will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, OpenClawCN generates audio
...
OpenClawCN also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, OpenClawCN currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `openclawcn configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:openclawcn`: all built-in OpenClawCN tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.openclawcn/sandboxes`
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "openclawcn-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "openclawcn-sandbox-browser:bookworm-slim",
          containerPrefix: "openclawcn-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenClawCN uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.openclawcn/agents/<agentId>/agent/models.json` or by defining the same schema inside your
OpenClawCN config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, OpenClawCN writes/merges a `models.json` into
`~/.openclawcn/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. OpenClawCN uses
...
- Shortcut: `openclawcn onboard --auth-choice opencode-zen`.
...
Shortcut: `openclawcn onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `openclawcn onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `openclawcn onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `openclawcn onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `openclawcn onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENCLAWCN_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.openclawcn/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.openclawcn/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, OpenClawCN stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.openclawcn/skills` (workspace skills
...
from `~/.openclawcn/extensions`, `<workspace>/.openclawcn/extensions`, plus any
...
OpenClawCN can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control server.
...
- Note: the control server is started by the running gateway (OpenClawCN.app menubar, or `openclawcn gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/openclawcn" }
...
- Examples: `"/ui"`, `"/openclawcn"`, `"/apps/openclawcn"`.
...
- When a connection comes from one of these IPs, OpenClawCN uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `openclawcn gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OPENCLAWCN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OPENCLAWCN_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. OpenClawCN
...
macOS app behavior:
- OpenClawCN.app watches `~/.openclawcn/openclawcn.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.openclawcn/openclawcn.json` (or `OPENCLAWCN_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.openclawcn/openclawcn.json` (or `OPENCLAWCN_CONFIG_PATH`)
...
- `OPENCLAWCN_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `openclawcn --dev …` → uses `~/.openclawcn-dev` + shifts ports from base `19001`
- `openclawcn --profile <name> …` → uses `~/.openclawcn-<name>` (port via config/env/flags)
...
Example:
```bash
OPENCLAWCN_CONFIG_PATH=~/.openclawcn/a.json \
OPENCLAWCN_STATE_DIR=~/.openclawcn-a \
openclawcn gateway --port 19001
```
...
Gmail helper config (used by `openclawcn webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "openclawcn@gmail.com",
...
- Set `OPENCLAWCN_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, OpenClawCN defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__openclawcn/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__openclawcn__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `OPENCLAWCN_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.openclawcn/bridge/tls/bridge-{cert,key}.pem when omitted.
...
Controls LAN mDNS discovery broadcasts (`_openclawcn-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclawcn-bridge._tcp` under `~/.openclawcn/dns/` using the standard discovery domain `openclawcn.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `openclawcn.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `openclawcn.internal` via that server
