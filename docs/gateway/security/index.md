...
## Quick check: `openclawcn security audit`
...
- Tighten local perms (`~/.openclawcn` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

OpenClawCN is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, OpenClawCN also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**: `~/.openclawcn/credentials/<channel>-allowFrom.json`
...
`openclawcn security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${OPENCLAWCN_GATEWAY_PASSWORD}
```
...
OpenClawCN stores session transcripts on disk under `~/.openclawcn/agents/<agentId>/sessions/*.jsonl`.
...
OpenClawCN can refresh the skills list mid-session:
...
- If you install plugins from npm (`openclawcn plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openclawcn/extensions/<pluginId>/` (or `$OPENCLAWCN_STATE_DIR/extensions/<pluginId>/`).
  - OpenClawCN uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
openclawcn pairing list <channel>
openclawcn pairing approve <channel> <code>
```
...
By default, OpenClawCN routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenClawCN has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.openclawcn/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
4. **Re-run audit**
   - `openclawcn security audit --deep` and confirm the report is clean.
...
- `~/.openclawcn/openclawcn.json`: `600` (user read/write only)
- `~/.openclawcn`: `700` (user only)

`openclawcn doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENCLAWCN_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__openclawcncn__/canvas/` and `/__openclawcncn__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_openclawcncn-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `OPENCLAWCN_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `openclawcn doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENCLAWCN_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `OPENCLAWCN_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenClawCN
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. OpenClawCN verifies the identity by resolving the
...
- OpenClawCN will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.openclawcn/` (or `$OPENCLAWCN_STATE_DIR/`) may contain secrets or private data:

- `openclawcn.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `openclawcn status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openclawcn", "@mybot"] }
      }
    ]
  }
}
...
- Prefer a dedicated profile for the agent (the default `openclawcn` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only OpenClawCN clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openclawcn gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENCLAWCN_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/openclawcn/openclawcncn-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openclawcn/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + OpenClawCN version
...
Found a vulnerability in OpenClawCN? Please report responsibly:

1. Email: [security@openclawcncn.com](mailto:security@openclawcncn.com)
