...
description: "Complete field-by-field reference for ~/.openclawcn/openclawcn.json"
---

# Configuration Reference

Every field available in `~/.openclawcn/openclawcn.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenClawCN uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `openclawcn doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["openclawcncn-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-openclawcn",
          requireMention: false,
...
Mattermost ships as a plugin: `openclawcn plugins install @openclawcn/mattermost`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@openclawcn", "openclawcn"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@openclawcn"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `openclawcn.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenClawCN auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenClawCN assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        docker: {
          image: "openclawcncn-sandbox:bookworm-slim",
          containerPrefix: "openclawcncn-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "openclawcncn-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "openclawcncn-sandbox-browser:bookworm-slim",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
    ...
    list: [
      {
        ...
        groupChat: { mentionPatterns: ["@openclawcn"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:openclawcn`   | All built-in tools (excludes provider plugins)                                           |
...
OpenClawCN uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openclawcn/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENCLAWCN_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `openclawcn onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `openclawcn onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `openclawcn onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `openclawcn onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `openclawcn onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `openclawcn onboard --auth-choice minimax-api`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPENCLAWCN_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENCLAWCN_GATEWAY_PORT` > `gateway.port` > `18789`.
...
OPENCLAWCN_STATE_DIR=~/.openclawcncn-a \
openclawcn gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.openclawcncn-dev` + port `19001`), `--profile <name>` (uses `~/.openclawcncn-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-openclawcncn-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENCLAWCN_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OPENCLAWCN_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__openclawcncn__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclawcncn__/a2ui/`
...
- Also serves A2UI at `/__openclawcncn__/a2ui/`.
...
- Hostname defaults to `openclawcn`. Override with `OPENCLAWCN_MDNS_HOSTNAME`.
...
Setup: `openclawcn dns setup --apply`.
...
{
  gateway: {
    auth: { token: "${OPENCLAWCN_GATEWAY_TOKEN}" },
  },
}
```
...
- Default log file: `/tmp/openclawcn/openclawcncn-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `openclawcn doctor --fix` can strip unknown keys).
