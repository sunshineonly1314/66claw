...
- Replaces the legacy `gateway` command. CLI entry point: `openclawcn gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
openclawcn gateway --port 18789
# for full debug/trace logs in stdio:
openclawcn gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
openclawcn gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.openclawcn/openclawcn.json` (or `OPENCLAWCN_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openclawcn__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `OPENCLAWCN_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OPENCLAWCN_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OPENCLAWCN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.openclawcn.<profile>`
- Linux: `openclawcn-gateway-<profile>.service`
- Windows: `OpenClawCN Gateway (<profile>)`

Install metadata is embedded in the service config:
- `OPENCLAWCN_SERVICE_MARKER=openclawcn`
- `OPENCLAWCN_SERVICE_KIND=gateway`
- `OPENCLAWCN_SERVICE_VERSION=<version>`
...
```bash
openclawcn --dev setup
openclawcn --dev gateway --allow-unconfigured
# then target the dev instance:
openclawcn --dev status
openclawcn --dev health
```

Defaults (can be overridden via env/flags/config):
- `OPENCLAWCN_STATE_DIR=~/.openclawcn-dev`
- `OPENCLAWCN_CONFIG_PATH=~/.openclawcn-dev/openclawcn.json`
- `OPENCLAWCN_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- Base port = `gateway.port` (or `OPENCLAWCN_GATEWAY_PORT` / `--port`)
- `browser.controlUrl port = base + 2` (or `OPENCLAWCN_BROWSER_CONTROL_URL` / config override)
- `canvasHost.port = base + 4` (or `OPENCLAWCN_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `OPENCLAWCN_CONFIG_PATH`
- unique `OPENCLAWCN_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
openclawcn --profile main gateway install
openclawcn --profile rescue gateway install
```

Example:
```bash
OPENCLAWCN_CONFIG_PATH=~/.openclawcn/a.json OPENCLAWCN_STATE_DIR=~/.openclawcn-a openclawcn gateway --port 19001
OPENCLAWCN_CONFIG_PATH=~/.openclawcn/b.json OPENCLAWCN_STATE_DIR=~/.openclawcn-b openclawcn gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `openclawcn health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `openclawcn`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `openclawcn gateway install` writes `~/Library/LaunchAgents/com.openclawcn.gateway.plist`
    (or `com.openclawcn.<profile>.plist`).
  - `openclawcn doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are OpenClawCN profile services.
  ...
  - Cleanup: `openclawcn gateway uninstall` (current service) and `openclawcn doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `openclawcn gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- OpenClawCN.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.openclawcn.gateway` (or `com.openclawcn.<profile>`).
- To stop it cleanly, use `openclawcn gateway stop` (or `launchctl bootout gui/$UID/com.openclawcn.gateway`).
- To restart, use `openclawcn gateway restart` (or `launchctl kickstart -k gui/$UID/com.openclawcn.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `openclawcn gateway install` first.
  - Replace the label with `com.openclawcn.<profile>` when running a named profile.

## Supervision (systemd user unit)
OpenClawCN installs a **systemd user service** by default on Linux/WSL2. We
...
`openclawcn gateway install` writes the user unit. `openclawcn doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/openclawcn-gateway[-<profile>].service`:
```
[Unit]
Description=OpenClawCN Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/openclawcn gateway --port 18789
Restart=always
RestartSec=5
Environment=OPENCLAWCN_GATEWAY_TOKEN=
...
```
systemctl --user enable --now openclawcn-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/openclawcn-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now openclawcn-gateway[-<profile>].service
```
...
## CLI helpers
- `openclawcn gateway health|status` — request health/status over the Gateway WS.
- `openclawcn message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `openclawcn agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `openclawcn gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `openclawcn gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `openclawcn gateway` and the legacy TCP control port.
