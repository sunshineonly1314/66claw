...
OpenClawCN can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. OpenClawCN requires a shared password.
...
- `token` (default when `OPENCLAWCN_GATEWAY_TOKEN` is set)
- `password` (shared secret via `OPENCLAWCN_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. OpenClawCN verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
OpenClawCN only treats a request as Serve when it arrives from loopback with
...
Prefer `OPENCLAWCN_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
openclawcn gateway --tailscale serve
openclawcn gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want OpenClawCN to undo `tailscale serve`
