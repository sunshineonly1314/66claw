...
`openclawcn doctor` is the repair + migration tool for OpenClawCN. It fixes stale
...
```bash
openclawcn doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
openclawcn doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
openclawcn doctor --repair --force
```
...
```bash
cat ~/.openclawcn/openclawcn.json
```
...
- Extra workspace dir detection (`~/openclawcn`).
...
you to run `openclawcn doctor`.

Doctor will:
- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.openclawcn/openclawcn.json` with the updated schema.
...
migrated via `openclawcn doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.openclawcn` folders exist across
  home directories or when `OPENCLAWCN_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.openclawcn/openclawcn.json` is
...
offers to remove them and install the OpenClawCN service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named OpenClawCN gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `openclawcn doctor --generate-gateway-token` to force token
...
Notes:
- `openclawcn doctor` prompts before rewriting supervisor config.
- `openclawcn doctor --yes` accepts the default repair prompts.
- `openclawcn doctor --repair` applies recommended fixes without prompts.
- `openclawcn doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `openclawcn gateway install --force`.
