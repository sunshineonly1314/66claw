...
The Gateway broadcasts its presence via mDNS (`_openclawcn-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
- and authenticate with the same token (`OPENCLAWCN_BROWSER_CONTROL_TOKEN` env preferred)
...
- Prefer env vars for the token (`OPENCLAWCN_BROWSER_CONTROL_TOKEN`) instead of storing it in config on disk.
...
1. Check Gateway logs: `/tmp/openclawcn/openclawcn-YYYY-MM-DD.log` (or `logging.file`).
...
1. Email: security@openclawcn.com
