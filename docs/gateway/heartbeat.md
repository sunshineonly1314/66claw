...
- During heartbeat runs, OpenClawCN treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openclawcn sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenClawCN skips the heartbeat run entirely (no model call).
