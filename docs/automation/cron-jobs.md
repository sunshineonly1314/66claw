...
- Run history: `~/.openclawcn/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
openclawcn cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openclawcn cron edit <jobId> --agent ops
openclawcn cron edit <jobId> --clear-agent
```
```
...
```bash
openclawcn system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`openclawcn system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `OPENCLAWCN_SKIP_CRON`.
