...
Remote Gateway:
```bash
openclawcn tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openclawcn tui --deliver`
...
- Check the Gateway logs: `openclawcn logs --follow`.
- Confirm the agent can run: `openclawcn status` and `openclawcn models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `openclawcn agents list` and your routing config.
