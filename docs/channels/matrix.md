...
Matrix is an open, decentralized messaging protocol. OpenClawCN connects as a Matrix **user**
...
```bash
openclawcn plugins install @openclawcn/matrix
```

Local checkout (when running from a git repo):

```bash
openclawcn plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openclawcn plugins install @openclawcn/matrix`
   - From a local checkout: `openclawcn plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenClawCN calls the same
...
- On first connection, OpenClawCN requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenClawCN logs a warning.
...
- Approve via:
  - `openclawcn pairing list matrix`
  - `openclawcn pairing approve matrix <CODE>`
...
- On startup, OpenClawCN resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
