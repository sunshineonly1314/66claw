...
```bash
openclawcn channels login --channel whatsapp --account work
```
...
<Note>
OpenClawCN recommends running WhatsApp on a separate number when possible. (The channel metadata and onboarding flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for OpenClawCN
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current OpenClawCN channel architecture.
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[openclawcn]`
...
<AccordionGroup>
  ...
  <Accordion title="Logout behavior">
    `openclawcn channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
