...
```bash
openclawcn plugins install @openclawcn/msteams
```

Local checkout (when running from a git repo):
```bash
openclawcn plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OpenClawCN will offer the local install path automatically.
...
3) Configure OpenClawCN with those credentials.
...
## Goals
- Talk to OpenClawCN via Teams DMs, group chats, or channels.
...
- On startup, OpenClawCN resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.openclawcn/openclawcn.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OpenClawCN, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `openclawcn-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `openclawcn plugins install @openclawcn/msteams`
   - From a local checkout: `openclawcn plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OpenClawCN in Teams", "full": "OpenClawCN in Teams" },
...
OpenClawCN handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OpenClawCN only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OpenClawCNShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
OpenClawCN sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `openclawcn message poll --channel msteams --target conversation:<id> ...`
...
**CLI:**
```bash
openclawcn message send --channel msteams \
...
openclawcn message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
openclawcn message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
openclawcn message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
openclawcn message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
