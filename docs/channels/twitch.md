...
read_when:
  - Setting up Twitch chat integration for OpenClawCN
---
# Twitch (plugin)

Twitch chat support via IRC connection. OpenClawCN connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openclawcn plugins install @openclawcn/twitch
```

Local checkout (when running from a git repo):

```bash
openclawcn plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `OPENCLAWCN_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openclawcn",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use OPENCLAWCN_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENCLAWCN_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
