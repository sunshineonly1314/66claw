...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `openclawcn doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `openclawcn logs --follow`
...
- DM messages can carry `message_thread_id`; OpenClawCN routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    OpenClawCN can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, OpenClawCN retries as plain text.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, OpenClawCN enqueues system events like:
...
```bash
openclawcn message send --channel telegram --target 123456789 --message "hi"
openclawcn message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `openclawcn channels status` warns when config expects unmentioned group messages.
    - `openclawcn channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `openclawcn logs --follow` for skip reasons
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `openclawcn doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `openclawcn doctor --fix` can resolve legacy `@username` entries to IDs.
