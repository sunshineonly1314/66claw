...
```bash
openclawcn plugins install @openclawcn/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
openclawcn plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "OpenClawCN" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openclawcn pairing list nextcloud-talk`
  - `openclawcn pairing approve nextcloud-talk <CODE>`
