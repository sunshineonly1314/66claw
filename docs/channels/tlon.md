...
Tlon is a decentralized messenger built on Urbit. OpenClawCN connects to your Urbit ship and can
...
```bash
openclawcn plugins install @openclawcn/tlon
```

Local checkout (when running from a git repo):

```bash
openclawcn plugins install ./extensions/tlon
```
...
By default, OpenClawCN blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `openclawcn message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenClawCN replies in-thread.
