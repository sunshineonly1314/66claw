...
read_when:
  - Setting up Zalo Personal for OpenClawCN
...
- Install via CLI: `openclawcn plugins install @openclawcn/zalouser`
- Or from a source checkout: `openclawcn plugins install ./extensions/zalouser`
...
Approve via:
- `openclawcn pairing list zalouser`
- `openclawcn pairing approve zalouser <code>`
...
- On startup, OpenClawCN resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `openclawcn channels logout --channel zalouser && openclawcn channels login --channel zalouser`
