---
summary: "Use Venice AI privacy-focused models in OpenClawCN"
read_when:
  - You want privacy-focused inference in OpenClawCN
...
```bash
openclawcn onboard --auth-choice venice-api-key
```
...
```bash
openclawcn onboard --non-interactive \
...
```bash
openclawcn chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OpenClawCN shows all available Venice models. Pick based on your needs:
...
```bash
openclawcn models set venice/claude-opus-45
openclawcn models set venice/llama-3.3-70b
```

List all available models:

```bash
openclawcn models list | grep venice
```

## Configure via `openclawcn configure`

1. Run `openclawcn configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenClawCN automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openclawcn chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
openclawcn chat --model venice/claude-opus-45

# Use uncensored model
openclawcn chat --model venice/venice-uncensored

# Use vision model with image
openclawcn chat --model venice/qwen3-vl-235b-a22b

# Use coding model
openclawcn chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `openclawcn models list` to see currently available models. Some models may be temporarily offline.
