---
summary: "Use Synthetic's Anthropic-compatible API in OpenClawCN"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OpenClawCN registers it as the
...
```bash
openclawcn onboard --auth-choice synthetic-api-key
```
...
Note: OpenClawCN's Anthropic client appends `/v1` to the base URL, so use
