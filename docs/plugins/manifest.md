...
read_when:
  - You are building a OpenClawCN plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (openclawcn.plugin.json)

Every plugin **must** ship a `openclawcn.plugin.json` file in the **plugin root**.
OpenClawCN uses this manifest to validate configuration **without executing plugin
