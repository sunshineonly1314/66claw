...
| 变量 | 作用 | 生产环境 |
|------|------|----------|
| `OPENCLAWCN_CN=1` | 启用 CN 版本授权检查 | 需要设置 |
| `OPENCLAWCN_REGION=cn` | 同上 | 需要设置 |
| `OPENCLAWCN_DEV=1` | DEV 模式（仅开发构建有效） | **无效** |
| `OPENCLAWCN_LICENSE_DEV=1` | DEV 模式（仅开发构建有效） | **无效** |
...
```bash
# 授权验证成功
grep "License check passed" /var/log/openclawcn.log

# RSA 验证
grep "RSA signature verification" /var/log/openclawcn.log

# 反调试
grep "security:anti-debug" /var/log/openclawcn.log

# 完整性校验
grep "security:integrity" /var/log/openclawcn.log

# 心跳
grep "license:heartbeat" /var/log/openclawcn.log
```
...
# 开发环境启用 DEV 模式
# 注意：生产构建中此方法无效！
export OPENCLAWCN_LICENSE_DEV=1
pnpm dev
```
