...
```bash
gcloud projects create my-openclawcncn-project --name="OpenClawCN Gateway"
gcloud config set project my-openclawcncn-project
```
...
```bash
gcloud compute instances create openclawcncn-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh openclawcncn-gateway --zone=us-central1-a
```
...
OPENCLAWCN_WORKSPACE_DIR=/home/$USER/.openclawcn/workspace
...
```bash
gcloud compute ssh openclawcncn-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
```bash
# Stop the VM first
gcloud compute instances stop openclawcncn-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type openclawcncn-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start openclawcncn-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create openclawcncn-deploy \
...
2. Grant Compute Instance Admin role (or narrower custom role):
   ...
   ```bash
   gcloud projects add-iam-policy-binding my-openclawcncn-project \
     --member="serviceAccount:openclawcncn-deploy@my-openclawcncn-project.iam.gserviceaccount.com" \
