...
| 注册来源 | `skills/` 目录 + gitee/clawdskillsproxy 远程仓库 | `openclawcn.yaml` → `mcp.servers[]` |
...
| 去重逻辑 | 无 | `openclawcn-tools.ts:169-177` 按 tool name 去重 |
| 规模 | 3000+ | 通常 20 个以内 |
...
- [ ] 在 `openclawcn-tools.ts` 中合并 skill entries + MCP tools 为统一索引
...
| `src/agents/openclawcn-tools.ts:149-179` | ClawdBot 工具 + 插件 + MCP 合并 |
