...
2. **会话中工具注入** — agent session 创建后工具列表冻结（`src/agents/openclawcn-tools.ts:167-179`）
...
**位置**: `src/agents/openclawcn-tools.ts:167-179`
...
```typescript
// src/agents/openclawcn-tools.ts — 修改工具组装流程

export async function createOpenClawCNTools(options: {
...
**位置**: `src/config/types.openclawcn.ts` / `src/config/zod-schema.ts`
...
- [ ] 修改 `src/agents/openclawcn-tools.ts` — session 创建前调用路由
