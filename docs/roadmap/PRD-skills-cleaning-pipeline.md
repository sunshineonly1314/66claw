...
OpenClaw/OpenClawCN 社区 skills registry 目前收录了 **3000+** 个社区技能（awesome-openclaw-skills 精选了 1715+）。这些技能的 SKILL.md 内容会被 **直接注入 AI Agent 的系统提示词**，是一个高危攻击面：
...
| 组件 | 技术选型 | 说明 |
|------|---------|------|
| 清洗引擎 | TypeScript (Node.js 22+) | 与 OpenClawCN 主工程一致 |
...
export const DANGEROUS_PATTERNS: PatternMatch[] = [
  ...
  },
  {
    ...
    category: "data_exfil",
    severity: "critical",
    description: "访问 OpenClawCN 凭据目录",
    regex: /\.openclawcn[/\\]credentials/i,
  },
  {
    pattern_id: "DE-004",
    category: "data_exfil",
...
- 访问 ~/.openclawcn/credentials/、~/.ssh/、.env 等敏感路径
...
检测技能是否：
- 名称伪装成系统级/官方技能（如 "system-core"、"openclawcn-update"）
- 使用紧急/恐吓性语言迫使用户执行操作
- 诱导用户关闭安全功能或泄露敏感信息
...
3. frontmatter 中的 metadata.openclawcn.install 字段定义了安装行为，需审查
...
```bash
# 完整流水线
openclawcn skills wash --input ./raw-skills --output ./cleaned-skills

# 仅运行 Layer 1 (规则引擎，不消耗 API)
openclawcn skills wash --input ./raw-skills --layer 1 --output ./layer1-report

# 仅运行安全审计 (Layer 1 + Layer 2)
openclawcn skills wash --input ./raw-skills --layer 2 --output ./security-report

# 审计单个技能
openclawcn skills wash --skill ./skills/brave-search/SKILL.md

# 从断点续跑
openclawcn skills wash --resume ./skills-wash-checkpoint.json

# 指定并发数和模型
openclawcn skills wash --input ./raw-skills --concurrency 10 --model qwen-max

# 仅汉化 (跳过安全审计，用于已审核的技能)
openclawcn skills wash --input ./cleaned-skills --translate-only --output ./zh-skills

# 查看清洗报告
openclawcn skills wash report --input ./cleaned-skills/report/

# 导出到 ClawdSkillsProxy
openclawcn skills wash export --input ./cleaned-skills --target proxy
```
