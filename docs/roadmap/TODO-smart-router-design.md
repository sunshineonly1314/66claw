# OpenClawCN 智能路由 (Smart Router) 设计方案

## Context

当前 openclawcn 的模型选择是静态的 — 所有消息都走同一个模型和 thinking level。不管用户发"你好"还是发 500 行代码让 debug，用的都是同一个配置。Performance Profile (economy/balanced/power) 只调 thinking 深度，不换模型本身。
...
**5. `src/config/types.openclawcn.ts`** — 加 2 行
...
```typescript
// ========================================
// OpenClawCN 专属功能：智能路由 (Smart Router)
// ========================================
...
| `src/config/types.openclawcn.ts` | 修改 | +3 行 |
