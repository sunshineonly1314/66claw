---
summary: "First-run onboarding flow for OpenClawCN (macOS app)"
read_when:
...
- Writes credentials to `~/.openclawcn/credentials/oauth.json`
...
The app can install the global `openclawcn` CLI via npm/pnpm so terminal
...
On the first agent run, OpenClawCN bootstraps a workspace (default `~/clawd`):
...
```bash
openclawcn webhooks gmail setup --account you@gmail.com
```
