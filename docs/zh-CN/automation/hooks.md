...
Hooks 提供了一个可扩展的事件驱动系统，用于响应智能体命令和事件自动执行操作。Hooks 从目录中自动发现，可以通过 CLI 命令管理，类似于 OpenClawCN 中 Skills 的工作方式。
...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 OpenClawCN 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `openclawcn webhooks` 获取 Gmail 助手命令。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 OpenClawCN 的行为

## 入门

### 捆绑的 Hooks

OpenClawCN 附带三个自动发现的捆绑 hooks：
...
在新手引导期间（`openclawcn onboard`），你将被提示启用推荐的 hooks。向导会自动发现符合条件的 hooks 并呈现供选择。
...
3. **捆绑 hooks**：`<openclawcn>/dist/hooks/bundled/`（随 OpenClawCN 附带）
...
Hook 包是标准的 npm 包，通过 `package.json` 中的 `openclawcn.hooks` 导出一个或多个 hooks。使用以下命令安装：
...
这些 hooks 不是事件流监听器；它们让插件在 OpenClawCN 持久化工具结果之前同步调整它们。
...
- [捆绑 Hooks README](https://github.com/openclawcn/openclawcn/tree/main/src/hooks/bundled)
