...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openclawcn sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openclawcn doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
1. **Check agent resolution:**
   ```exec
   openclawcn agents list --bindings
   ```

2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=openclawcn.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${OPENCLAWCN_STATE_DIR:-$HOME/.openclawcn}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
