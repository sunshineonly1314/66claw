...
- `/usage cost` in chats: local cost summary aggregated from OpenClawCN session logs.
- CLI: `openclawcn status --usage` prints a full per-provider breakdown.
- CLI: `openclawcn channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
