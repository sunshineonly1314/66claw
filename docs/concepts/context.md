...
read_when:
  - You want to understand what “context” means in OpenClawCN
...
“Context” is **everything OpenClawCN sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (OpenClawCN-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How OpenClawCN builds the system prompt

The system prompt is **OpenClawCN-owned** and rebuilt each run. It includes:
...
By default, OpenClawCN injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). OpenClawCN also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `24000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
