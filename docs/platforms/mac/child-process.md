...
service via the external `openclawcn` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `com.openclawcn.gateway`
  (or `com.openclawcn.<profile>` when using `--profile`/`OPENCLAWCN_PROFILE`).
...
```bash
launchctl kickstart -k gui/$UID/com.openclawcn.gateway
