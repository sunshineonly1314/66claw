...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `com.openclawcn`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "com.openclawcn" AND category CONTAINS "voicewake"' --level info --style compact
  ```
