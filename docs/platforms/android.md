...
- You can run the CLI (`openclawcn`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.openclawcn/openclawcn.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `openclawcn.internal.`) on the gateway host and publish `_openclawcncn-gw._tcp` records.
...
```bash
openclawcn nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__openclawcncn__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__openclawcncn__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__openclawcncn__/a2ui/`.
