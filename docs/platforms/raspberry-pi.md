---
summary: "OpenClawCN on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenClawCN on a Raspberry Pi
  - Running OpenClawCN on ARM devices
  - Building a cheap always-on personal AI
---

# OpenClawCN on Raspberry Pi

## Goal

Run a persistent, always-on OpenClawCN Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenClawCN features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openclawcn
...
```bash
# Check logs
journalctl -u openclawcn --no-pager -n 100

# Common fix: rebuild
cd ~/openclawcn  # if using hackable install
npm run build
sudo systemctl restart openclawcn
```
