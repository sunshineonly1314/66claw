---
summary: "Run OpenClawCN Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: OpenClawCN Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install OpenClawCN  
2) Run `openclawcn onboard --install-daemon`  
...
```bash
ssh exe.dev new --name=openclawcn
```

Then connect:

```bash
ssh openclawcn.exe.xyz
```

Tip: keep this VM **stateful**. OpenClawCN stores state under `~/.openclawcn/` and `~/clawd/`.
...
- `~/.openclawcn/openclawcn.json` config
- model auth profiles
...
```bash
ssh -N -L 18789:127.0.0.1:18789 openclawcn.exe.xyz
```
...
```bash
export OPENCLAWCN_GATEWAY_TOKEN="$(openssl rand -hex 32)"
openclawcn gateway --bind lan --port 8080 --token "$OPENCLAWCN_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.openclawcn/openclawcn.json`:
...
Notes:
- Non-loopback binds require `gateway.auth.token` (or `OPENCLAWCN_GATEWAY_TOKEN`).
- `gateway.remote.token` is only for remote CLI calls; it does not enable local auth.

Then point exe.dev’s proxy at `8080` (or whatever port you chose) and open your VM’s HTTPS URL:

```bash
ssh exe.dev share port openclawcn 8080
```

Open:
- `https://openclawcn.exe.xyz/`
...
On Linux, OpenClawCN uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status openclawcn-gateway[-<profile>].service
```
