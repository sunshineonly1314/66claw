---
title: Lobster
summary: "Typed workflow runtime for OpenClawCN with resumable approval gates."
description: Typed workflow runtime for OpenClawCN — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets OpenClawCN run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: OpenClawCN runs one Lobster tool call and gets a structured result.
...
OpenClawCN launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In OpenClawCN tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the OpenClawCN Gateway (see the [Lobster repo](https://github.com/openclawcn/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), OpenClawCN keeps core tools enabled. To restrict core
