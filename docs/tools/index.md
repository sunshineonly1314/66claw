---
summary: "Agent tool surface for OpenClawCN (browser, canvas, nodes, message, cron) replacing legacy `openclawcncn-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openclawcncn-*` skills
title: "Tools"
---

# Tools (OpenClawCN)

OpenClawCN exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openclawcncn-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openclawcn.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenClawCN logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`openclawcn node run`).
...
- Requires a Brave API key (recommended: `openclawcn configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenClawCN-managed browser.
...
- Quick smoke: `openclawcn nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openclawcn gateway` restart in-place)
