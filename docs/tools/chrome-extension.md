---
summary: "Chrome extension: let OpenClawCN drive your existing Chrome tab"
read_when:
...
The OpenClawCN Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
OpenClawCN then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenClawCN release (npm package) as static files. There is no separate “build” step.

After upgrading OpenClawCN:
- Re-run `openclawcn browser extension install` to refresh the installed files under your OpenClawCN state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenClawCN ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OpenClawCN to control.
...
- `ON`: attached; OpenClawCN can drive that tab.
- `…`: connecting to the local relay.
- `!`: relay not reachable (most common: browser relay server isn’t running on this machine).

If you see `!`:
- Make sure the Gateway is running locally (default setup), or run `openclawcn browser serve` on this machine (remote gateway setup).
- Open the extension Options page; it shows whether the relay is reachable.

## Do I need `openclawcn browser serve`?
...
you typically **do not** need `openclawcn browser serve`.

The Gateway’s built-in browser control server will start on `http://127.0.0.1:18791/` and OpenClawCN will
auto-start the local relay server on `http://127.0.0.1:18792/`.

### Remote Gateway (Gateway runs elsewhere) — **yes**

If your Gateway runs on another machine, run `openclawcn browser serve` on the machine that runs Chrome
...
Debugging: `openclawcn sandbox explain`
...
```bash
export OPENCLAWCN_BROWSER_CONTROL_TOKEN="<token>"
```
...
`openclawcn browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openclawcn browser extension install` first to copy the extension to a stable location under your OpenClawCN state directory.
