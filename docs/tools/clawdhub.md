...
ClawdHub is the **public skill registry for OpenClawCN**. It is a free service: all skills are public, open, and visible to everyone for sharing and reuse. A skill is just a folder with a `SKILL.md` file (plus supporting text files). You can browse skills in the web app or use the CLI to search, install, update, and publish skills.

Site: [clawdhub.com](https://clawdhub.com)

## Who this is for (beginner-friendly)

If you want to add new capabilities to your OpenClawCN agent, ClawdHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4) Start a new OpenClawCN session so it picks up the new skill.
...
## How it fits into OpenClawCN

By default, the CLI installs skills into `./skills` under your current working directory. If a OpenClawCN workspace is configured, `clawdhub` falls back to that workspace unless you override `--workdir` (or `CLAWDHUB_WORKDIR`). OpenClawCN loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.openclawcn/skills` or bundled skills, workspace skills take precedence.
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to OpenClawCN workspace).
...
`clawdhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/openclawcn/skills` and `~/.openclawcn/skills`). This is designed to find older skill installs without extra flags.
