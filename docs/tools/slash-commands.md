...
- `commands.config` (default `false`) enables `/config` (reads/writes `openclawcn.json`).
...
- For full provider usage breakdown, use `openclawcn status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from OpenClawCN session logs.
...
Notes:
- Overrides apply immediately to new config reads, but do **not** write to `openclawcn.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`openclawcn.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[openclawcn]"
