...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `openclawcn/plugin-sdk` alias is a prefix match.
    alias: [
      {
        find: "openclawcn/plugin-sdk/account-id",
        replacement: path.join(repoRoot, "src", "plugin-sdk", "account-id.ts"),
      },
      {
        find: "openclawcn/plugin-sdk",
  ...
  test: {
    ...
    include: ["src/**/*.test.ts", "extensions/**/*.test.ts", "test/format-error.test.ts", "test/memory/**/*.test.ts"],
